...
export function getAgeGroupForUser(user) {
  return user?.ageGroup ?? getAgeGroupByBirthYear(user?.birthYear) ?? "open";
}

export function inferRegionSelection(region = "") {
  const normalized = String(region).replace(/\s/g, "");
  for (const group of REGION_TREE) {
    const district = group.districts.find((item) => normalized.includes(item.replace(/[시군구]$/u, "")) || normalized.includes(item));
