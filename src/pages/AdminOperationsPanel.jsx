import { AlertTriangle, CheckCircle2, Clock3, Database, RadioTower } from "lucide-react";
import Metric from "../components/admin/AdminMetric.jsx";
import AdminDashboardPanel from "../components/admin/AdminDashboardPanel.jsx";
import { ADMIN_DASHBOARD_MIN_LEVEL } from "../../shared/lib/adminDashboardPolicy.js";
...
export default function AdminOperationsPanel({ controller }) {
  return (
    <div className="admin-operations-dashboard">
      {controller.adminLevel >= ADMIN_DASHBOARD_MIN_LEVEL ? <AdminDashboardPanel loadDashboard={controller.app.actions.loadAdminDashboard} /> : null}
      <ReportOperationsPanel controller={controller} />
    </div>
  );
}

function ReportOperationsPanel({ controller }) {
  ...
  if (!operations) {
    return (
      <Card className="section-card admin-queue-state" role={hasError ? "alert" : "status"}>
        <strong>{hasError ? "신고 현황을 불러오지 못했습니다." : "신고 현황을 불러오는 중입니다."}</strong>
  ...
  return (
    <section className="admin-operations-dashboard" aria-label="신고 현황">
      <div className="section-title-row">
        <h2>처리할 신고</h2>
        <Button type="button" variant="secondary" disabled={isLoading} onClick={refreshQueue}>신고 새로고침</Button>
      </div>
      {hasError ? <div className="ui-empty-state-compact" role="alert">신고 현황을 갱신하지 못했습니다. 다시 새로고침해 주세요.</div> : null}
      <section className="admin-operation-metrics" aria-label="신고 운영 지표">
        ...
        <Metric label="오늘 처리" value={metrics.processedToday} detail="실제 처리 완료 시각 기준" onClick={() => navigateToReportQueue("processedToday", "history")} />
        <Metric
          label="가장 오래된 미처리"
      ...
      <div className="admin-operation-lower-grid">
          ...
          {operations.timings.firstResponse || operations.timings.processing ? (
            <div className="admin-timing-list">
              {operations.timings.firstResponse ? (
                <div className="ui-panel"><span>첫 담당 배정까지</span><strong>{formatDuration(operations.timings.firstResponse.averageMs)}</strong><small>담당 배정 {operations.timings.firstResponse.sampleCount}건 기준</small></div>
              ) : null}
              {operations.timings.processing ? (
                <div className="ui-panel"><span>접수부터 처리까지</span><strong>{formatDuration(operations.timings.processing.averageMs)}</strong><small>처리 완료 {operations.timings.processing.sampleCount}건 기준</small></div>
              ) : null}
            </div>
        ...
        <Card className="section-card admin-operation-card">
          ...
          <div className="admin-system-list">
            ...
            <DeliveryStatus icon={RadioTower} title="Discord 알림" status={operations.systems.discordBridge} sampleLabel="최근 발송" />
            <DeliveryStatus icon={Database} title="리마인더 알림" status={operations.systems.reminderWorker} sampleLabel="최근 발송" />
          </div>
