...
function getDefaultJoinDraft(post, teams, currentUser, state) {
  ...
  const teamOnly = isTeamOnlyRoom(post) && !individualOnlyRoom;
        ...
        {memberIds.map((playerId) => {
          ...
          return (
            <div key={playerId} className={["arena-record-roster-row", rosterIds.has(playerId) ? "selected" : "", !eligible ? "ineligible" : ""].filter(Boolean).join(" ")}>
              ...
              {reserveCapacity > 0 ? (
                <Button type="button" size="sm" variant={isReserve ? "primary" : "secondary"} disabled={!eligible || (!tournamentRoster && isLeader) || (!isReserve && reserveIds.length >= reserveCapacity)} onClick={() => setPlayerState(playerId, "reserve")}>
                  후보
                </Button>
              ) : null}
...
function MatchRecordParticipantSetupPanel({ match, users, teams, currentUserId, onSave }) {
  ...
  const userById = useMemo(() => ({ ...Object.fromEntries(users.map((user) => [user.id, user])), ...userSnapshots }), [userSnapshots, users]);
  ...
  const togglePlayer = (sideName, user) => {
    ...
    const nextIds = selected ? ownIds.filter((playerId) => playerId !== user.id) : [...ownIds, user.id].slice(0, capacity);
  ...
  const renderUserResult = (sideName) => (user) => (
    <div key={user.id} className="search-picker-result-row search-picker-result-row-actionable">
      <button type="button" className="search-picker-result-main" disabled={selectedIds.has(user.id)} onClick={() => togglePlayer(sideName, user)}>
        <strong>{user.name}</strong>
  ...
  const renderSelectedPlayers = (sideName, playerIds) => (
    <div className="arena-record-setup-selected">
      {playerIds.map((playerId) => {
        ...
        return (
          <button key={playerId} type="button" disabled={fixed} onClick={() => togglePlayer(sideName, user)}>
            <ProfileEmblem user={user} className="small" />
            <span><strong>{user?.name ?? "선수"}</strong><em>{fixed ? "방장 고정" : "선택 해제"}</em></span>
          </button>
        );
      })}
    </div>
  ...
  const renderTeamResult = (team) => (
    <div key={team.id} className="search-picker-result-row search-picker-result-row-actionable">
      <button type="button" className="search-picker-result-main" onClick={() => { setTeamBSnapshot(team); setTeamQuery(""); setFeedback(""); }}>
    ...
    <div className="arena-record-setup-panel">
      ...
      {composition === "individual" ? (
        ...
        <div className="arena-record-setup-grid">
          <label>
            A사이드 내 팀
            <select value={teamAId} onChange={(event) => { setTeamAId(event.target.value); if (event.target.value === teamBSnapshot?.id) setTeamBSnapshot(null); setFeedback(""); }}>
          ...
          <label>
            ...
            />
            {teamBSnapshot ? <span className="arena-record-team-selected"><TeamEmblem team={teamBSnapshot} /><strong>{teamBSnapshot.name}</strong><button type="button" onClick={() => setTeamBSnapshot(null)}>해제</button></span> : null}
          </label>
        </div>
      )}
...
function getSourceMatchAction(match, userId, teams = [], userById = {}) {
  ...
  if (effectiveStatus === "approval") return {
    label: "최종 확정 대기",
    detail: match.refereeId ? "배정 심판의 최종 확정을 기다립니다." : "방장의 최종 확정을 기다립니다.",
  };
  const phase = getMatchRoomPhase(match);
    ...
    setSourceDisputeDraft((current) => (
      current.matchId === sourceMatch.id && current.resultKey === resultKey
        ...
        : {
          ...
          requestedSide: "teamA",
          requestedScore: String(sourceMatch.result?.scoreA ?? sourceMatch.teamA?.score ?? 0),
        }
    ));
  ...
  const submitSourceDispute = (event) => {
    ...
    if (!sourceMatch.refereeId) {
      app.actions.disputeMatch(sourceMatch.id, {
        kind: "team_score",
        side: sourceDisputeDraft.requestedSide,
        requestedScore: Number(sourceDisputeDraft.requestedScore),
              ...
              <div className="arena-join-panel">
                {matchRoom ? (
                  <div className="arena-owner-panel">
                    ...
                    {sourceMatchApprovalOpen && !sourceMatch.refereeId ? (
                      <form className="arena-dispute-editor" onSubmit={submitSourceDispute}>
                        <div className="arena-dispute-score-row">
                          <label>
                            이의 대상
                            <select value={sourceDisputeDraft.requestedSide} disabled={!canRequestSourceMatchPointDispute} onChange={(event) => setSourceDisputeDraft((current) => ({ ...current, requestedSide: event.target.value }))}>
                              <option value="teamA">{sourceMatch.teamA?.name ?? "A"}</option>
                              <option value="teamB">{sourceMatch.teamB?.name ?? "B"}</option>
                            </select>
                          </label>
                          <label>
                            요청 점수
                            <input type="number" min="0" max="999" disabled={!canRequestSourceMatchPointDispute} value={sourceDisputeDraft.requestedScore} onChange={(event) => setSourceDisputeDraft((current) => ({ ...current, requestedScore: event.target.value }))} />
                          </label>
                        </div>
                        <label className="memo-label">
                          점수 정정 사유
                    ...
                    {!sourceMatchRecordBoardFirst && !sourceMatchIsRecordRoom && Boolean(sourceMatch.refereeId) && !sourceMatchAction.disputed && (canSubmitSourceMatchLiveResult || canSubmitSourceMatchPostgameResult || canSubmitSourceMatchRecorderResult) ? (
                      <SourceMatchDisputeEditor
                        match={sourceMatch}
                        userById={userById}
                        canReview={false}
                    ...
                    {!sourceRoomReadOnly && canFinalizeSourceMatch ? (
                      <Button type="button" onClick={() => app.actions.finalizeMatch?.(sourceMatch.id)}>최종 확정</Button>
                    ) : null}
