...
import { getMatchFormatLabel } from "../../../shared/lib/matchFormats.js";
import {
  ...
  return (
    <div className="ui-action-row match-creation-wizard-actions">
      ...
      <span className="ui-action-row match-creation-wizard-secondary-actions">
        <Button type="button" touchFriendly variant="secondary" disabled={disabled} onClick={onCancel}>
          <X size={16} /> 취소하기
        </Button>
        {previousStep ? (
          <Button type="button" touchFriendly variant="secondary" disabled={disabled} onClick={() => onStepChange(previousStep.id)}>
      ...
      <span className="ui-action-row ui-action-row-end match-creation-wizard-primary-actions">
        {nextStep ? (
          <Button type="button" touchFriendly disabled={disabled} onClick={() => onStepChange(nextStep.id)}>
            다음: {nextStep.label} <ChevronRight size={17} />
          </Button>
        ) : submitIssue ? (
          <Button type="button" touchFriendly disabled={disabled} onClick={onFixIssue}>입력 확인하기 <ChevronRight size={17} /></Button>
        ) : submitLabel ? (
          <Button type="button" touchFriendly disabled={disabled || submitDisabled} onClick={onSubmit}>{submitLabel}</Button>
        ) : null}
      </span>
    </div>
  );
...
export function MatchRosterPolicyFields({ draft, onChange }) {
  ...
  return (
    <div className="match-roster-policy-fields" data-create-field="roster" tabIndex={-1}>
...
export function MatchCostPolicyFields({ draft, onChange }) {
  ...
  return (
    <div className="match-cost-policy-fields" data-create-field="cost" tabIndex={-1}>
...
export function MatchCreationFinalSummary({ draft, summaryType = "match", errors = [], warnings = [] }) {
  ...
  const personalRecordRows = [
    ...
    { label: "경기 방식", value: getMatchFormatLabel(draft.mode || "1v1", draft) },
  ...
  const matchRecordRows = [
    ...
    { label: "경기 방식", value: getMatchFormatLabel(draft.mode || "5v5", draft) },
    { label: "구장", value: draft.court || "구장 미정" },
  ...
  const sentence = summaryType === "personal_record"
    ...
    : summaryType === "match_record"
      ? "기록을 만든 뒤 참가자 명단과 팀 점수를 입력하고, 참가자에게 내 참가 확인을 요청하세요. 확인한 선수만 개인 MMR 반영 대상이 되며 팀 MMR은 반영하지 않습니다."
      : summaryType === "tournament"
