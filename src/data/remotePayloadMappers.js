...
export function fromRemoteApprovedCourt(row = {}) {
  ...
  return {
    ...
    lng: row.lng ?? payload.lng,
    publicAccess: row.public_access ?? payload.publicAccess ?? "unknown",
    registrationOrigin: row.registration_origin ?? payload.registrationOrigin ?? "user_request",
    facilityName: row.facility_name ?? payload.facilityName ?? payload.baseName,
    courtUnit: row.court_unit ?? payload.courtUnit,
    sido: row.sido ?? payload.sido,
    sigungu: row.sigungu ?? payload.sigungu,
    emd: row.emd ?? payload.emd,
    indoorOutdoor: row.indoor_outdoor ?? payload.indoorOutdoor,
    nameModifiedAt: row.name_modified_at ?? payload.nameModifiedAt ?? null,
    nameModifiedBy: row.name_modified_by ?? payload.nameModifiedBy ?? null,
