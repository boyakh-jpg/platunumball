...
export function makeSoloRecordStats(stats = {}) {
  return Object.fromEntries(
    PLAYER_STAT_FIELDS.map((field) => [
      field.id,
      toSoloRecordNumber(stats[field.id]),
    ]),
  );
}
...
export function makeSoloRecordAnonymousSide({ count, entries = [] } = {}) {
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    return {
      ...
      ...(entry.linkedProfileId ? { linkedProfileId: String(entry.linkedProfileId) } : {}),
    };
  });
}
...
export function fromRemoteMatch(row, context) {
  ...
  const disputes = projectMatchDisputeRows(context.disputesByMatch.get(row.id));
  const agreements = {
