...
begin
  if to_regclass('public.match_results') is not null then
    ...
    execute 'alter table public.match_results add column if not exists period_scores jsonb not null default ''[]''::jsonb';
    if not exists (
      select 1 from pg_constraint
      where conrelid = 'public.match_results'::regclass
        and conname = 'match_results_period_scores_check'
    ) then
      execute 'alter table public.match_results add constraint match_results_period_scores_check check (jsonb_typeof(period_scores) = ''array'' and jsonb_array_length(period_scores) <= 5)';
    end if;
  end if;
end;
$$;
...
-- Public individual rooms balance headcount, side averages, and within-side MMR spread.
begin;
...
-- Allow the current controller to recover an accidentally ended, unrecognized clock.
begin;
...
-- 2026-08-14: canonical per-period scores saved with the final result.
begin;
...
begin
  ...
  -- During play, only the score owner may derive the split from the clock.
