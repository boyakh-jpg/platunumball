-- Keep configured regulation time below the server wall-clock cutoff.

do $migration$
begin
  if to_regprocedure('public.rankball_apply_room_rule_patch_pre_duration_limit(jsonb,jsonb,text)') is null then
    if to_regprocedure('public.rankball_apply_room_rule_patch(jsonb,jsonb,text)') is null then
      raise exception 'room_rule_patch_missing' using errcode = '55000';
    end if;
    alter function public.rankball_apply_room_rule_patch(jsonb, jsonb, text)
      rename to rankball_apply_room_rule_patch_pre_duration_limit;
  end if;
end;
$migration$;
...
begin
  next_rules := public.rankball_apply_room_rule_patch_pre_duration_limit(
    p_current_rules,
    p_patch,
    p_mode
  );
  period_count := coalesce((next_rules->>'periodCount')::integer, 1);
  period_minutes := coalesce((next_rules->>'periodMinutes')::integer, 1);

  if period_count * period_minutes > 63 then
    raise exception 'match_regulation_duration_exceeded' using errcode = '23514';
  end if;
...
begin
  ...
  force_end_at := case
    when session_row.clock_started_at is not null
      then session_row.clock_started_at + interval '90 minutes'
    else null
  end;
...
begin
  for candidate in
    ...
    where session.clock_started_at is not null
      and session.clock_started_at + interval '90 minutes' <= server_now
...
revoke all on function public.rankball_apply_room_rule_patch_pre_duration_limit(jsonb, jsonb, text)
  from public, anon, authenticated, service_role;
revoke all on function public.rankball_apply_room_rule_patch(jsonb, jsonb, text)
  from public, anon, authenticated, service_role;
revoke all on function public.rankball_match_clock_force_end_if_due(text, timestamptz)
  from public, anon, authenticated;
revoke all on function public.rankball_match_clock_force_end_due_batch(timestamptz, integer)
  from public, anon, authenticated;
