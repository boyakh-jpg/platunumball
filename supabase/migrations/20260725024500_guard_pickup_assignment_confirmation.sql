...
begin
  ...
  if coalesce(current_match.rules->>'sideAssignmentStatus', '') <> 'draft'
     or coalesce(current_match.rules->>'sideAssignmentRevision', '') !~ '^[1-9][0-9]*$' then
    raise exception 'pickup_side_assignment_draft_required' using errcode = '23514';
  end if;
...
begin
  ...
  set team_a_id = null,
      ...
      rules = (coalesce(rules, '{}'::jsonb) - 'sideAssignmentConfirmedAt' - 'sideAssignmentConfirmedBy')
        || jsonb_build_object('sideAssignmentStatus', 'draft'),
      agreed_at = null,
      updated_at = now_at
  where id = safe_match_id;
