...
alter table public.court_request_evidence
  ...
  alter column field_lng drop not null,
  alter column field_accuracy_meters drop not null,
  alter column field_distance_meters drop not null,
  alter column field_captured_at drop not null;
...
begin
  ...
  live_location_verified := coalesce(
    evidence_row.field_accuracy_meters <= 20
      and evidence_row.field_distance_meters <= 30
  ...
  if jsonb_array_length(evidence_row.photo_keys) < 2
    or photo_location_status in ('uncertain', 'mismatch')
