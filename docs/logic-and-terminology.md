...
17. 연습 경기의 QR 출석 패널과 경기시계는 운영 출석 API를 호출하지 않고 격리된 연습 QR과 명단·출석 state로 동일한 시작 상태를 계산한다. 출전·후보 전원의 연습 출석이 끝나야 경기 시작을 허용하며, `ranked=false` 때문에 친선전으로 오인되지 않도록 방 종류는 `연습경기`로 표시한다. 연습 경기시계 점수 응답도 운영 응답과 같은 점수 revision을 제공한다.
