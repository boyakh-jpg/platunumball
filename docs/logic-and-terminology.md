...
3. 전용 scope는 `closed`와 개인·사후 기록방을 제외하고 `waiting / locked / checkin / live / postgame / dispute / record / cancelled / void` phase를 함께 반환한다. 서버는 미확정·진행 경기를 종료 경기보다 먼저 제한 조회하고 cursor로 나머지 경기를 이어서 제공한다. 업무함은 이를 `지금 / 다음 / 지난`으로 분류하며, 로딩·실패·빈 상태를 분리하고 실패를 빈 목록으로 숨기지 않는다. 추가 조회 실패 시 기존 목록과 cursor를 보존하며 새로고침은 첫 페이지부터 다시 조회한다.
