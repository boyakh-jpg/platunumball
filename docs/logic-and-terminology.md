...
13. canonical 팀 엠블럼 또는 사용자가 현재 편집 화면에서 선택한 로컬 엠블럼이 있으면 먼저 공용 원형 크롭 편집기에서 위치와 확대 비율을 정하고 최대 `320×320px`, `96KB` WebP로 기계적으로 정규화한다. 정규화 원본은 자동 적용하지 않는다. 사용자가 같은 편집 팝업의 `선화로 변경`을 명시적으로 실행한 경우에만 브라우저 Canvas가 배경색과 경계를 기계적으로 판별해 투명 금색 선화 후보를 만들고 팝업 안 미리보기에 표시한다. 사용자가 결과를 확인한 뒤 `확인`을 누른 경우에만 팝업을 닫고 미리보기와 현재 저장본에 적용한다. 취소하거나 위치·크기를 다시 조정하거나 변환 품질 기준을 통과하지 못하면 기존 엠블럼을 유지한다. 로컬 파일과 변환 data URL은 서버에 저장하지 않는다. 임의 클라이언트 object key는 허용하지 않고 서버가 확인한 canonical `team-emblems/` key만 공개 스냅샷에 보존한다.
