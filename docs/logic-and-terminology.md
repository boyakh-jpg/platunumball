...
2. 브라우저 편집 화면의 로컬 엠블럼 Blob과 data URL은 현재 화면 메모리에서만 유지한다. 외부 PNG 생성 API의 사전 처리 WebP도 요청 메모리에서 검증·배치·스타일 변환한 뒤 응답과 함께 폐기하며 DB·R2·Storage에 저장하지 않는다. 영수증 draft 생성·수정·공개 조회·복제·claim은 guest 엠블럼 입력이나 key를 저장·투영·승계하지 않고 공개 draft 생성 API는 엠블럼 입력을 거부한다. 만료 maintenance는 기존 운영 버전이 만든 안전한 draft 전용 R2 key가 있으면 객체를 먼저 정리한 뒤 만료된 draft 행을 삭제한다.
