...
1. 매칭 생성의 `경기 목적`은 `친선전`과 `경쟁전`만 사용한다. 친선전은 MMR 미반영, 경쟁전은 MMR 반영을 뜻하며 후보 출전 여부를 설명하거나 강제하지 않는다.
2. `픽업`은 경기 목적이 아니라 `팀 구성` 방식이다. `경기 전 구성`은 시작 전에 A/B사이드와 출전·후보를 정하고, `현장 픽업`은 개인 참가자를 받아 현장에서 팀과 교대 순서를 정한다. 현장 픽업은 친선전·MMR 미반영으로 고정한다.
...
6. 개인 구성 기록방에서 방장이 A/B 실제 참가자를 저장하면 즉시 기록 대상 명단이 된다. 별도 초대 수락은 만들지 않고, 참가자 전원의 최종 기록 승인을 동의 절차로 사용한다.
7. 팀 구성 기록방은 `팀 선택 완료 → 각 팀장 출전 명단 확정 → 기록 입력·양 팀장 승인` 순서다. 팀 선택 저장과 양 팀 명단 확정을 같은 상태로 표시하지 않는다.
...
3. 픽업 초대는 지정된 A/B 출전 또는 순환 대기 위치로 수락한다. 방장을 제외한 참가자는 본인 슬롯 관리에서 정원이 남은 A/B 출전·순환 대기 위치로 이동할 수 있다. 방장은 생성 사이드를 유지한다.
...
3. 픽업 정원은 팀 정원이 아니라 `코트 N명씩 · 순환 대기 최대 N명씩`으로 표시한다. 고정 선발·후보 명칭을 사용하지 않는다.
4. 방 모달 규칙은 경기 구성, 경기 시계와 마지막 스톱 구간, 종료 기준, 휴식, 연장, 사용 공, 공격권, 파울, 만남 장소를 저장된 값으로 표시한다. 경기계약서도 같은 중앙 rule formatter를 사용한다.
5. 압축 모집방 응답도 모달을 여는 즉시 잘못된 기본값을 잠깐 보여주지 않도록 `matchIntent`, 출전정책, 마지막 스톱 구간과 표시용 운영 정책을 보존한다.
