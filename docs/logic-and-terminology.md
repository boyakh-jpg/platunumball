...
6. 라이벌 방 저장 뒤 A팀을 먼저 고정하고 B팀 주장에게 팀 초대를 생성한다. 어느 단계든 실패하면 불완전한 방을 닫고 성공으로 이동하지 않는다.
...
10-1. 방장의 심판 미출석 요청은 `pending`만 저장하고 신뢰도를 바꾸지 않는다. pending 재요청은 막으며 상대 사이드장의 최초 확인 transaction에서만 신뢰도를 한 번 차감한다.
...
3. 참가 취소는 서버 transaction 하나에서 출전·후보 명단, 파티, 출석, 동의, 승인, 규칙 확인을 정리한다. 출전선수가 취소하면 확정 단계에서는 같은 사이드 첫 후보를, 출석 단계에서는 출석 완료 후보를 자동 승격한다. 픽업 경기는 팀 배정을 다시 미확정으로 돌린다.
4. 후보 승격 뒤에도 한 사이드 출전 인원이 정원보다 적으면 경기는 자동 취소하지 않고 `rosterNeedsFill=true`와 부족 사이드를 저장한다. 방장은 인원을 보충하거나 경기 취소를 선택한다.
...
8. 공용 방 모달은 모집, 확정, 출석, 진행, 종료, 이의, 기록 단계별 도움말을 같은 팝업에서 제공한다. 도움말은 방장·사이드장·심판·모바일 전광판 담당자·선수 역할과 참가 취소·교체·기록 확정 흐름을 설명하며 별도 메뉴별 방 모달을 만들지 않는다.
