...
21. Instagram 영수증 봇은 공식 Webhook의 서명 검증을 통과한 텍스트 DM만 처리한다. 입력은 `영수증` 명령과 명시적 필드 형식을 사용하고 19번의 외부 입력 parser·PNG renderer를 재사용한다. Meta 이벤트 ID, 발신자 ID, 정규화 본문은 서버 비밀키 HMAC 해시로만 저장하며 원문과 원 ID는 저장하지 않는다. `연결`은 256비트 일회용 capability를 발급하되 DB에는 그 HMAC만 저장하고, 로그인 뒤 `/api/instagram/account`가 Bearer의 Auth 사용자에서 서버가 확정한 canonical 프로필만 연결한다. 연결하면 같은 발신자 해시의 기존 요약 기록도 해당 프로필에 귀속한다. `기록`은 연결 계정의 최근 5건, 로그인 연결 화면과 API는 최근 10건의 compact 요약만 반환하며 조회 프로필 ID를 클라이언트에서 받지 않는다. 영수증 요청은 연결 전에는 발신자, 연결 뒤에는 canonical 프로필 principal 기준으로 하루 `INSTAGRAM_BOT_DAILY_LIMIT=10`을 적용한다. 반복 방지는 발신자 기준 30초 쿨다운·시간당 5회, 전체 시간당 100회, 같은 정규화 본문 5분 중복 차단을 적용한다. `연결`·`기록` 명령은 일일 영수증 한도를 소모하지 않지만 발신자 쿨다운·시간당 한도·전체 시간당 한도와 이벤트 중복 방지는 동일하게 적용한다. 이벤트 해시 원자 예약과 모든 제한은 Supabase RPC 하나에서 판정하며 중복·제한 요청에는 답장과 렌더를 실행하지 않는다. Instagram이 가져갈 공개 이미지 URL에는 별도의 256비트 무작위 capability ID만 넣고, 렌더 입력 JSON과 Story·Feed preset만 설정된 TTL 동안 `instagram_receipt_bot_render_jobs`에 임시 보관한다. 이는 19번의 직접 PNG API 메모리 전용 규칙을 변경하지 않는 Instagram 전달용 예외다. 기록에는 검증·정규화된 영수증 요약만 저장하고 PNG 바이트·사진·엠블럼은 DB·Supabase Storage·R2에 저장하지 않으며 요청 시 메모리에서 렌더한 뒤 폐기한다. 만료 렌더 입력·연결 코드는 삭제하고, 제한 메타데이터는 7일, 미연결 기록은 30일, 연결 기록은 최대 1년 보존한다. Instagram 검증 GET의 `hub.verify_token` 쿼리만 해당 route·method에서 전역 URL credential 차단의 프로토콜 예외로 허용하며 POST와 다른 token 쿼리는 계속 거부한다.

21-1. Meta의 `entry.messaging` 직접 이벤트와 `entry.changes[field=messages].value` 변경 이벤트를 하나의 canonical DM 이벤트로 정규화한다. 형식이 유효하고 제한 판정을 통과한 Instagram 영수증 요청에는 즉시 처리 안내를 보낸 뒤 영수증 이미지를 전송한다. 형식 오류에는 사용법을 보내며 중복·제한 요청에는 답장과 렌더를 실행하지 않는다.
