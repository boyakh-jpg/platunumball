...
8. 심판 프로필은 활성 `referee_appointments`의 `candidate/silver/gold/platinum/official`을 서버 기준으로 사용한다. 알파 테스트 심판 로그인 ID는 검색 자격과 동일하게 임명 row가 없어도 `candidate` 프로필을 연다. 공개된 확정 경기 중 해당 사용자가 `referee_id`인 경기만 최근 심판 경기와 활동 통계에 포함하며 비공개 경기는 제외한다.
