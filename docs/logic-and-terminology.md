...
2. `픽업`은 경기 목적이 아니라 팀 구성 방식이다. `prearranged`는 시작 전에 A/B사이드와 출전·대기를 정하고, `pickup`은 모집 중 하나의 참가자 풀만 사용한 뒤 체크인에서 A/B와 대기 선수를 정한다. 현장 픽업은 `matchPurpose=friendly`, MMR 미반영으로 고정한다.
3. 후보 출전정책은 후보 정원 아래의 단일 control만 원본으로 사용한다. 경기 목적을 바꿔도 사용자가 고른 `최소 1회 출전`, `균등 순환`, `출전 보장 없음`을 덮어쓰지 않는다. 현장 픽업만 `균등 순환`으로 고정한다.
...
6. 사후 기록방은 `참가 사실 확인`과 `결과 승인`을 분리한다. 방장이 A/B 후보 명단을 제출해도 각 선수의 참가 사실이 자동 확정되지 않으며, 본인이 참가를 확인한 뒤에만 실명 참가자와 결과 승인자로 등록한다. 방장, 팀장, 심판은 다른 사람을 대신해 확인하거나 승인할 수 없다.
7. 개인 구성 기록방은 `명단 제출 → 각 선수 참가 확인 → 기록 입력 → 확인 참가자 결과 승인` 순서다. 팀 구성 기록방은 `팀 선택 → 각 팀장 출전 명단 제출 → 각 선수 참가 확인 → 기록 입력 → 양 팀 확인 참가자 결과 승인` 순서다. 팀 선택, 명단 제출, 참가 확인, 결과 승인을 같은 상태로 표시하지 않는다.
8. 사후 기록방을 중단하는 행동과 알림은 `경기 취소`가 아니라 `기록 취소`로 표시한다. 예정 경기 취소·경기 무효와 구분한다.
9. 사후 기록의 참가 확인과 결과 승인은 무응답을 자동 동의로 처리하지 않는다. 요청 즉시, 12시간 후, 22시간 후에 앱 알림을 만들고 연결된 Discord 알림은 사용자가 해당 연동을 켠 경우에만 같은 일정으로 보낸다.
...
12. 참가자가 `참가하지 않음` 또는 `결과 반대`를 선택하면 무응답으로 처리하지 않고 즉시 이의조정 상태로 전환한다. 이의가 열려 있는 동안 해당 기록의 개인 통계와 전적도 반영하지 않는다.
13. 이의조정은 방장이 원본을 일방 확정하는 기능이 아니다. 관계자에게 수정안을 다시 보내고 수정된 참가 명단과 결과에 대해 새 참가 확인 또는 새 결과 승인을 받아야 한다. 이전 승인은 수정 시각 이후의 승인으로 대체한다.
14. 참가 확인 요청과 결과 승인 요청은 멱등 키를 분리하고, 요청자·응답자·시각·IP/기기 위험 신호·변경 전후 값을 감사 로그에 남긴다. 같은 계정의 다중 슬롯, 본인 대리 승인, 명단 수정 뒤 이전 승인 재사용을 서버에서 거부한다.
...
6. 픽업 체크인에서는 먼저 참석 여부를 확정한 뒤 참석자만 A/B 출전과 사이드별 대기에 배정한다. `배정 확정` 전에는 경기 시작을 허용하지 않는다.
...
9. 균등 교대 방식은 경기 구간이 4개면 `매 쿼터`, 2개면 `하프 종료`, 단일 구간이면 `3분`, `5분`, `7분`, `10분` 간격 또는 `직접 교대`를 사용한다. 시스템은 출전·대기 횟수를 바탕으로 교대 대상을 추천할 뿐 슬롯을 자동 변경하지 않는다.
10. 교대 추천은 방장 또는 배정 심판이 확정해야 반영한다. 실제 교대 시각, 들어간 선수, 나온 선수, 사이드, 확정자를 교대 이벤트로 저장하며 지각·부상·퇴장·운영자 직접 변경을 허용하고 사유를 남긴다.
...
4. 방 모달 규칙은 경기 구성, 경기 시계와 마지막 스톱 구간, 종료 기준, 휴식, 연장, 사용 공, 공격권, 파울, 만남 장소를 저장된 값으로 표시한다. 경기계약서도 같은 중앙 rule formatter를 사용한다.
...
8. 픽업은 개인 참가 모집방을 사용한다. `teamOnly=false`, `ranked=false`, `official=false`, `playingTimePolicy=equal_rotation`, `lineupSelectionPolicy=no_fixed_starter`로 저장하고 확정 인원 균등 비용을 기본값으로 사용한다. 모집·잠금에서는 통합 참가자 풀을 사용하고 체크인에서 방장 또는 배정 심판이 추천을 검토해 A/B와 교대를 확정한다. 추천을 자동 팀배정이나 자동 로테이션으로 표시하지 않는다.
...
16. 룰북의 BOXTIER 기록 절차는 경기 진행 권한과 본인 확인 권한을 구분한다. 사후 기록의 참가 사실 확인과 결과 승인은 각 참가자 본인만 처리하며 방장·팀장·심판이 대신할 수 없다.
...
- 개인 구성은 방모달에서 A/B 참가 후보를 각 사이드 정원만큼 계정으로 선택한다. 생성자는 A사이드에 포함되어야 하고 중복 선수는 금지한다. 각 후보가 참가 사실을 직접 확인해야 실명 실제 참가자와 결과 승인 대상이 된다.
- 팀 구성은 방모달에서 서로 다른 등록 팀 두 개를 먼저 고른다. 각 팀 주장이 자기 팀의 출전 후보만 정원만큼 제출하며 후보 슬롯은 사용하지 않는다. 각 출전 후보가 참가 사실을 직접 확인한 뒤 양 팀의 확인 참가자 전원이 결과 승인 대상이 된다.
- 참가 후보 명단과 팀 로스터가 끝나기 전에는 참가 확인 요청을 막고, 참가 확인이 시작된 뒤 명단을 바꾸면 기존 확인과 결과 승인을 무효화한다. 서버는 `recordParticipantCandidateIds`, 참가 확인자, `recordApproverIds`를 분리해 검증하며 방장·팀장 대리 확인을 거부한다.
- 참가 확인 또는 결과 승인 요청 후 24시간까지 무응답자가 있으면 자동 승인하지 않고 `부분 확인`으로 전환한다. 무응답자의 실명 통계·전적은 제외하고 경기 전체 MMR·팀 MMR·승패·연승은 반영하지 않는다. `참가하지 않음`이나 `결과 반대` 응답은 이의조정으로 전환한다.
...
17. 중복주소 코트번호 정리는 동일한 정규화 주소가 2행 이상인 묶음을 시작점으로 하고 DB의 동일 위치 기준인 35m 이내 구장까지 한 묶음으로 확장한다. 좌표와 ID의 결정적 순서로 모든 행에 `1코트`, `2코트`, `N코트`를 부여한다. 도로명주소 건물번호 뒤의 시설명은 `administrative_fallback`·`unresolved` 또는 현재 시설명이 저장된 읍면동과 같은 행에만 반영하고, 확정 OSM명과 수동명은 덮어쓰지 않는다. 주소 시설명이 반영되면 기존 OSM 근거는 보존하되 반영상태를 `skipped_manual`로 바꿔 현재 이름과 과거 추론을 구분한다. 기존 번호 교환이 충돌하면 묶음을 임시 고유번호로 옮긴 뒤 최종 번호를 저장한다. 공개·운영·검증 상태 등 이름 외 필드는 바꾸지 않으며 기존 관리자 수정 RPC와 감사 로그를 사용한다.
