...
7. 경기시계를 시작하면 모바일 전광판의 집중 화면과 브라우저 전체화면을 즉시 요청한다. 시작 실패 시 원래 화면으로 복귀하고, 경기 종료 성공 또는 종료 상태 수신 시 집중 화면과 전체화면을 해제한다. 브라우저가 전체화면을 거부하면 집중 화면은 유지한다. 화면 유지와 부저는 브라우저·기기 제한이 있는 베타 기능으로 안내한다. 수동 전체화면은 경기시계를 최초 시작하기 전에는 열 수 없고, 한 번 시작한 뒤에는 일시정지 상태에서도 사용할 수 있다. 자동 상대 확정, 자동 점수·공격권 인식, 공인 심판 자동 인증, 실제 라이브 중계처럼 구현되지 않은 기능은 사용 설명에서 현재 지원으로 표시하지 않는다.
