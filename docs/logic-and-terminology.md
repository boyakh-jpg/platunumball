...
17. 중복주소 코트번호 정리는 동일한 정규화 주소가 2행 이상인 묶음을 시작점으로 하고 DB의 동일 위치 기준인 35m 이내 구장까지 한 묶음으로 확장한다. 좌표와 ID의 결정적 순서로 모든 행에 `1코트`, `2코트`, `N코트`를 부여하고, 도로명주소 건물번호 뒤에 명시된 시설명이 있으면 해당 묶음의 시설명 후보로 함께 반영한다. 기존 번호 교환이 충돌하면 묶음을 임시 고유번호로 옮긴 뒤 최종 번호를 저장한다. 공개·운영·검증 상태 등 이름 외 필드는 바꾸지 않으며 기존 관리자 수정 RPC와 감사 로그를 사용한다.
