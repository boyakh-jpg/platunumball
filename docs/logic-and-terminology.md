...
5. 확정 경기 영수증은 기존 경기의 팀·점수·날짜·장소·팀 MMR·현재 사용자 개인 스탯을 사용하고 `BOXTIER VERIFIED`로 표시한다. 양 팀 canonical ID가 모두 있는 확정 팀전만 팀 티어 선화 엠블럼을 표시한다. 서버가 조회 권한을 검증한 사용자는 공개 여부와 관계없이 확정·비개인기록 경기의 canonical 공개 스냅샷을 생성할 수 있으며 `solo`와 레거시 `personal_record`는 모두 개인기록으로 판정한다. 모든 QR은 `/app/receipt?draft={publicId}`를 가리킨다. 이 스냅샷은 누구나 조회할 수 있지만 안전 필드만 포함하고 원본 경기방의 visibility·조회·수정 권한은 바꾸거나 노출하지 않는다. 기존 canonical draft가 raw `match:{id}` seed를 가진 경우 공개 조회·복제 전에 원본 경기의 현재 확정·비개인기록 조건을 다시 검증하고 응답 seed를 HMAC 값으로 치환한다. 직접 작성·복제 draft는 verified·canonical 팀전 상태를 만들 수 없고 공식 전적이나 MMR에 반영하지 않는다. 외부 지도 URL과 소유권 비밀은 QR에 포함하지 않으며 웹 미리보기의 QR 영역도 같은 조회 대상으로 이동한다. 중앙 개인 스탯은 확정된 배정 심판 경기·대회의 현재 사용자 검증 스탯 또는 현재 사용자가 작성한 `personal_record`의 본인 스탯이 있을 때만 표시한다. 이때 현재 사용자는 인증 공급자의 계정 ID가 아니라 경기 `playerStats`와 같은 canonical 프로필 ID로 식별한다. 일반 무심판 경기와 타인이 작성한 개인 기록은 `GAME INFO`를 유지하며, 공개 스냅샷 생성 가능 여부와 개인 스탯 표시 판정은 분리한다.
