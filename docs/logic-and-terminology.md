...
5. 확정 경기 영수증은 기존 경기의 팀·점수·날짜·장소·팀 MMR·현재 사용자 개인 스탯을 사용하고 `BOXTIER VERIFIED`로 표시한다. 양 팀 canonical ID가 모두 있는 확정 팀전만 팀 티어 선화 엠블럼을 표시한다. 서버가 검증한 공개·확정·비개인기록 경기만 canonical 공개 스냅샷을 생성하며 모든 QR은 `/app/receipt?draft={publicId}`를 가리킨다. 이 스냅샷은 누구나 조회할 수 있지만 원본 경기방·비공개 기록·수정 권한은 노출하지 않는다. 직접 작성·복제 draft는 verified·canonical 팀전 상태를 만들 수 없고 공식 전적이나 MMR에 반영하지 않는다. 외부 지도 URL과 소유권 비밀은 QR에 포함하지 않으며 웹 미리보기의 QR 영역도 같은 조회 대상으로 이동한다.
