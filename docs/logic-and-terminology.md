...
2-1. 프로필 아이콘은 검수된 340개 allowlist의 확장자 없는 icon key만 `profiles.avatar_icon_key`에 저장한다. 기존 `01~80`은 2자리, 확장 `081~340`은 3자리 번호를 사용한다. 화면은 key로 자산 경로를 계산하며 allowlist 밖 key, 삭제된 파일, 이미지 로드 실패는 이름 첫 글자로 대체한다.
...
2-9. `081~340`은 52개 업적 계열을 브론즈·실버·골드·플래티넘·레전드 다섯 단계로 나눈다. 업적은 서버가 확인한 확정 경기·승리·출전 snapshot·매칭 확정·초대 수락·운영 수행·승인 구장·대회 상태만 사용한다. 사용자가 입력할 수 있는 득점·리바운드·어시스트·스틸·블록, 더블더블·트리플더블, 경기별 기록 순위는 해금 조건에 사용하지 않는다. 조건 변경 전에 해금된 icon key는 회수하지 않는다.
