...
2. 운영 도메인 `platunumball.vercel.app`, `rankball_*` RPC와 DB 함수, 환경변수, 저장소 key, 테스트 로그인 ID, 패키지명은 기존 연결을 유지하는 내부 식별자다. 서비스명 변경만으로 이름을 바꾸지 않으며, 테스트 로그인 ID는 화면에서 번호 기반 표시명으로 변환한다.
3. 서버 오류의 내부 code와 detail은 판정·로그용으로 유지하되 사용자 화면에는 승인된 한국어 안내만 표시한다.
4. 룰북과 심판 시험 원문은 이번 용어 변경 범위에서 제외한다.
...
2-1. 프로필 아이콘은 검수된 활성 335개 allowlist의 확장자 없는 icon key만 `profiles.avatar_icon_key`에 저장한다. 기존 `01~80`은 2자리, 확장 `081~340`은 3자리 번호를 사용하되 `221~225`는 폐기된 4v4 계열의 번호 보존용 예약 key로 카탈로그와 allowlist에서 제외한다. 화면은 key로 자산 경로를 계산하며 allowlist 밖 key, 삭제된 파일, 이미지 로드 실패는 이름 첫 글자로 대체한다.
...
2-9. `081~340`은 활성 51개 업적 계열을 브론즈·실버·골드·플래티넘·레전드 다섯 단계로 나누며 `221~225` 다섯 번호는 예약 상태로 유지한다. 업적은 서버가 확인한 확정 경기·승리·출전 snapshot·매칭 확정·초대 수락·운영 수행·승인 구장·대회 상태만 사용한다. 사용자가 입력할 수 있는 득점·리바운드·어시스트·스틸·블록, 더블더블·트리플더블, 경기별 기록 순위는 해금 조건에 사용하지 않는다. 조건 변경 전에 해금된 활성 icon key는 회수하지 않는다.
...
2-13. `206~220`, `226~230`은 `1v1`, `2v2`, `3v3`, `5v5` 확정 경기 5·25·75·150·300회의 단계 업적이다. RankBall 경기 방식에 없는 `4v4`의 기존 번호 `221~225`는 뒤 번호를 바꾸지 않기 위한 예약 key이며 화면에 표시하거나 해금·선택하지 않는다.
...
11. 이의 처리자가 수정안을 확인하면 양팀 새 과반 승인 단계로 돌아간다. 반려하면 이의 draft를 폐기하고 변경 전 원본 결과를 확정한다.
...
7. 구장 이름은 사용자가 직접 입력한 시설/장소명과 구조화된 코트 구분을 합쳐 생성한다. 시설명은 공백·괄호형 농구장·붙여 쓴 농구장·코트 번호 표기만 보수적으로 정규화하며 주소의 동 이름은 기본 이름에 붙이지 않는다.
...
38. 공공 원장 일괄 적재의 표준 구장명은 저장·가공 권한이 확인된 공공데이터·SFMS·OSM 원본 `구장명`만 사용한다. NFKC, 공백, 괄호형 `농구장(장소명)`, 붙여 쓴 `농구장`, 코트 번호 표기만 보수적으로 정규화하고 원본명과 변환 단계를 함께 보존한다. `이름보정대상` 또는 시설 식별자가 없는 일반명은 자동 승인하지 않는다.
39. 공공 원장 일괄 표준화에서는 외부 지역검색 결과를 대량 수집해 표준명으로 저장하지 않는다. 외부 제공자의 별도 저장·가공 허용을 확인하기 전에는 원본 공공데이터 이름과 RankBall 내부 중복 후보만 사용한다.
