...
20. 공개 `POST /mcp`는 Streamable HTTP MCP 서버이며 `create_basketball_receipt` 도구 하나를 제공한다. 도구는 19번의 입력 parser와 메모리 PNG renderer를 재사용하고 MCP `image/png` content를 직접 반환하며 DB·R2·Storage에 저장하지 않는다. 공개 입력의 `thermal`·`score`는 각각 canonical `classic-thermal`·`boxtier-score`로 매핑한다. 팀명·최종 점수·경기 날짜·장소·경기 방식은 사용자가 제공한 실제 값이어야 하고 누락값을 추측하지 않는다. 쿼터 합계 불일치와 농구 외 경기·상거래 영수증 요청은 거부한다. 공개 MCP는 IP별 인스턴스 메모리 rate limit을 적용하고 전역 방어는 WAF가 맡는다. ChatGPT·Claude의 자동 도구 선택은 사용자가 승인된 BoxTier 앱 또는 사용자 지정 MCP connector를 먼저 설치·연결한 범위에서만 동작한다.
