...
20-1. MCP 영수증 도구는 별도 표시 resource·`openai/outputTemplate`·위젯 이미지 메타데이터를 연결하지 않는다. 표준 `content[type=image]`만 반환해 네이티브 클라이언트가 표시하게 하며, 같은 결과를 중복 렌더링하는 로딩 카드나 iframe을 만들지 않는다.
