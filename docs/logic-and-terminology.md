...
1. 공개 모집, 비공개 초대, 대회, 기존 경기 기록, 개인 기록 생성은 같은 단계형 탐색을 사용한다. 공개·비공개 경기는 `기본 설정 → 규칙·운영 → 구장·비용`의 3단계다. 경기 성격, 인원, 팀, 일정은 기본 설정에 포함하고 운영·취소 정책은 규칙 단계에 포함한다. 별도 확인 단계는 두지 않고 마지막 입력 단계 아래에서 최종 요약·오류·주의를 확인한 뒤 생성한다. 대회·경기 기록·개인 기록은 같은 공용 단계·정책을 쓰되 비용, 후보, MMR처럼 해당 유형에 없는 항목을 렌더링하거나 저장하지 않는다.
...
4. `친선전`, `경쟁전`, `픽업` 프리셋은 네 경기 방식에서 모두 사용할 수 있다. `완전 경쟁`은 `경쟁전 + 출전 보장 없음`과 중복되므로 별도 경기 성격으로 노출하지 않는다. 일반 프리셋의 첫 단계 출전 정책은 `최소 1회 출전`과 `출전 보장 없음`을 제공하고 픽업은 `균등 순환`으로 고정한다.
...
8. 픽업은 기존 개인 참가 모집방을 재사용한다. `hostJoinMode=player`, `teamOnly=false`, `ranked=false`, `official=false`, `playingTimePolicy=equal_rotation`, `lineupSelectionPolicy=no_fixed_starter`로 저장하고 확정 인원 균등 비용을 기본값으로 사용한다. 현재 자동 팀배정·자동 로테이션 엔진은 없으므로 방장이 현장에서 팀과 교대 순서를 수동으로 정하며, UI와 최종 요약도 이를 자동 기능처럼 표시하지 않는다.
...
7. 방장만 각 큐 항목을 `가결` 또는 `부결`한다. 가결은 해당 요청만 작업 결과에 반영하고 부결은 반영하지 않는다. 다른 open 항목이 남으면 경기를 계속 보류하고, 마지막 항목 처리 뒤 양쪽 결과 재승인 단계로 돌아간다.
...
- 경기기록방은 인원수와 별개로 `개인 구성` 또는 `팀 구성`만 허용한다. 생성 시 빈 기록방을 만들고, 개인 구성은 방장이 A/B 참가자를 채우며 팀 구성은 두 팀을 고른 뒤 각 팀장이 실제 출전 명단을 확정한다. 혼합 구성과 후보는 허용하지 않는다.
...
6. 방장은 새로고침 뒤 복원되는 open 큐에서 각 건을 `가결` 또는 `부결`한다. DB는 경기 단위 잠금으로 각 판정을 안전하게 직렬 커밋한다.
...
10. 마지막 open 건 처리 뒤 양 팀 새 과반 승인 단계로 돌아간다.
...
12. 운영 신뢰도 증감값은 `rating_policy`의 활성 버전을 원본으로 쓴다. 최고관리자만 전용 server action/RPC로 변경하며 경기 확정, 파울, 후보 기록자, 심판, 추천, 심판 불참, 방 닫기, 반복 강퇴, 허위 구장 등록에 적용한다.
...
- 개인 구성은 방모달에서 A/B 실제 참가자를 각 사이드 정원만큼 계정으로 선택한다. 생성자는 A사이드에 포함되어야 하고 중복 선수는 금지한다. 구성 완료 후 실제 참가자 전원이 결과 승인 대상이다.
- 팀 구성은 방모달에서 서로 다른 등록 팀 두 개를 먼저 고른다. 각 팀 주장이 자기 팀의 실제 출전자만 정원만큼 확정하며 후보 슬롯은 사용하지 않는다. 양 팀 주장만 결과 승인 대상이다.
- 참가자 구성과 팀 로스터가 끝나기 전에는 결과 제출을 막는다. `recordApproverIds`는 구성 완료 시 서버가 고정하고 승인 RPC와 최종 확정 RPC가 같은 목록을 검증한다.
- 경기 기록방은 KST 경기 종료 일시가 현재보다 과거이고 24시간 이내일 때만 생성한다. 미래 시각과 24시간 초과 요청은 서버가 거부한다.
- 모든 신규 사후 기록방은 MMR·공식경기·사전등록·연령 제한·구장 예약·후보·대기·비용·장비·경기 전 운영 조건을 비활성화한다.
...
17. 주소명 일괄 정리는 도로명주소의 건물번호 뒤에 명시된 시설명만 시설명 후보로 사용한다. 동일한 정규화 주소가 2행 이상이면 좌표와 ID의 결정적 순서로 모든 행에 `1코트`, `2코트`, `N코트`를 부여한다. 공개·운영·검증 상태 등 이름 외 필드는 바꾸지 않으며 기존 관리자 수정 RPC와 감사 로그를 사용한다.
...
2. 친선·픽업 프리셋은 `ranked=false`, 경쟁 프리셋은 `ranked=true`로 저장한다. 사용자는 경기성격으로만 MMR 반영 여부를 선택한다.
...
9. `경기 기록`, `대회`는 이전 화면의 `pickup` 상태를 상속하지 않는다. 경기 기록은 후보 없는 사후 명단과 MMR 미반영 정책을 사용하고, 대회는 일반 명단과 선택한 MMR 정책을 적용한다.
10. 경기 성격·공개 범위·경기 인원 변경으로 서버에 아직 저장하지 않은 공통 입력이 사라지면 생성 플로우 버그로 본다.
