...
1. 신규 심판 자격 취득·임명은 신뢰도 90 이상이어야 한다. 자격 취득 뒤 활동 유지 기준은 `REFEREE_ACTIVE_TRUST_MIN = 70`이다.
2. 시험·신규 임명 기준은 `REFEREE_TRUST_MIN = 90`, 검색·초대·배정 등 활동 기준은 `REFEREE_ACTIVE_TRUST_MIN = 70`으로 분리한다.
3. 심판 시험은 주 1회만 가능.
...
7. 심판 검색창은 입력 전 focus 시에도 현재 활성 자격을 가진 심판을 제한 조회해 추천한다. 신규 자격 취득·임명은 신뢰도 90 이상, 활성 심판 검색·초대·배정은 신뢰도 70 이상을 요구한다. 방·경기에 저장된 구형 `refereeTrustMin` 값은 신규 자격 취득 기준의 스냅샷이며 활성 심판 검색·초대·배정 기준을 70보다 높이지 않는다. 허용된 알파 테스트 심판은 테스트 진행을 위해 신뢰도 하락과 무관하게 자격을 유지한다. 검색 서버는 활성 `referee_appointments`와 허용된 알파 테스트 심판을 먼저 확정한 뒤 공개 프로필을 조회하며, 일반 프로필 상위 결과를 먼저 자르지 않는다. 이름·해시태그·지역 입력 검색에도 같은 자격 필터를 적용하고 현재 경기 선수는 결과에서 제외한다.
