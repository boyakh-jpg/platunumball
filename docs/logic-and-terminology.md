...
8. 경기 장소는 지도 선택 결과만 허용한다. 지도에서 구장을 선택하면 구장명과 내부 원주소를 보존한다. 자유 표기는 선택 입력인 짧은 장소에만 저장하며 지도 검색 지역 판정에는 사용하지 않는다. 영수증에는 짧은 장소가 있으면 그것만 표시하고, 비어 있으면 구장명을 표시하며 전체 원주소는 출력하지 않는다. 좌측 상단 일련번호는 권한이나 소유권 키가 아닌 `#BT-XXXXXX` 형식의 표시용 경기 해시태그다. 익명 영수증 seed는 최초 생성 시 한 번 만들고 사진·점수·문구 수정으로 바꾸지 않는다. canonical 경기는 서버 비밀키로 경기 ID를 HMAC한 opaque seed를 사용해 같은 경기에서 항상 같은 해시태그를 유지하고 공개 응답에 원본 경기 ID를 노출하지 않는다. seed 변경은 사용자가 `새 영수증 만들기`를 명시적으로 실행할 때만 허용한다.
