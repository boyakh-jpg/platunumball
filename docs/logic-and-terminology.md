...
1. `나와 팀`은 개인 프로필(`/app/profile`)과 팀 공간(`/app/teams`)의 공용 상위 메뉴다. 두 화면과 하위 상세 화면에서 히어로 아래의 `내 프로필 / 내 팀`으로 바로 전환한다. 팀을 플레이에 포함하지 않으며, 기존 팀 소속·초대·관리·탐색 권한과 데이터 조회 범위는 유지한다.
2. 설정(`/app/settings`)과 알림(`/app/notifications`)은 공용 상단에서 직접 연다. 게스트는 기존 `getLoginPath`로 선택한 도구의 복귀 경로를 전달하고 로그인 후 해당 화면으로 돌아온다. 메뉴 이동만으로 인증·관리자 권한을 부여하지 않는다. 관리자 진입은 프로필 보조 목록의 기존 서버 권한 기준을 유지한다.
