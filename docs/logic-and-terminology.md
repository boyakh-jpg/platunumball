...
2. 픽업 직접 참가와 초대는 통합 풀의 다음 빈자리를 사용한다. 서버 호환용 A/B·후보 값은 남은 정원에 따라 자동 균등 배치하되 체크인 전에는 사용자에게 노출하지 않는다.
3. 체크인 단계에서만 방장 또는 배정 심판이 A/B와 교대 순서를 확정한다. 점수·기록 처리를 위해 배정 이후의 A/B는 유지한다.
