...
12-1. `user_room_feed.card_json` is refreshed by DB triggers on `recruiting_posts`, `recruiting_applications`, `matches`, `match_players`, and display-name dependencies from `profiles`, `teams`, `approved_courts`, and legacy `courts`.
