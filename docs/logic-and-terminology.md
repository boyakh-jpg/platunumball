...
10. `/api/recruiting/list` default reads are compact list-card responses when no explicit `postId` is requested. Chat, old notifications, reports, matches, tournaments, and broad app metadata stay out of the list response; full room state still comes from single post detail load or `listOnly=false`.
