...
1. 팀 득실 통계는 현재 상세 경기와 팀 기록 archive를 경기 ID로 중복 제거해 합산한다. 화면에는 적재 범위를 `불러온 기록`으로 명시한다.
2. 프로필 전적은 `profile_match_summaries`의 전체 확정 경기수·승·패·무를 사용한다. 개인 기록 평균은 실제 배정 심판이 저장한 stats row가 있는 `stat_match_count`만 분모로 사용한다.
3. 프로필 요약 row가 아직 없으면 화면에 이미 적재된 최근 확정 기록만 임시 집계하고 범위를 `최근`으로 표시한다.
4. 시즌 라이벌 매치 만들기는 내 팀을 A사이드, 추천 상대를 B사이드로 지정한 비공개 팀방을 만든다. 생성 직후 기존 팀 선택 action으로 양 팀을 확정하므로 방 모달에서 B사이드 팀 검색을 다시 표시하지 않는다.
5. 경쟁전 개인 MMR 허용 범위는 선택한 1v1·2v2·3v3·5v5 모드 MMR을 기준으로 계산하고 해당 모드 값이 없을 때만 통합 MMR을 사용한다. 팀방의 기준값은 팀 MMR을 유지하되 선수별 참가 자격은 선택 모드 MMR과 연령 제한을 함께 적용한다.
...
2. 출전선수와 후보선수는 모두 출석 대상이다. 선수로 참가하는 방장도 포함하고 비선수 방장과 배정 심판은 제외한다. 시작 전 스캔은 `on_time`, 시작 시 남은 `pending`은 같은 DB transaction에서 `no_show`, 사전 등록 `no_show`의 시작 후 스캔은 `late`로 저장한다.
...
10. 대회 상세 응답은 타인의 `refereeAppointments` 원문을 노출하지 않는다. 서버가 해당 대회 심판 ID만 검증한 `eligibleRefereeIds` 스냅샷을 내려 후보 표시, 로컬 판정, DB 배정 판정이 같은 자격 결과를 사용한다.
11. 시뮬레이션 전용 `rankball-001`, `rankball-011` 계정의 심판 자격 예외는 클라이언트와 DB에 동일하게 적용한다. 일반 계정의 신뢰도·자격·임기 조건은 완화하지 않는다.
12. 참여 승인 뒤 자격이 만료·회수된 심판은 승인 이력은 유지하되 유효 심판 수와 배정 후보에서 제외하고 `교체 필요`로 표시한다.
...
43. 인증된 서버 action이 401을 반환하면 클라이언트는 현재 Supabase 세션을 한 번 갱신하고 같은 요청을 한 번만 재시도한다. 지도처럼 읽기 전용 조회가 그래도 실패하면 작성 상태를 버리지 않고 사용자가 같은 화면에서 다시 시도한다.
...
10. 사용 설명의 QR 출석은 경기시계를 사용하는 일반 공개 매칭과 대회 경기의 기능으로 안내한다. 공개 매칭은 선택 기능이고 대회는 기본 활성이다. `QR 출석은 경기 20분 전부터 시작합니다. 출전·후보 선수가 모두 출석하면 예정시간 전에도 경기를 시작할 수 있습니다.`를 기본 안내로 사용한다. 5분 토큰, 로그인한 사전 등록 선수 제한, 시작 후 QR은 사전 등록 `no_show`를 원래 사이드 후보로 등록할 뿐 실제 출전이나 팀 배치를 확정하지 않는다는 점을 함께 설명한다.
...
2. 경기시계를 사용하는 일반 경쟁전은 공개·비공개 모두 QR 출석을 기본 사용한다. 명시적으로 끌 수 있다.
3. 대회는 QR 출석을 사용한다. 경기 기록(`match_record`)과 개인 기록(`solo`), 경기시계 미사용 경기는 QR 출석을 사용하지 않는다.
