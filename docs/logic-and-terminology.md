...
11. 쿼터 휴식 권장 상한은 5분, 정규 구간 절반 뒤 하프타임은 10분이다. 휴식 시작은 서버의 최신 `endPeriod` 이벤트 시각으로 계산한다. 초과 시 시계 담당 기기에 경고와 부저를 표시하되 현장 변수를 위해 다음 구간 시작을 막거나 경기를 강제 종료하지 않는다. 휴식은 MMR 인정 진행시간에 포함하지 않는다.
