...
1. 공개 모집, 비공개 초대, 대회, 기존 경기 기록, 개인 기록 생성은 같은 단계형 탐색을 사용한다. 공개·비공개 경기는 `기본 설정 → 규칙·운영 → 구장·비용 → 확인`의 4단계다. 경기 성격, 인원, 팀, 일정은 기본 설정에 포함하고 운영·취소 정책은 규칙 단계에 포함한다. 대회·경기 기록·개인 기록은 같은 공용 단계·정책을 쓰되 비용, 후보, MMR처럼 해당 유형에 없는 항목을 렌더링하거나 저장하지 않는다.
...
4. `친선·균등출전`, `픽업·자유경기`, `일반 경쟁`, `완전 경쟁` 프리셋은 네 경기 방식에서 모두 사용할 수 있다. 일반 프리셋의 첫 단계 출전 정책은 `최소 1회 출전`과 `출전 보장 없음`을 제공하고 픽업은 `균등 순환`으로 고정한다.
5. 5v5 일반 경쟁 기본 시계는 `8분 × 4쿼터 러닝타임, 마지막 2분 스톱타임`이다. `10분 × 4쿼터 스톱타임` 공식형을 별도 프리셋으로 선택할 수 있다. 1v1·2v2·3v3은 해당 방식의 공용 기본 규칙을 사용한다.
...
1. 방 만들기 UI에서는 `ranked`를 `MMR 반영`으로 표시한다. 켜면 정규전으로 저장하고 개인·팀 MMR 후보가 되며, 끄면 친선전으로 저장한다.
2. `official`은 `검증 강화`로 표시한다. 내부 저장값은 공식경기지만 사용자는 심판·증빙 등 더 높은 신뢰 조건을 적용하는 옵션으로 이해해야 한다.
3. `preRegistered`는 `명단 사전 확정`으로 표시한다. 내부 저장값은 사전등록이지만 사용자는 경기 전 명단과 조건을 고정해 사후 변경 감액 위험을 줄이는 옵션으로 이해해야 한다.
4. 세 값은 모두 `기록 반영` 묶음 안에 둔다. 서로 다른 저장값이므로 서버 payload 이름은 바꾸지 않는다.
