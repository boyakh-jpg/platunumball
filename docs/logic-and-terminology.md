...
## 제2원칙: 업로드 이미지 저장은 Cloudflare R2

1. 사용자·운영자가 업로드하는 이미지 원본과 정규화·축소 파생본은 공개 또는 비공개 Cloudflare R2 bucket에만 저장한다.
2. Supabase Storage, DB Base64/blob, Vercel runtime filesystem에는 업로드 이미지 객체를 신규 저장하지 않는다. DB에는 불변 R2 object key와 해시·크기·형식·소유권 등 검증과 표시에 필요한 메타데이터만 저장한다.
3. 공개 이미지는 공용 asset base에서 object key로 URL을 계산하고, 비공개 이미지는 권한을 검사하는 서버 API만 읽는다. 브라우저가 R2 관리 API나 쓰기 token에 직접 접근하지 않는다.
4. 저장소에 포함해 배포하는 정적 UI 자산과 Discord 등 외부 서비스가 소유하는 원격 이미지는 이 업로드 저장 원칙의 대상이 아니다.
5. 기존 `community-post-images` Supabase Storage 경로는 전환 부채다. 기존 객체를 R2로 복사하고 DB key를 검증한 뒤 신규 업로드·조회·삭제를 R2로 전환하며, 검증 전 기존 객체를 삭제하지 않는다.
