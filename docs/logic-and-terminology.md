...
| 이의신청 | `dispute` | `status=approval/disputed`이고 창 열림 | 기록, 이의, 채팅 | 이의 제기/재개/무효 |
| 기록방 | `record` | `status=confirmed` 또는 이의 시간 만료 | 읽기 전용 기록 | 따봉 기간이면 따봉 |
...
| 출전 선수 | 내 슬롯 관리, 초대, READY | 확인 | 출석체크 | 자기 기록 확인/따봉/신고 |
| 후보 선수 | 내 후보 슬롯 관리, 초대, READY | 확인 | 출석체크 | 기록자 가능, 교체 가능, 신고 가능 |
...
4. 미출석 선수는 방장/심판이 강퇴할 수 있어야 한다.
5. 경기 직전 결원이 생기면 방 수정 가능.
6. 팀전이어도 경기준비방에서는 현실 대응을 위해 룰/인원 수정 가능.
7. 정규전에서 사후 인원 수정은 MMR 반영을 낮추거나 제외한다.
8. 심판이 있으면 경기 시작 이후 운영권은 심판으로 넘어간다.
...
6. 경기 종료 후에는 개인활약 수정 범위를 줄인다.
7. 점수와 파울은 중요하므로 종료 후에도 제한된 시간 안에 수정 가능해야 한다.
8. 경기 종료 후 갑자기 뛴 사람이 있으면 방장/심판이 등록 가능.
9. 무기명 추가 선수는 기록에는 남기되 MMR 반영 제외.
...
4. 심판이 있으면 심판이 처리한다.
5. 심판이 없으면 방장이 처리한다.
6. 이의신청 중에는 채팅과 기록판을 볼 수 있어야 한다.
7. 이의 처리 후 승인 재개 또는 무효 처리.
8. 시간이 지나면 기록방으로 넘어간다.
