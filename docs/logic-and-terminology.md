...
10. 한 줄 코멘트는 canonical 경기에서도 사용자가 직접 편집하는 선택 입력이며 경기 메모를 자동 복사하지 않는다. 두 출력 스타일은 태그·제어문자 제거와 공백 정규화를 거친 하나의 최대 22자 값을 공유한다. 출력은 약 11자 기준 최대 2줄로 나누고 비어 있으면 코멘트 행 전체를 출력하지 않는다. 개인 티어가 있으면 티어를 별도 세 번째 내용 행으로 배치한다.
