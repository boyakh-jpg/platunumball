...
5. 지역관리자 이상이 승인하면 `sanctionStatus=approved`, `official=true`, MMR 계수 1의 공식 대회가 된다. 현재 관리자 appointment에는 관할 지역 필드가 없으므로 승인 권한은 활성 `regionManager` 이상으로 강제하며 임의 일반 관리자에게 열지 않는다.
...
7. 실제 경기는 승인된 대회 심판 중 양 팀과 무관한 심판을 1명 배정한다. 같은 심판의 같은 날짜·시간 중복 배정을 금지하고, 심판 미배정·자격 만료·소속 충돌 시 일정 저장과 경기 시작을 DB에서 거부한다.
8. 기존 진행 중 대회는 갑자기 멈추지 않도록 구형 흐름을 유지한다. 새 권위 규칙은 생성 시 `rules.governanceVersion=2`가 저장된 대회부터 적용한다.
...
13. 공개·비공개 팀전의 상대 application은 주장 대표 1명 row를 DB 참가 trigger에서도 허용한다. 팀 자체의 참가 가능 인원 검사는 유지하고 실제 출전 정원은 참가 뒤 사이드장의 출전·후보 명단 확정 단계에서 강제한다.
...
11. 경기시계 점수판은 기존 시계 조회와 함께 3초마다 `match_results`를 다시 읽는다. 후보 기록자나 심판이 저장한 점수는 별도 전역 polling 없이 다음 시계 조회에 반영한다.
12. 일반 경기 결과 승인 action도 `최종 승인`으로 표시한다. 참가자는 실행 직전 이의신청을 마지막으로 확인한다. 결과·이의 화면의 수동 새로고침은 방장 또는 경기관리자에게만 제공한다.
...
9. 사후 기록의 최종 승인은 무응답을 자동 동의로 처리하지 않는다. 요청 즉시, 12시간 후, 22시간 후에 앱 알림을 만들고 연결된 Discord 알림은 사용자가 해당 연동을 켠 경우에만 같은 일정으로 보낸다.
10. 요청 후 24시간이 지나도 무응답자가 있으면 기록은 `부분 확인`으로 전환한다. 무응답자는 공개 실명 명단, 개인 통계, 전적, 업적, 추천 관계에 반영하지 않고 `미확인 참가자`로 표시한다. 확인하고 승인한 참가자의 사실 기록은 유지할 수 있다.
11. `부분 확인` 기록은 참가 인원 수와 승인 인원 수, 미확인 인원 수를 함께 표시하며 전체 확정 기록처럼 표시하지 않는다. 한 명이라도 무응답이면 경기 전체 MMR, 팀 MMR, 승패, 연승과 경쟁 통계는 반영하지 않는다.
12. 참가 사실 또는 결과가 다른 참가자가 `결과가 다릅니다`로 이의를 제기하면 무응답으로 처리하지 않고 즉시 이의조정 상태로 전환한다. 이의가 열려 있는 동안 해당 기록의 개인 통계와 전적도 반영하지 않는다.
13. 이의조정은 방장이 원본을 일방 확정하는 기능이 아니다. 관계자에게 수정안을 다시 보내고 수정된 참가 명단과 결과에 대해 새 최종 승인을 받아야 한다. 이전 승인은 수정 시각 이후의 승인으로 대체한다.
...
2. 픽업은 개인 참가만 허용한다. 선택 즉시 팀 ID·팀 명단·상대팀 대표값을 지우고 `hostJoinMode=player`, `teamOnly=false`, `ranked=false`, `official=false`, `mmrLimitMode=off`를 적용한다. 숨겨진 팀 정원 검사나 팀 부족 문구를 실행하지 않는다.
...
- 최종 승인 요청 후 24시간까지 무응답자가 있으면 자동 승인하지 않고 `부분 확인`으로 전환한다. 무응답자의 실명 통계·전적은 제외하고 경기 전체 MMR·팀 MMR·승패·연승은 반영하지 않는다. 결과 반대 응답은 이의조정으로 전환한다.
...
2. 경기 목적이 친선이면 `ranked=false`, 경쟁이면 `ranked=true`로 저장한다. 현장 픽업은 친선 목적을 강제한다. 사용자는 경기 목적에서만 MMR 반영 여부를 선택한다.
...
3. 샷클락은 방 생성 장비가 아니다. `gameClockEnabled=true`인 경기에서 경기 시작 뒤 담당자가 경기시계의 `사용 안 함/30초/35초/60초` 중 선택한다. 기록원은 실제 심판·기록 권한 배정으로만 판단한다.
...
8. 샷클락은 사용 안 함, 30초, 35초, 60초 중 선택하고 선택 즉시 서버에 저장한다. `사용 안 함`으로 저장한 경기는 실제 경기 화면에 샷클락 표시·초기화 action을 만들지 않는다. 설정 시간은 화면에서 자동 감소하고 0초에 부저를 울린 뒤 0초를 유지한다. 공격권 전환을 서버가 판정하지 않으므로 자동 재시작하지 않으며 다음 공격권에서 경기시계 담당자가 전체 타일을 눌러 초기화한다. MMR 정상 사용 판단은 샷클락이 아니라 경기시계를 기준으로 한다.
...
6. 경기시계와 심판은 선택 기능이다. 경기시계를 쓰지 않거나 심판이 없어도 경기·기록 흐름을 막지 않는다. 경기시계의 정상 사용 여부는 서버시간 기록과 기존 검증 조건으로 판단하며 샷클락 사용 여부를 검증 기준으로 쓰지 않는다.
...
7. 실제 모듈에는 `app` facade와 `clockClient` 계약만 주입한다. 안내용 연습방은 나머지 더미 참가자를 로컬 수락 상태로 채우고 한 자리를 비워 사용자가 공용 빈 슬롯 검색과 초대 action을 직접 실행하게 한다. 더미 초대 수락, 더미 출석, 예시 기록 입력, 더미 기록 승인은 사용자가 실제 공용 action을 이어서 체험하기 위한 보조 action이다. 픽업 팀 생성·배정 확정은 보조 action이 대신 처리하지 않고 사용자가 공용 `팀 나누기 → 배정 확정` action을 직접 실행한다. 채팅 polling no-op도 공용 action과 같은 동기 cleanup 반환 계약을 지킨다.
