...
3. 공개 ID 조회는 로그인 없이 누구나 영수증 표시용 안전 필드를 읽을 수 있고 수정 권한은 주지 않는다. 로그인 뒤 capability를 가진 동일 브라우저만 해당 draft를 한 사용자 프로필로 claim할 수 있으며, 다른 사용자는 QR만으로 원본 draft를 claim·수정할 수 없다.
