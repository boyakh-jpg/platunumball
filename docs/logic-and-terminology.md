...
4. 팀·대회·모집방·경기·선수 프로필 공유는 공용 상세 경로와 공유 버튼을 사용한다. 기기 공유창을 우선 열고 미지원·실패 시 링크를 복사한다. 팀·대회·공용 경기방은 현재 상세 데이터로 만든 안내 문구도 함께 공유·복사한다. 공유창 취소는 복사 없이 종료한다. 복사까지 실패하면 버튼 아래에 직접 선택 가능한 주소 또는 안내 문구와 주소를 제공한다. 공유는 조회·참가·수정 권한을 추가하지 않으며 로그인 복귀 경로에 원래 대상과 쿼리를 보존한다. 개인 기록에는 경기 공유 action을 제공하지 않는다.
