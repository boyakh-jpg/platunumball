...
5. 팀원 초대는 현재 검색 결과에서 사용자가 직접 선택한 선수만 발송 대상으로 삼는다. 검색어 변경, 결과 없음, 발송 성공 뒤에는 이전 선택을 지우며 첫 후보를 자동 선택하지 않는다.
...
1. 팀 초대 취소, 팀원 역할 변경, 팀원 제외는 하나의 팀 관리 요청만 처리한다. 처리 중에는 팀 관리 입력을 잠그고 실패 이유를 같은 패널에 표시한다.
