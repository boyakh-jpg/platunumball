...
5. 팀·대회·공용 경기방의 공유 이미지는 사용자가 요청할 때 브라우저에서 PNG로 생성한다. QR에는 공유 버튼과 같은 상세 주소를 넣으며 업로드나 별도 공유 DB row를 만들지 않는다. 이미지는 공유 시점의 안내이고 최신 정보와 참가·가입·초대·승인 조건은 원래 상세 화면을 따른다. 기존 경기 결과 영수증은 기존 발급·공유 흐름을 유지한다.
