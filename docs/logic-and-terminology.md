...
2-1. 프로필 아이콘은 검수된 300개 allowlist의 확장자 없는 icon key만 `profiles.avatar_icon_key`에 저장한다. 기존 `01~80`은 2자리, 확장 `081~300`은 3자리 번호를 사용한다. 화면은 key로 자산 경로를 계산하며 allowlist 밖 key, 삭제된 파일, 이미지 로드 실패는 이름 첫 글자로 대체한다.
2-2. 잠긴 아이콘은 카탈로그에서 미리 볼 수 있지만 선택·저장은 서버에서 거부한다. 달성한 icon key는 `profile_icon_unlocks`에 저장하며 MMR·신뢰도·연승이 내려가도 다시 잠그지 않는다.
2-3. `06~20`은 순서대로 팀 가입 1회, 확정 경기 3·5·10회, 누적 50득점, 3승, 팀 주장, 누적 20어시스트, 누적 100득점, 누적 50어시스트, 누적 200득점, 2점 차 이내 승리 3회, 스틸+블록 25개, 50리바운드, 경기 10회와 신뢰도 90을 조건으로 한다.
...
2-5. `51~65`는 순서대로 첫 승, 3연승, 2점 차 이내 승리, 더블더블, 트리플더블, 경기 50회, 20득점·리바운드+어시스트 10 이상 경기 3회, 팀 경기 10회, 100어시스트, 심판 5회, 기록원 5회, 구장 후기·승인 제보 3회, 팀 주장, 경기 20회와 신뢰도 95, 경기 30회와 신뢰도 98을 조건으로 한다.
2-6. `66`은 확정 경기 1회다. `67~75`는 통합 MMR 800·1000·1200·1400·1600·1800·2000·2200·2400 도달 시 순서대로 해금한다.
2-7. `76~80`은 순서대로 확정 경기 25회, 밤 9시 이후 경기 10회, 5연승, 리바운드+블록 150개, 확정 경기 100회·60승·통합 MMR 2000·신뢰도 95를 조건으로 한다.
2-8. 프로필 아이콘 팝업의 `저장`은 표시 원본, icon key, 배경색, 테두리 사용 여부와 색을 한 DB 함수에서 함께 검증·저장한다. `initial` 또는 `discord`로 전환해도 기존 `avatar_icon_key`를 삭제하지 않는다.
2-9. `081~300`은 44개 업적 계열을 브론즈·실버·골드·플래티넘·레전드 다섯 단계로 나눈다. 발송만으로 늘어나는 초대, 미승인 구장 등록, 취소·미확정 경기는 업적에 포함하지 않는다.
2-10. `081~140`은 확정 경기, 승리, 최고 연승, 접전 승리, 랭크 경기, 공인 경기, 공개 경기, 비공개 경기, 팀 경기, 팀 경기 승리, 비공개 팀전, 매칭 성사 횟수의 단계 업적이다. 매칭 성사는 `recruiting_posts.confirmed_at`이 있고 해당 프로필이 방장·출전자·신청자·파티 후보 중 하나일 때만 센다.
2-11. `141~180`은 누적 득점·리바운드·어시스트·스틸·블록, 더블더블, 트리플더블, 고효율 경기 횟수의 단계 업적이다. 고효율 경기는 한 경기 20득점 이상과 리바운드+어시스트 10 이상을 동시에 충족해야 한다.
2-12. `181~205`는 확정 경기별 득점·리바운드·어시스트·스틸·블록 1위 횟수의 단계 업적이다. 공동 1위도 인정하되 각각 10득점·5리바운드·5어시스트·2스틸·2블록의 최소 기록을 충족해야 한다.
2-13. `206~230`은 `1v1`, `2v2`, `3v3`, `4v4`, `5v5` 확정 경기 횟수의 단계 업적이다.
2-14. `231~270`은 심판, 기록원, 수락된 매칭 초대, 수락된 팀 초대, 승인된 구장 등록, 활성 구장 리뷰, 후보 등록 경기, 후보 출전 승격 횟수의 단계 업적이다. 초대는 발송 수가 아니라 상대가 수락해 저장된 건만 센다.
2-15. `271~300`은 대회 확정 경기, 참가한 서로 다른 대회, 대회 경기 승리, 토너먼트 결승 진출, 토너먼트 우승, 실제 시작한 대회 개최 횟수의 단계 업적이다. 토너먼트 결승은 해당 대회의 가장 높은 저장 라운드로 판정한다.
3. 팀 엠블럼 표시 원본은 `initial`, `upload` 중 하나다. 기본값으로 전환해도 저장된 사진 key를 삭제하지 않으며, 다시 `upload`를 선택하면 같은 사진을 사용한다.
3-1. 팀 기본값 글자 기준은 기존 첫 글자 `initial`, 전체 `팀 이름`, 팀장이 입력한 1~8자 `약칭` 중 하나다. 기존 팀은 `initial`을 유지한다. 팀 이름은 공백을 우선하고 필요하면 문자 경계에서 최대 세 줄로 균형 배치해 원형 안전영역을 넘지 않는다.
...
1. 5분 주기 worker에서 경기 시작 시각 정각 알림은 최대 약 5분 늦을 수 있으므로 만들지 않는다.
2. 방관리자 시작 안내는 경기 10분 전 출석 확인과 5분 전 시작 준비로 예약한다. 5분 전 Discord delivery는 예정 시작 시각을 `expiresAt`으로 가지며, worker가 시작 시각 이후에 잡으면 발송하지 않고 `discord_notification_expired`로 취소한다.
...
6. 회귀 테스트의 현재 생성 기대값은 `match-manager-start-5m`이며 폐기된 `match-manager-start-now`는 생성 필수값이 아니다. `match-manager-start-now`는 과거 대기 row 정리 검증에만 포함한다.
...
9. 대회 일정 저장은 시작 전 확정 경기(`confirmed`)에도 허용한다. 시작·종료·취소·무효·닫힘 상태만 일정 변경을 막는다.
10. 일정 저장 후 양 팀장에게 홈 처리 항목·웹 알림·Discord 전달 대상 알림을 만들고, 팀장은 생성 시점 스냅샷에서 출전 인원 정확히 N명과 후보 최대 2명을 확정한다.
...
| --- | --- |
| 경기 시작 전 | 기록 불가 |
| 경기 중 + 심판 있음 | 현재 심판이 양 사이드 전체 기록 |
| 경기 중 + 심판 없음 + 후보 기록자 있음 | 기록자가 자기 사이드 전체 기록 |
...
팀 점수는 별도 입력값이 아니다. 각 사이드 실제 출전 선수의 `PTS` 합계를 단일 원본으로 사용하며 모든 기록 저장 때 다시 계산한다.
공개·비공개, 개인전·팀전, 친선·정규, 리그·토너먼트 여부는 위 기록 권한을 바꾸지 않는다.
...
7. 구장 이름은 지도 핀의 공식 시설명과 구조화된 코트 구분을 합쳐 자동 생성한다. 주소의 동 이름은 기본 이름에 붙이지 않는다.
...
21. 서로 다른 장소의 표준 구장명이 같으면 동 또는 지역 식별자를 이름 뒤에 붙이고, 해시태그는 별도 고유 식별자로 유지한다.
...
35. 구장 카드의 외부 지도 링크는 저장된 좌표로 화면을 맞추면서 구장명과 주소 검색을 함께 전달해 장소 결과를 표시한다. 좌표가 없을 때만 이름·주소 검색으로 대체한다.
...
- 선수 상세 프로필도 같은 대표팀 계산 결과만 표시하고 해당 팀 상세로 연결한다. 공개 화면에서 전체 `app_settings`를 읽거나 노출하지 않는다.
...
3. 최초 선점 뒤 `tournamentSideAssignmentLocked=true`를 저장한다. 일정 변경은 출전 확정과 마감 상태만 초기화하고 A사이드·방장 배정은 바꾸지 않는다.
...
4. 시즌 허브의 처리할 경기는 경기 메뉴로 이동하지 않고 시즌 화면 위의 공용 경기방 모달로 연다.
