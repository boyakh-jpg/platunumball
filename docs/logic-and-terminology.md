...
13. canonical 팀에 업로드 엠블럼이 있으면 브라우저 Canvas가 배경색과 경계를 기계적으로 판별해 투명 금색 선화 후보를 만든다. 사용자가 홈팀·원정팀별 `선화 엠블럼 사용`을 선택한 경우에만 미리보기와 저장본에 적용하며, 변환 품질 기준을 통과하지 못하면 선택 여부와 무관하게 중립 엠블럼을 유지한다. 임의 클라이언트 object key는 허용하지 않고 서버가 확인한 canonical `team-emblems/` key만 공개 스냅샷에 보존한다.
