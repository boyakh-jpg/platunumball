...
5. 확정 경기 영수증은 기존 경기의 팀·점수·날짜·장소·팀 MMR·현재 사용자 개인 스탯을 사용하고 `BOXTIER VERIFIED`로 표시한다. 양 팀 canonical ID가 모두 있는 확정 팀전만 팀 티어 선화 엠블럼을 표시한다. 직접 작성 draft와 공개 draft는 verified·canonical 팀전 상태를 만들 수 없고 공식 전적이나 MMR에 반영하지 않는다. QR은 canonical 경기 또는 서버 공개 영수증 URL만 가리키며 외부 지도 URL과 소유권 비밀을 포함하지 않는다. 웹 미리보기의 QR 영역도 같은 조회 대상으로 이동한다.
...
10. 한 줄 코멘트는 canonical 경기에서도 사용자가 직접 편집하는 선택 입력이다. 경기 메모를 자동 복사하지 않고 최대 10자로 제한하며, 비어 있으면 영수증에 출력하지 않는다.
