...
1. 비회원 경기 영수증의 입력값은 브라우저에 24시간 임시 보관한다. 배경 사진은 IndexedDB에만 보관하고 서버, Supabase Storage, R2에 업로드하지 않는다. 사진은 하나의 크롭 영역에서 Pointer Events로 한 손가락 가로·세로 이동과 두 손가락 이동·확대·축소·회전을 처리한다. 데스크톱 자유 회전 버튼도 같은 canonical 회전값을 갱신하며 별도 변환 슬라이더를 두지 않는다.
...
10. 한 줄 코멘트는 canonical 경기에서도 사용자가 직접 편집하는 선택 입력이다. 경기 메모를 자동 복사하지 않고 최대 11자로 제한하며, 비어 있으면 영수증에 출력하지 않는다.
