...
2-9. `081~300`은 44개 업적 계열을 브론즈·실버·골드·플래티넘·레전드 다섯 단계로 나눈다. 업적은 서버가 확인한 확정 경기·승리·출전 snapshot·매칭 확정·초대 수락·운영 수행·승인 구장·대회 상태만 사용한다. 사용자가 입력할 수 있는 득점·리바운드·어시스트·스틸·블록, 더블더블·트리플더블, 경기별 기록 순위는 해금 조건에 사용하지 않는다. 조건 변경 전에 해금된 icon key는 회수하지 않는다.
...
2-13. `206~230`은 `1v1`, `2v2`, `3v3`, `4v4`, `5v5` 확정 경기 5·25·75·150·300회의 단계 업적이다.
...
7. 구장 이름은 핀 주소로 조회한 시설/장소명과 구조화된 코트 구분을 합쳐 생성한다. 장소 후보는 사용자가 직접 선택하고 후보가 없으면 직접 입력하며, 주소의 동 이름은 기본 이름에 붙이지 않는다.
...
36. 장소 후보는 핀의 도로명·지번 주소로 Naver 지역 검색에 등록된 공원·체육시설·학교·공공장소를 조회한다. 동일 주소를 우선하고 좌표가 확인되는 후보는 500m 이내만 보조로 보여주며 최대 3개를 사용자가 선택한다. 검색 실패나 후보 없음은 등록을 막지 않고 직접 입력으로 대체한다.
