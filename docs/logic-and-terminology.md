...
11. 영수증 경기 방식은 공용 지원 방식과 같은 `1v1`, `2v2`, `3v3`, `5v5`만 사용한다. `other`를 별도 방식으로 만들지 않으며 알 수 없는 레거시 값은 `3v3`으로 정규화한다.
12. 대회·리그 이름과 1Q·2Q·3Q·4Q·OT 점수는 모두 선택 표시값이다. 입력값은 영수증의 두 팀 엠블럼 사이에만 출력하며 canonical 최종 점수, 경기 진행 상태, 대회 결과, 개인·팀 MMR을 변경하지 않는다.
