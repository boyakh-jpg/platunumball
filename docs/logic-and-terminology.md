...
1. 비회원 경기 영수증의 입력값은 브라우저에 24시간 임시 보관한다. 배경 사진은 IndexedDB에만 보관하고 서버, Supabase Storage, R2에 업로드하지 않는다. `BOXTIER SCORE`의 사진은 하나의 크롭 영역에서 Pointer Events로 한 손가락 가로·세로 이동과 두 손가락 이동·확대·축소·회전을 처리한다. 데스크톱 자유 회전 버튼도 같은 canonical 회전값을 갱신하며 별도 변환 슬라이더를 두지 않는다. `CLASSIC THERMAL`은 같은 canonical 사진 변환값을 명시적 이동·확대 입력으로 편집할 수 있다.
...
10. 한 줄 코멘트는 canonical 경기에서도 사용자가 직접 편집하는 선택 입력이며 경기 메모를 자동 복사하지 않는다. `BOXTIER SCORE`는 최대 11자다. `CLASSIC THERMAL` 한줄평은 태그·제어문자 제거와 공백 정규화 뒤 CJK 문자 가중치 2, 그 외 문자 가중치 1로 최대 56이며 비어 있으면 행 전체를 출력하지 않는다.
...
16. `CLASSIC THERMAL`은 `receiptStyle`, `receiptLocale`, `includePhoto`, 사진 변환값, `receiptComment`, 양 팀 짧은 이름을 로컬 draft·공개 snapshot·복제·claim 경로에 보존한다. 사진 원본과 화면 메모리의 임시 엠블럼은 공개 payload에 포함하지 않는다. 공식 경기 ID·점수·구간 점수·출전자 수·심판 배정·검증 여부는 trusted canonical 경로에서만 저장하며 사용자가 `VERIFIED`를 만들 수 없다.
