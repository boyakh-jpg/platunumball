...
2. Google 로그인 복귀 뒤 draft를 유지하되 기존 경기 연결 기능이 완성되기 전에는 자동 저장하거나 중복 경기를 생성하지 않는다.
3. 공개 QR은 canonical 박스티어 공개 경기 URL이 있을 때만 사용한다. 임시 영수증에는 QR을 만들지 않고 외부 지도 URL을 직접 담지 않는다.
