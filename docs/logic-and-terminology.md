...
12. 경기 종료 권한자는 경기시계 인정 조건과 무관하게 경기 lifecycle을 종료할 수 있다. 시작 창 또는 최소 인정시간을 충족하지 못한 상태의 버튼은 `강제 종료`로 표시하고, 종료된 경기시계가 미사용 판정이어도 이 action을 숨기지 않는다.
...
1. 구장 신청은 모바일 후면 카메라를 우선 여는 native capture input으로 현장 사진 1~4장을 한 장씩 누적한다. 촬영한 사진은 개별 재촬영·삭제할 수 있다. 원본 파일 용량을 제한하지 않고 브라우저에서 긴 변 1280px 이하, WebP, 장당 목표 220KB로 자동 변환하며 서버는 장당 300KB 이하 결과만 받는다.
2. 사진은 공개 자산 버킷과 분리된 비공개 R2 버킷에 `court-requests/{requestId}/{sha256}.webp`로 저장한다. DB에는 object key와 해시만 저장하고 관리자 전용 API만 원본을 읽는다.
3. 구장 신청은 현장 GPS 확인으로 시작한다. 서버와 지도에 사용할 기본 핀은 현재 GPS 좌표로 자동 지정하고 해당 좌표를 역지오코딩해 주소를 채운다. 주소 조회 실패나 위치 보정이 필요할 때만 주소 검색·지도 핀 조정을 사용하며, 조정 뒤에는 현장 GPS와 핀 거리를 다시 확인한다. 위치 확인이 없거나 10분이 지난 경우 촬영을 진행하지 않는다. 위치 정확도 20m 이하이고 핀과 현장 위치가 30m 이하일 때만 자동승인하며 정확도·거리 기준을 벗어나면 관리자 검토로 보낸다.
4. AI는 사진별 농구 코트·골대·전체 모습·코트 형태·스크린샷 또는 합성 위험을 구조화해 반환한다. 최종 결정은 서버 정책 함수가 하며 AI 응답에 DB 승인 권한을 주지 않는다.
5. 자동승인은 야외·공개 구장, 사진 2장 이상, 신뢰도 90 이상 또는 과거 승인 2건 이상, 30m 내 중복 없음, 사진별 구조화 판정, 서버의 필수 시각 증거 5개, 사진별 EXIF 위치 일치가 모두 충족되어야 한다. 같은 사진 재사용은 허용하지 않는다. EXIF 위치가 없거나 일부 사진에만 있으면 신청은 저장하되 자동승인하지 않는다.
...
21. 사진 위치는 모든 사진이 현장 GPS와 지도 핀에서 각각 50m 이내면 `matched`, 일부 사진만 위치가 있으면 `partial`, 최대 차이가 50m 초과 150m 이하면 `uncertain`, 150m 초과면 `mismatch`, 위치가 없으면 `unavailable`이다. 증거 신뢰도는 각각 1.0, 0.85, 0.6, 0.25, 0.75로 계산하며 `matched`만 자동승인 후보가 된다. 이 값은 해당 신청의 증거 신뢰도일 뿐 사용자 신뢰점수를 자동 차감하지 않는다. 사용자 제재는 18항의 관리자 인정 절차만 따른다.
