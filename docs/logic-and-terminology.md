...
8. 일반 경기의 최종 승인은 배정 심판이 있으면 배정 심판만, 무심판이면 방장만 실행한다. 결과 제출 뒤 최소 3분의 이의 제출 시간을 보장하고, 실행 직전 `더 이상 이의가 없음을 확인하셨나요?` 확인창을 거친다. 과반 승인, 전원 승인, 팀장 승인은 일반 경기에 적용하지 않는다.
...
11. 일반 live 경기, `match_record`, `personal_record`는 서로 다른 기록 유형이다. `match_record`는 24시간 동안 실제 참가자의 `내 참가 확인`과 문제 신고를 받고, 전체 참가자의 2/3 이상이 확인했으며 열린 이의·신고가 없을 때 자동 확정한다. 개인 스탯 row는 만들지 않고 확인자에게만 `ratingScale=0.2`의 낮은 개인 MMR 반영률을 적용하며 팀 MMR은 반영하지 않는다. `personal_record`는 작성자 본인 소유·MMR 미반영 규칙을 유지한다.
12. 일반 경기의 운영자 수동 승인이 없어도 `disputeMinutes`가 끝나고 열린 이의·신고가 없으면 보험성 자동 확정을 시도한다. 심판 경기는 배정 심판의 개인기록이 완성된 경우에만 자동 확정하고, 무심판 경기는 개인기록 없이 점수만 확정한다. 이 자동 확정은 운영 권한을 다른 참가자에게 승계하지 않는다.
...
6. 사후 기록방은 참가 사실과 입력된 경기 결과를 참가자 본인의 `내 참가 확인` 한 번으로 처리한다. 방장, 팀장, 심판은 다른 사람을 대신해 확인할 수 없다.
7. 개인 구성 기록방은 `명단 제출 → 점수 입력 → 각 참가자의 내 참가 확인` 순서다. 팀 구성 기록방은 `팀 선택 → 각 팀장 출전 명단 제출 → 점수 입력 → 각 참가자의 내 참가 확인` 순서다. 팀 선택·명단 제출과 참가 확인을 같은 상태로 표시하지 않는다.
8. 사후 기록방을 중단하는 행동과 알림은 `경기 취소`가 아니라 `기록 취소`로 표시한다. 예정 경기 취소·경기 무효와 구분한다.
9. 사후 기록은 결과 제출 직후와 12시간 후에 미확인자에게 확인 알림을 보낸다. 참가자는 본인 참가 사실과 입력된 결과를 한 번에 확인하거나 공통 경기 신고를 사용한다.
10. 결과 제출 24시간 뒤 전체 실제 참가자의 2/3 이상이 확인했고 열린 이의·신고가 없을 때 기록을 자동 확정한다. 확인자에게만 `ratingScale=0.2`의 낮은 개인 MMR을 반영하고 팀 MMR은 반영하지 않는다.
11. 자동확정 전까지는 `N/M명 확인`과 `확정 기준 2/3 이상`을 표시한다. 자동확정 뒤 불복은 공통 경기 신고로 처리한다.
12. 열린 이의·신고가 있으면 자동 확정과 MMR 반영을 보류한다. `match_record`에는 개인기록 row, 미제출 검사, 개인 득점 합계 검사를 만들지 않는다.
13. 참가 확인 요청은 멱등 키를 사용하고 요청자·응답자·시각·IP/기기 위험 신호·변경 전후 값을 감사 로그에 남긴다. 같은 계정의 다중 슬롯, 본인 대리 확인, 명단 수정 뒤 이전 확인 재사용을 서버에서 거부한다.
...
- 개인 구성은 방모달에서 A/B 참가자를 각 사이드 정원만큼 계정으로 선택한다. 생성자는 A사이드에 포함되어야 하고 중복 선수는 금지한다. 점수 입력이 끝나면 각 참가자가 본인 참가 사실과 입력된 결과를 `내 참가 확인`으로 확인한다.
- 팀 구성은 방모달에서 서로 다른 등록 팀 두 개를 먼저 고른다. 각 팀 주장이 자기 팀의 출전 명단만 정원만큼 제출하며 후보 슬롯은 사용하지 않는다. 점수 입력 뒤 각 참가자는 본인 계정으로 `내 참가 확인`을 할 수 있다.
- 참가 명단과 팀 로스터가 끝나기 전에는 참가 확인 요청을 막고, 요청 뒤 명단이나 결과를 바꾸면 기존 확인을 무효화한다. 서버는 `recordApproverIds`와 실제 출전 명단을 함께 검증하며 방장·팀장 대리 확인을 거부한다.
- 점수 제출 24시간 뒤 참가자의 2/3 이상이 확인했고 열린 이의·신고가 없으면 자동확정한다. 확인자에게만 낮은 개인 MMR 반영률을 적용하고 미확인자는 제외하며 팀 MMR은 반영하지 않는다.
...
9. `경기 기록`, `대회`는 이전 화면의 `pickup` 상태를 상속하지 않는다. 경기 기록은 후보 없는 사후 명단과 확인자 한정 `ratingScale=0.2` 개인 MMR·팀 MMR 미반영 정책을 사용하고, 대회는 일반 명단과 선택한 MMR 정책을 적용한다.
