...
9. 심판 임명과 관리자 임명은 `startsAt`, `endsAt`, `appointedBy`, `reason`을 가진 기간제 권한이다.
10. 관리자 등급은 `owner`, `opsLead`, `moderator`, `support` 순서로 둔다.
11. 상위 등급만 하위 등급 임명을 제안할 수 있고, 최종 적용은 서버 권한과 `auditLog`를 통과해야 한다.
12. 만료된 임명은 자동 비활성으로 취급한다.
...
2. 구장 등록요청에는 실제 주소, 상세 위치 메모, 위도/경도가 있어야 한다.
3. 한국 주소 배포 기준은 Daum 주소검색으로 주소를 선택하고 Kakao Maps Geocoder로 위도/경도를 저장한다.
...
1. 팀 선택 없음.
2. 방장이 초대한 사람만 들어온다.
3. 같은 사이드에 같은 실제 팀 소속 선수가 있으면 파티 맺기 제안 가능.
4. A/B사이드는 각자 내 슬롯 관리에서 이동 가능.
5. 비공개 개인전도 같은 방 모달을 쓴다.
...
4. 상대 팀 참여자는 출전/후보를 선택해서 들어온다.
5. 출전은 경기 방식 수만큼, 후보는 최대 2명까지 가능하다.
6. 팀 전용 공개방에서는 다른 팀 초대/참여를 아무나 할 수 없다.
...
| --- | --- | --- | --- | --- |
| 방장 | 방 수정, 초대, 강퇴, 확정, 취소 | 방 수정, 취소 | 출석 관리, 강퇴, 룰 수정, 시작 | 심판 없을 때 종료/결과/이의 처리 |
...
| 심판 | 심판 표시 | 심판 표시 | 경기 시작 전 대기 | 기록/종료/이의/결과 처리 우선 |
...
5. 초대 수락 시 방이 꽉 차 있으면 입장 실패.
6. 출전 명단 확정 후 남은 초대장은 만료.
7. 초대받은 방은 홈 액션 큐와 매칭 필터에서 눈에 띄게 표시.
8. 초대 수락 후 바로 해당 방 모달을 연다.
...
4. 출석체크는 방장/심판이 처리한다. 본인이 직접 출석 처리하지 않는다.
5. 미출석 선수는 방장/심판이 강퇴할 수 있어야 한다.
6. 경기 직전 결원이 생기면 방 수정 가능.
7. 팀전이어도 경기준비방에서는 현실 대응을 위해 룰/인원 수정 가능.
8. 정규전에서 사후 인원 수정은 MMR 반영을 낮추거나 제외한다.
9. 심판이 있으면 경기 시작 이후 운영권은 심판으로 넘어간다.

## 경기 시작/종료 원칙

1. 경기 시작은 방장 또는 심판이 누른다.
2. 경기 종료는 방장 또는 심판이 누른다.
3. 심판이 있으면 심판이 우선한다.
...
4. 심판이 있으면 심판이 처리한다.
5. 심판이 없으면 방장이 처리한다.
6. 이의신청부터 방 모달은 열람 전용이다. 채팅 입력, 슬롯 관리, 초대, 방 수정, 경기 운영 버튼은 닫는다.
7. 이의신청이 접수되면 기존 결과를 `disputeDraftResult`로 복제한다.
8. 이의 수정 중에는 `match.result`를 직접 덮지 않고 `disputeDraftResult`만 임시 저장한다.
9. 이의 처리자는 심판이 있으면 심판, 심판이 없으면 방장이다.
10. 이의 처리자가 수정안을 확인하면 양팀 재승인 없이 바로 결과를 확정한다.
11. 확정 후 불복은 재승인이 아니라 신고로 처리한다.
12. 무효가 필요하면 무효 처리한다.
13. 시간이 지나면 기록방으로 넘어간다.
...
1. 구장은 해시태그를 가진다.
2. 구장 해시태그는 사용자가 정하지 않고 숫자로 자동 부여.
3. 정식구장과 골대만 있는 장소를 구분할 수 있어야 한다.
4. 주소, 위치 설명, 조명, 유료/무료, 예약 가능 여부를 가진다.
5. 구장 클릭/호버 시 구장 카드가 뜬다.
6. 지도는 카드에서 `지도로 보기`를 눌렀을 때만 연다.
7. 구장 등록 요청은 설정에서 한다.
8. 사진 업로드는 비용/트래픽 때문에 최소화한다.
