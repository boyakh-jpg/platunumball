...
3. 사용자는 현재 GPS로 핀·주소를 자동 지정하거나 주소 검색 후 지도 핀을 확정한다. 이 선택은 입력 방법일 뿐 승인 종류가 아니다. 서버는 10분 이내 현장 GPS가 정확도 20m 이하이고 핀과 30m 이내인 경우와 사진 EXIF GPS가 핀과 일치하는 경우를 각각 위치 증거로 계산한다.
4. AI는 사진별 농구 코트·골대·전체 모습·코트 형태·스크린샷 또는 합성 위험을 구조화해 반환한다. 최종 결정은 서버 정책 함수가 하며 AI 응답에 DB 승인 권한을 주지 않는다.
5. 자동승인은 야외·공개 구장, 사진 2장 이상, 신뢰도 90 이상 또는 과거 승인 2건 이상, 30m 내 중복 없음, 사진별 구조화 판정과 필수 시각 증거 5개가 모두 충족되어야 한다. 위치는 유효한 현장 GPS 또는 모든 사진의 EXIF GPS 일치 중 하나로 충족한다. 사진 GPS가 핀과 `uncertain` 또는 `mismatch`이면 현장 GPS가 있어도 자동승인하지 않는다. 같은 사진 재사용은 허용하지 않는다.
