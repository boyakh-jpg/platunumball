...
1. 비회원 경기 영수증의 입력값은 브라우저에 24시간 임시 보관한다. 배경 사진은 IndexedDB에만 보관하고 서버, Supabase Storage, R2에 업로드하지 않는다. 두 출력 스타일은 하나의 사진과 canonical 변환값을 공유한다. 미리보기 Pointer Events와 이동·확대·회전 입력은 모두 같은 값을 갱신하며 스타일 전환 시 사진 편집 상태를 초기화하지 않는다.
...
10. 한 줄 코멘트는 canonical 경기에서도 사용자가 직접 편집하는 선택 입력이며 경기 메모를 자동 복사하지 않는다. 두 출력 스타일 모두 태그·제어문자 제거와 공백 정규화 뒤 최대 22자로 제한한다. `BOXTIER SCORE`는 11자 단위 최대 2줄로 조판하고 개인 티어가 있으면 그 다음 3번째 줄에 둔다. 비어 있으면 감열 영수증의 한줄평 행 전체를 출력하지 않는다.
...
16. 영수증 입력 화면은 하나의 canonical draft만 사용하고 `receiptStyle`은 미리보기·내보내기 renderer만 바꾼다. `receiptLocale`, `includePhoto`, 사진 변환값, `comment`, 양 팀 짧은 이름은 로컬 draft·공개 snapshot·복제·claim 경로에 공통 보존하며 기존 `receiptComment`는 `comment`와 동기화한다. 사진 원본과 화면 메모리의 임시 엠블럼은 공개 payload에 포함하지 않는다. 출전자 수는 canonical 실제 출전자 ID 집합에서 계산하고 공식 경기 ID·점수·구간 점수·심판 배정·검증 여부와 함께 trusted 경로에서만 저장한다.
