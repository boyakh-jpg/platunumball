...
- 경기 메뉴 방 분기 필터는 방만들기 생성 흐름 기준으로 `공개 모집`, `비공개 초대`, `개인전`, `팀전`, `기록방`을 나눈다.
- `공개 모집`/`비공개 초대`는 `visibility` 기준이고, `개인전`/`팀전`은 실제 팀 파티/팀 id/`hostJoinMode` 기준이며, `기록방`은 `rules.recordType='solo'` 기준이다.
