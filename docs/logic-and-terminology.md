...
5. 확정 경기 영수증은 기존 경기의 팀·점수·날짜·장소·팀 MMR·현재 사용자 개인 스탯을 사용하고 `BOXTIER VERIFIED`로 표시한다. 양 팀 canonical ID가 모두 있는 확정 팀전만 팀 티어 선화 엠블럼을 표시한다. 직접 작성 draft와 공개 draft는 verified·canonical 팀전 상태를 만들 수 없고 공식 전적이나 MMR에 반영하지 않는다. QR은 canonical 경기 또는 서버 공개 영수증 URL만 가리키며 외부 지도 URL과 소유권 비밀을 포함하지 않는다.
...
8. 지도에서 구장을 선택하면 짧은 지역 주소와 원주소를 별도 보존하고, 영수증에는 짧은 주소를 우선 표시하며 비어 있을 때만 원주소를 표시한다. 좌측 상단 일련번호는 권한이나 소유권 키가 아닌 `#BT-XXXXXX` 형식의 표시용 경기 해시태그다. canonical 경기 또는 공개 draft ID가 있으면 해당 공개 ID를 seed로 사용하고, 없으면 영수증 입력값으로 결정적으로 생성한다.
9. 확정 경기의 공용 경기방 모달은 `경기 영수증 만들기`를 노출하고 canonical match ID를 `/app/receipt?match={matchId}`로 전달한다. 프로필 기록 등 진입 화면마다 별도 모달이나 경기 계산을 복제하지 않는다.
