...
13. 영수증 편집 화면은 Team A·B별 로컬 사진을 공용 원형 크롭 팝업에서 중심·확대·가로·세로 위치 조정하고, 결정론적 로컬 선화 변환을 확인한 뒤 이번 영수증에만 적용할 수 있다. 원본과 변환 결과는 현재 화면 메모리에만 두고 DB·object key·guest 업로드·draft·공개 스냅샷·R2·Storage·IndexedDB에 저장하지 않으며 초기화·새로고침 시 폐기한다. 로그인 후 팀을 만들고 팀 상세의 `영수증 엠블럼 만들기`에서 저장한 canonical 팀 자산은 Team A·B별 `저장 엠블럼 사용`으로 다음 영수증에서도 재사용한다. 팀 상세는 같은 공용 원형 크롭·자동 변환·확인 절차와 서버 검증을 거쳐 `teams.receipt_emblem_key`·`receipt_emblem_updated_at` 및 `team-emblems/{teamId}/` R2 key를 관리한다. 영수증 화면은 팀이 없거나 저장본이 없으면 로그인·팀 생성 경로를 안내하며 임의 클라이언트 object key와 guest 엠블럼 key를 draft에 보존하지 않는다.
   편집 중 엠블럼 resolver는 현재 영수증의 로컬 선화를 우선하고, 없으면 선택한 canonical 팀 저장본을 사용한다. 공개 스냅샷은 사용자가 고른 적용 상태와 `teams.receipt_emblem_key`만 함께 보존해 QR 재조회·새로고침에서도 canonical 팀 영수증 엠블럼을 재현한다. 기록 영수증은 현재 팀의 `receipt_emblem_key`를 우선 사용하고, 팀 목록 로딩 전에는 공개 스냅샷 또는 draft에 보존된 canonical key로 복원한다. 사용자가 선택한 `homeUseLineArt`·`awayUseLineArt` 상태가 참일 때 미리보기와 저장 Canvas에 같은 resolver 결과를 반영한다. 실제 canonical 팀의 저장 엠블럼은 티어 엠블럼을 대체하지만 팀 티어 문구는 유지하고, canonical 팀이 없는 무기명 영수증은 팀 티어 문구를 비운다. 선화 결과는 투명 정사각 영역 안에서 비율을 보존해 정규화하고 선화·저장본·티어·중립 엠블럼은 동일한 표시 영역을 사용한다.
...
15. AI 선화 변환은 앱이 직접 호출하지 않는다. 사용자가 외부 도구에 붙여 넣을 프롬프트만 복사하며, 원형·비율·글자를 유지한 고대비 단색 선화와 완전 투명 alpha PNG를 우선 요구한다. alpha 출력이 불가능한 도구에만 균일한 순수 `#00FF00` 배경을 보조 조건으로 허용한다. 반환 이미지는 다시 로컬 기계 변환과 사용자 선택을 거쳐야 하며 자동 적용하지 않는다.
...
17. `CLASSIC THERMAL` Story와 Feed는 같은 정규화 데이터와 결정론적 Canvas 레이아웃을 사용한다. 사진 제외는 사진 포함 레이아웃에서 정확히 288px을 제거하고, 구간 점수·한줄평이 없으면 각각의 영역과 종이 높이도 제거한다. Feed는 종이 전체를 축소해 표시하며 어느 영역도 크롭하지 않는다. QR은 공개 영수증 URL만 담고 다른 감열 노이즈 처리에서 제외한다.
