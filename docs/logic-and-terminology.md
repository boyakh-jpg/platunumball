...
2. 유효한 비회원 영수증은 공유 시 서버에 사진을 제외한 공개 draft를 만든다. QR에는 추측 불가능한 공개 ID만 넣고 소유권 capability는 공개 ID별로 원 작성자 브라우저의 `HttpOnly`, `SameSite=Lax` 쿠키와 서버 해시로 분리한다. draft는 30일 뒤 만료하며 생성은 IP 해시 기준 시간당 10건으로 제한한다.
3. 공개 ID 조회는 로그인 없이 누구나 영수증 표시용 안전 필드를 읽을 수 있고 수정 권한은 주지 않는다. 공개 응답은 내부 원주소와 정확한 개인 MMR을 제외한다. 로그인 뒤 capability를 가진 동일 브라우저만 해당 draft를 한 사용자 프로필로 claim할 수 있으며, 다른 사용자는 QR만으로 원본 draft를 claim·수정할 수 없다.
...
8. 경기 장소는 지도 선택 결과만 허용한다. 지도에서 구장을 선택하면 구장명과 내부 원주소를 보존한다. 자유 표기는 짧은 장소에만 저장하며 지도 검색 지역 판정에는 사용하지 않는다. 경기 장소와 짧은 장소 중 하나만 있으면 위치 필수값을 충족한다. 영수증에는 짧은 장소가 있으면 그것만 표시하고, 비어 있으면 구장명을 표시하며 전체 원주소는 출력하지 않는다. 좌측 상단 일련번호는 권한이나 소유권 키가 아닌 `#BT-XXXXXX` 형식의 표시용 경기 해시태그다. 익명 영수증 seed는 최초 생성 시 한 번 만들고 사진·점수·문구 수정으로 바꾸지 않는다. canonical 경기는 서버 비밀키로 경기 ID를 HMAC한 opaque seed를 사용해 같은 경기에서 항상 같은 해시태그를 유지하고 공개 응답에 원본 경기 ID를 노출하지 않는다. seed 변경은 사용자가 `새 영수증 만들기`를 명시적으로 실행할 때만 허용한다.
...
13. 영수증 편집 화면은 로그인 여부와 관계없이 팀 이미지를 브라우저에서 결정론적 선화로 변환해 현재 영수증 미리보기와 저장 Canvas에 적용할 수 있다. 이 로컬 선화는 메모리에서만 유지하고 DB·object key·guest 업로드·draft·공개 스냅샷에 보존하지 않는다. 로그인 후 팀을 만들고 팀 상세의 `영수증 엠블럼 만들기`에서 저장한 canonical 팀 자산은 Team A·B별 `저장 엠블럼 사용`으로 다음 영수증에서도 재사용한다. 팀 상세는 공용 원형 크롭·자동 변환·확인 절차와 서버 검증을 거쳐 `teams.receipt_emblem_key`·`receipt_emblem_updated_at` 및 `team-emblems/{teamId}/` R2 key를 관리한다.
...
14. AI 선화 변환은 앱이 직접 호출하지 않는다. 사용자가 외부 도구에 붙여 넣을 프롬프트만 복사하며, 원형·비율·글자를 유지한 고대비 단색 선화와 완전 투명 alpha PNG를 우선 요구한다. alpha 출력이 불가능한 도구에만 균일한 순수 `#00FF00` 배경을 보조 조건으로 허용한다. 반환 이미지는 다시 로컬 기계 변환과 사용자 선택을 거쳐야 하며 자동 적용하지 않는다.
...
1. `/app/receipt?draft={publicId}`로 연 공개 영수증은 사진을 제외한 공개 payload의 팀명, 점수, 날짜, 경기 방식, 경기 성격, 장소, 코멘트, 대회명, 구간 점수를 영수증 입력 초안에 채운다. 수신자는 이 값을 로컬에서 편집할 수 있지만 원본 공개 draft는 변경하지 않는다.
