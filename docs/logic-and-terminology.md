...
13. 영수증 편집 화면의 즉석 팀 사진은 공용 원형 크롭 편집기에서 위치와 확대 비율을 정하고 최대 `320×320px`, `96KB` WebP로 자동 정규화한다. 사용자가 같은 팝업의 `선화로 변경`을 명시적으로 실행한 경우에만 브라우저 Canvas가 투명 금색 선화 후보를 만들며, 결과 확인 뒤 `확인`을 눌러야 현재 영수증에 적용한다. 이 즉석 파일과 변환 data URL은 서버에 저장하지 않는다. 팀장은 팀 상세의 `영수증 엠블럼 만들기`에서 같은 크롭·자동 변환·확인 절차를 거쳐 영수증 전용 선화를 팀 자산으로 저장·교체·삭제할 수 있다. 팀 상세는 자동 변환 단계에서 미리보기와 서버 전송용 WebP를 함께 확정하고, `확인`에서는 사용자가 본 결과를 재변환하지 않고 저장한다. 저장 실패 시 편집 팝업을 유지하고 권한·동시 변경·저장 경로 오류를 구체적으로 표시한다. 영수증 전용 자산은 일반 팀 대표 엠블럼과 분리된 `teams.receipt_emblem_key`·`receipt_emblem_updated_at`으로 관리하고, 서버가 내용 해시 기반 canonical `team-emblems/{teamId}/` R2 key를 생성한 뒤 팀장 권한과 비교 후 갱신으로 확정한다. 영수증은 저장본을 자동 적용하지 않고 사용자가 Team A·B별 `팀 엠블럼 불러오기`를 눌렀을 때만 사용한다. 해당 팀에 저장본이 있으면 영수증의 즉석 사진 업로드를 잠그며, 변경·삭제는 팀 상세에서만 한다. 임의 클라이언트 object key는 허용하지 않고 서버가 확인한 canonical `team-emblems/` key만 공개 스냅샷에 보존한다.
