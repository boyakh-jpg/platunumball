...
1. 운영 데이터, 사용자 값, 테마 색상, 경로, 화면별 예외값을 컴포넌트에 하드코딩하지 않는다. canonical DB/API, 공용 helper와 디자인 토큰을 사용한다.

## 제2원칙: 업로드 이미지 저장소 분리

1. 이미지 저장소는 파일마다 용량이나 월 사용량을 보고 바꾸지 않고 미디어 종류별 canonical 정책으로 고정한다.
2. 게시판 사진처럼 Supabase DB row의 생성·수정·탈퇴 수명주기에 결합된 사용자 콘텐츠는 Supabase Storage를 사용한다. 운영 권한, 소유권과 연결 row를 서버가 검증한 뒤 업로드·교체·삭제한다.
3. 팀 엠블럼 등 여러 화면에서 반복 조회하는 공개 이미지와 구장 등록 증거 등 서버만 읽는 비공개 이미지는 각각 Cloudflare R2 공개·비공개 bucket을 사용한다.
4. DB에는 공개 URL이나 Base64/blob 대신 object key와 해시·크기·형식·소유권 등 검증과 표시에 필요한 메타데이터만 저장한다. bucket·경로·사용량 기준은 공용 서버 정책과 환경 설정에서 관리하며 화면에 하드코딩하지 않는다.
5. 브라우저는 Supabase service role이나 R2 관리 token으로 직접 쓰지 않는다. 공개 URL 계산과 비공개 읽기, 업로드·삭제는 권한을 검사하는 서버 경계를 통과한다.
6. Supabase Storage 또는 전송량의 기본 경고 기준은 포함량의 70%, R2 이관 검토 기준은 85%다. 기준은 운영 설정으로 관리하며 자동으로 파일별 저장소를 전환하지 않는다.
7. 미디어 종류를 다른 저장소로 옮길 때는 전체 종류 단위로 복사, 해시 검증, DB key·provider 전환, 읽기 검증, 기존 객체 정리 순서를 따른다. 검증 전 기존 객체를 삭제하지 않는다.
8. 저장소에 포함해 배포하는 정적 UI 자산과 Discord 등 외부 서비스가 소유하는 원격 이미지는 이 업로드 저장 원칙의 대상이 아니다.

## 2026-08-06 프로필 운영자 예약어와 금지어

1. 영문 `boxtier`는 대소문자를 구분하지 않고, 한글 `박스티어`와 함께 닉네임·해시태그·호환 handle에서 단일 active owner 운영자만 사용할 수 있다.
2. 예약어와 금지어는 Unicode NFKC 정규화 후 문자·숫자 외 구분자를 제거한 값으로 비교한다. 띄어쓰기나 기호를 끼워 넣어도 우회할 수 없다.
...
## 2026-08-16 랜딩 게스트 진입

1. 비로그인 랜딩의 `로그인 없이 둘러보기`는 기존 공개 `/app` 경로로 이동한다. 별도 인증 세션이나 권한 우회 상태를 만들지 않는다.
2. 공개 홈에서 생성·참가·개인화 action을 실행할 때는 기존 인증 경계가 원래 경로를 보존해 로그인으로 연결한다.

## 2026-08-23 공개 API 과금 보호

1. `publicRead` API는 서버가 추출한 클라이언트 네트워크의 IPv4 `/24` 또는 IPv6 `/64` 구간을 비밀키 HMAC으로 익명화하고, Supabase 고정 시간창 RPC 하나로 공통 요청량을 제한한다. 원본 IP와 헤더값은 저장하지 않는다.
2. 시간창과 허용량은 `PUBLIC_API_RATE_LIMIT_WINDOW_SECONDS`, `PUBLIC_API_RATE_LIMIT_MAX_REQUESTS` 환경변수를 canonical 운영 설정으로 사용한다.
3. 분산 제한 저장소나 비밀키를 사용할 수 없으면 공개 API를 `503`으로 닫는다. 애플리케이션 제한 바깥의 대규모 요청은 Vercel Firewall과 Spend Management가 맡는다.

## 2026-08-05 공개 메인 인증 CTA

1. 공개 메인(`/`)은 실제 인증 상태를 기준으로 비로그인 사용자에게 로그인만, 로그인 사용자에게 경기 찾기와 홈만 표시한다. 같은 화면에서 로그인과 홈을 동시에 노출하지 않는다.
2. 공개 메인의 열린 매칭과 최근 기록은 공개 서버 API가 `public` 범위의 최신 3개만 투영한다. 비공개 방, 참가자 ID, 방 상태, 규칙 원본, 메모와 증거는 응답하지 않는다.
3. 클라이언트 상태를 임시 fallback으로 사용할 때도 `open + public` 모집과 `confirmed + public` 경기만 표시한다.

## 2026-08-05 회원 탈퇴와 재가입 제한

1. 설정의 회원 탈퇴는 `탈퇴` 확인 입력 뒤 fresh 인증 서버 action으로만 실행한다. 테스트 계정은 탈퇴할 수 없다.
2. 탈퇴하면 개인 프로필·설정·Discord 연결·커뮤니티·즐겨찾기·알림·팀 소속·개인 요약 기록을 삭제하거나 익명화한다. 다른 참가자의 확정 경기 결과 정합성에 필요한 원본 경기 기록은 탈퇴 프로필 식별정보를 제거한 상태로 보존한다.
3. 활동 중인 팀의 팀장은 팀장 위임 또는 팀 삭제 전 탈퇴할 수 없다.
4. 연결된 모든 외부 로그인 계정은 원본 식별자를 저장하지 않는 provider+subject 해시 tombstone으로 탈퇴 후 7일 동안 프로필 재생성을 차단한다.
5. 신규 프로필 해시태그 본문은 3글자 이상이어야 한다. 추천값은 닉네임을 영문으로 변환한 본문을 우선 사용하고, 중복일 때만 임의 숫자 3자리를 붙인다. 기존 1~2글자 해시태그도 닉네임 기반 안전값으로 보정한다.

## 2026-08-05 익명 디렉터리 조회 경계

1. 비로그인 사용자는 `teams`, `team_members`, `affiliations` 원본 테이블을 직접 조회하지 않는다.
2. 공개 랜딩 통계와 최신 3개 공개 매칭·기록은 공개 서버 API의 최소 필드 투영을 사용하고, 공개 프로필은 민감 식별자를 제외한 `public_profiles` 투영만 사용한다.
3. 로그인 사용자의 기존 디렉터리 조회는 유지하며 쓰기 권한은 서버 action 경계를 따른다.

## 2026-08-05 커뮤니티 게시판

1. 로그인 사용자는 자유·질의·팀소개 게시글과 댓글·1단계 답글을 작성하고 자기 글과 댓글을 수정·소프트 삭제할 수 있다. 운영 레벨 `30` 이상은 공지 작성·고정, 사진 게시글 작성과 전체 글·댓글 삭제가 가능하다.
2. 게시글 추천은 사용자당 글마다 1회만 저장한다. 인기글은 최근 3일 안에 작성된 자유·질의·팀소개·사진 글 중 `추천 수 × 3 + 공개 댓글 수`가 높은 순으로 최대 5개를 표시하고 동점이면 최신 글을 우선한다.
3. 일반 사용자는 최근 24시간 기준 글 5개, 댓글·답글 합계 50개까지 작성할 수 있다. 운영자는 이 작성량 제한을 적용하지 않는다.
4. 차단한 사용자의 일반 글과 댓글은 보이지 않게 하되 운영 공지는 숨기지 않는다. 원댓글을 삭제하면 그 아래 답글도 함께 소프트 삭제한다.
5. 자유·질의·팀소개·공지는 텍스트만 지원한다. 사진 분류는 운영 레벨 `30` 이상만 WebP로 정규화된 이미지 1장을 첨부할 수 있다. 익명 글, 일반 사용자 이미지, 일반 파일, 댓글 추천, 2단계 이상 답글은 지원하지 않는다.
6. 작성 화면은 게시글·댓글의 서버 최대 길이를 넘긴 상태에서 등록 버튼을 활성화하지 않는다. 서버 검증은 그대로 최종 권위다.
7. 게시판 일반 목록과 프로필 커뮤니티 활동은 페이지당 30개를 조회한다. 프로필 활동의 글은 원문 게시글 URL로 이동한다.
8. 작성 게시글과 작성 댓글은 기본 공개다. 사용자는 두 범위를 각각 비공개로 바꿀 수 있고 본인은 항상 자기 활동을 본다. 타인 조회는 서버가 공개 설정을 다시 검사한다.
9. 게시판 작성자 카드의 대표팀은 `teamHistory` 공개가 허용되고 현재 팀 멤버십이 확인된 대표팀 한 팀만 응답에 포함한다.
10. 게시판 분류 control은 `공지 / 자유 / 질의 / 팀소개` 순서이며 공지를 기본으로 연다. 사진 분류는 게시판 탭에 노출하지 않되 기존 글과 서버 호환성은 유지한다. 글쓰기 분류는 현재 열린 게시판으로 고정하고 작성 폼에 분류 선택기를 두지 않는다. 공지는 운영 레벨 `30` 이상만 작성한다.
11. 조회수는 로그인 사용자와 익명 사용자 모두 게시글 상세 조회 시 한국 날짜 기준 식별자당 하루 1회 반영한다. 로그인 사용자는 프로필, 익명 사용자는 서버가 추출한 IPv4 `/24` 또는 IPv6 `/64` 네트워크 구간을 비밀키 HMAC한 식별자를 사용하며 원본 IP와 헤더값은 저장하지 않는다. 같은 네트워크 구간의 같은 날 반복 열기와 목록 노출은 조회수를 올리지 않는다.
12. 사진 원본은 클라이언트에서 최대 `1920px`, `640KB` WebP로 정규화하고 서버가 컨테이너·크기·운영 권한을 다시 검증한 뒤 전용 공개 Storage bucket에 저장한다. 게시글 삭제·사진 교체·회원 탈퇴 시 연결된 객체를 정리한다.

## 2026-08-05 공개 경쟁 개인전 MMR 균형

1. `public + ranked + hostJoinMode=player + 경기 전 구성`인 개인방만 모집 단계의 MMR 자동 균형을 사용한다. 친선전, 비공개방, 팀전, 현장 픽업은 기존 배치 규칙을 유지한다.
2. 출전 명단은 A/B 평균 MMR 차이와 각 사이드의 `최고 MMR - 최저 MMR` 폭을 함께 검사한다. 두 값 모두 방의 `narrow=120`, `normal=220`, `wide=360` 허용값 이하여야 한다.
3. 새 참가자는 가능한 출전 사이드 중 인원 차이, 규칙 초과량, 평균 차이, 사이드 폭이 작은 곳에 자동 배치한다. 출전 배치가 모두 균형을 악화시키면 빈 후보 슬롯에 배치하고 후보도 차면 참가를 거부한다.
4. 방장 이동, 선수 본인 이동, 후보 전환과 경기 확정도 같은 서버 규칙을 사용한다. 과거부터 불균형한 방은 총 초과량을 늘리지 않는 이동만 허용해 단계적으로 복구할 수 있다.

## 2026-08-03 목록 카드 구장·일정 원본

1. 경기·모집 목록 카드의 구장명은 `courtId`가 있으면 승인 구장 원본을 우선하고, 얇은 캐시의 누락값이나 `미정` 문자열이 이를 덮지 않는다.
2. 목록 일정은 `scheduledDate`·`scheduledTime`·`scheduledAt`만 표시하며, 일정 누락 시 방 생성일을 경기일로 대체하지 않는다.

## 2026-08-03 라이벌 매치·프로필 완료·팀 통계

1. 시즌 라이벌 매치 만들기는 내 팀을 A팀으로 지정한 비공개 팀방을 만든 뒤 상대 B팀의 현재 주장에게 팀 초대를 자동 생성한다. 생성 화면 진입 시 남아 있는 일반 생성 초안보다 라이벌 A/B팀과 비공개 팀방 기본값을 우선하며, 팀 ID는 생성 URL에도 보존하고 팀 디렉터리 재수화 중 지우거나 개인전으로 낮추지 않아 새로고침 뒤 같은 설정을 유지한다.
2. `profiles.onboarding_complete=true`가 가입정보 완료의 canonical 기준이다. 잠금 시각 누락은 기존 완료 사용자를 가입정보 화면으로 되돌리지 않으며 DB에서 비파괴 보정한다.
3. 팀 득실 통계는 현재 상세 경기와 팀 기록 archive를 경기 ID로 중복 제거해 합산한다. 전체 승·패·무, 득실 합계·평균, 최고 득점·최소 실점·최대 승리차를 같은 경기 범위에서 계산하고 화면에는 적재 범위를 `불러온 기록`으로 명시한다.
4. 개인 통계는 기본 공개이며 `privacy.statSummary=false`일 때만 다른 사용자에게 숨긴다.
5. 라이벌 매치는 A/B팀, 비공개, 팀전, 경기 전 구성을 생성 완료까지 강제한다. 선택 경기 인원만큼 양 팀에서 실제 출전 가능한 선수가 부족하면 MMR은 좁게→보통→넓게→제한 없음, 연령은 단일군→인접군→전체 순으로 필요한 만큼만 확장한다. 어느 조합으로도 인원을 채우지 못하는 경기 방식은 선택과 생성을 막는다.
6. 라이벌 방 저장 뒤 A팀을 먼저 고정하고 B팀장에게 팀 초대를 생성한다. 어느 단계든 실패하면 불완전한 방을 닫고 성공으로 이동하지 않는다.
6-1. 생성 단계의 `다음`은 단계만 이동한다. 방·경기·기록 저장은 마지막 단계의 명시적 생성 버튼으로만 시작한다.
6-2. 비공개 팀방의 `targetTeamId`는 B팀 참가 확정 전에도 선택된 상대 팀이다. 팀장 초대가 대기 중이면 방 모달은 해당 팀과 초대 대상 팀장을 B주장 `초대 대기` 슬롯으로 표시하고 B팀 검색을 다시 열지 않는다. 대기 슬롯은 수락 전 실제 참가 인원과 정원 충족 수에는 포함하지 않는다.
...
## 2026-08-03 운영 시뮬레이션 잔여 데이터

1. 중단된 운영 시뮬레이션의 `sim_trn_*` 대회와 연결 경기는 검증된 정확한 ID만 기존 exact-cleanup RPC로 정리한다. 일반 경기·대회 조건 삭제나 화면 숨김으로 대체하지 않는다.

## 2026-08-03 관리자 권한 조회 실패 경계

1. 서버가 명시적으로 `admin_required`를 반환한 경우만 관리자 권한 없음으로 처리한다. 네트워크·서버 오류는 권한 없음으로 간주하지 않고 같은 화면에서 재시도한다.

## 2026-08-03 확정 픽업 명단 투영

1. 확정된 픽업 경기방은 배정 상태 문자열과 무관하게 경기의 현재 출전·후보 명단을 canonical 명단으로 사용한다. 모집 당시 파티 스냅샷을 다시 합쳐 같은 선수를 중복 표시하지 않는다.
2. 출전↔후보 이동은 참가 관계를 해제하지 않는다. 일정·방 목록 참가 인원은 출전자와 후보를 중복 없이 함께 센다.
3. 일정 피드는 `match_players` 출전자와 `reserve_players` 후보를 함께 세고, 후보의 일정 참가 관계를 유지한다.

## 2026-08-01 내부 DB helper 권한

1. canonical RPC가 호출하는 이전 단계 helper는 DB 소유자만 실행한다. `service_role`을 포함한 외부 역할은 helper를 직접 호출하지 못하며 grant health가 이 상태를 감시한다.

## 2026-08-01 시즌 순위 canonical 기준

1. 운영 시즌 개인·팀 승격 순위는 브라우저에 적재된 일부 디렉터리나 경기 목록으로 계산하지 않는다. 활성 `match_record_archives`의 시즌 날짜 범위와 `match_record_participants`·`match_record_teams` 전체를 서버에서 집계한다.
2. 0경기 플레이어·팀도 현재 MMR 기준으로 순위 대상에 포함한다. 개인 `seasonScore`는 `통합 MMR + 승*12 - 패*6 + 시즌 통합 MMR 변화`, 팀은 `팀 MMR + 승*16 - 패*8 + 시즌 팀 MMR 변화`를 유지한다.
3. 서버 시즌 순위는 현재 사용자의 차단 목록을 개인 결과에 적용하고 지역 순위 공개 설정을 함께 반환한다. 원본 경기·개인 기록 상세는 노출하지 않는다.
4. Supabase가 없는 로컬·데모 환경은 같은 시즌 helper와 현재 demo state로 순위를 계산한다. 서버 API 실패 화면을 만들지 않는다.
5. 운영 시즌 순위의 원격 조회가 진행 중이거나 실패하면 이미 가진 로컬 순위 스냅샷을 빈 목록으로 덮지 않는다. 실패 안내와 같은 조회의 재시도를 제공하고, 성공 응답이 도착한 뒤에만 canonical 순위로 교체한다.

## 2026-08-01 인증 저장소·프로필 이름 경계

1. 브라우저 저장소 읽기·정리·쓰기는 각각 실패할 수 있다. 저장소 접근 실패는 인증 초기화를 중단시키지 않고 메모리 세션과 로그인 화면을 계속 사용하게 한다.
2. 프로필 닉네임은 앞뒤 공백을 제거한 1~20자 문자열이어야 한다. 화면, 로컬 reducer, 서버 저장 경계가 같은 정규화 규칙을 사용하며 빈 이름은 저장하지 않는다.
3. 프로필 경기기록 조회 실패는 로딩으로 남기지 않고 실패 안내와 같은 조회의 재시도 동작을 제공한다.
4. 계정 전환 직후 이전 계정 프로필이 남아 있으면 가입정보 여부를 판정하지 않는다. 현재 인증 사용자와 같은 `authUserId`의 프로필 hydration이 끝난 뒤에만 가입정보 화면 또는 홈을 결정한다.
5. 심판 없는 경기시계는 별도 심판 기록 점수가 없을 때 저장된 경기 점수를 표시한다. `null` 심판 점수를 숫자 `0`으로 간주해 실제 점수를 덮지 않는다.
6. 즐겨찾기 추가는 현재 유효한 대상을 검사한다. 해제는 팀 삭제·구장 비활성·심판 자격 회수 뒤에도 기존 저장 행을 제거할 수 있어야 한다.
...
1. 홈의 최근 전적은 초기 홈 피드 유무와 무관하게 현재 사용자 기록 API의 canonical 상세를 한 번 병합한 뒤 표시한다.

## 2026-07-31 관리자 비동기 처리 대상 고정

1. 플레이어 신고·경기 심사·팀/소속·사용자 운영 조치가 진행 중이면 목록, 검색, 필터, 처리 대상 입력을 잠근다.
2. 조치 응답은 요청을 시작한 신고 ID 또는 사용자 ID와 현재 선택 대상이 같을 때만 현재 상세 화면에 표시한다. 다른 대상으로 이동한 뒤 이전 응답을 현재 대상의 성공·실패처럼 표시하지 않는다.
3. 관리자 임명 회수·연장은 현재 활성 목록에 실제로 존재하는 임명 ID만 전송한다. 갱신으로 사라진 이전 ID는 첫 활성 임명으로 정규화한다.
4. 권한관리 대기 건수는 현재 페이지 행수가 아니라 관리자·심판 임명과 심판 신청의 서버 전체 대기 건수를 합산한다.

## 2026-07-31 계정·설정 변경 직렬화

1. 관리자 임명 검색은 검색어가 선택한 사용자 표시값과 달라지는 즉시 선택 사용자 ID를 비운다. 화면의 검색어와 실제 처리 대상이 달라진 상태에서는 임명할 수 없다.
2. 즐겨찾기 변경은 대상 종류와 대상 ID별로 한 번씩 처리한다. 같은 대상의 저장이 끝나기 전 연속 입력은 추가 토글로 처리하지 않는다.
3. 프로필, 가입 프로필, 일반 설정, Discord 연결·해제, 구장 검색·핀·등록요청, 알림 초대 응답은 저장 중 중복 실행을 막고 서버 실패를 성공으로 표시하지 않는다. 가입 프로필 저장과 기존 계정 복구를 위한 새 계정 해제는 동시에 실행하지 않는다. 저장 성공 뒤에는 최신 프로필 반영과 화면 이동까지 잠금을 유지하며, 실패하면 입력을 보존하고 재시도를 허용한다.
...
7. 알림 화면의 최신·이전 알림 조회, 팀 정보 조회와 팀 초대 수락 뒤 팀 상세 조회는 명시적인 `false`도 실패로 처리한다. 이전 알림 조회 실패는 기존 목록과 다음 커서를 보존하고 같은 추가 조회를 재시도한다. 최신 팀 정보를 못 받으면 팀 상세로 이동하지 않는다.

## 2026-07-31 팀 상세 원격 권한·삭제

1. 링크로 전달한 팀 preview는 첫 표시용일 뿐 관리 권한의 근거가 아니다. authoritative 팀 상세 조회가 성공하고 전체 명단임을 확인한 뒤에만 팀장 관리 action을 연다.
2. 팀 상세 조회 실패 시 preview가 있으면 표시를 유지하고 인라인 재시도를 제공한다. preview가 없으면 실패 화면에서 재시도한다.
3. 팀 삭제는 서버 성공을 확인한 뒤 로컬 팀을 제거하고 결과 Promise를 호출자에게 반환한다. 서버 실패를 성공처럼 닫거나 이동하지 않는다.

## 2026-07-31 팀 슬롯 이동 정원

1. 팀 파티의 출전·후보 이동도 개인 슬롯과 같은 사이드 정원 검사를 사용한다.
2. 대상 출전 또는 후보 정원이 찼으면 이동 action을 실행하지 않고 현재 배치를 유지한다. 다른 선수를 자동 맞교환하지 않는다.
3. 팀 주장은 후보가 될 수 있지만 출전·후보 명단에서 모두 해제할 수는 없다. 전체 명단 편집도 현재 주장 배치를 강제로 바꾸지 않는다.
4. 로컬 reducer와 서버 reducer는 출전자가 없는 파티도 `playerIds=[]`와 `partyReserves`로 동일하게 표현한다. 주장이 후보에 남아 있으면 파티를 유지한다.

## 2026-07-31 방 참가자 관리 직렬화

1. 참가자 새로고침·출석·출전/후보·사이드 이동·슬롯 교환·강퇴·실시간 교체는 현재 요청이 끝나기 전 같은 패널의 다음 명령을 실행하지 않는다.
2. 서버가 `false`, `{ok:false}`를 반환하거나 예외가 발생하면 현재 명단과 선택을 유지하고 실패로 표시한다. 강퇴 확인창과 슬롯 교환 선택은 성공한 뒤에만 닫는다.
3. 같은 슬롯 팝오버의 개별 출전·후보 이동과 전체 명단 저장은 하나의 pending lock을 공유한다.

## 2026-07-31 운영 변경 실패 처리

1. 차단·차단 해제, 심판 등록요청, 관리자 임명·연장·회수, 팀 변경은 서버 저장 성공을 확인한 뒤 완료로 표시한다.
2. 서버 저장 실패 시 입력을 지우거나 성공처럼 표시하지 않으며, 낙관 반영한 상태는 이전 상태로 되돌린다. 방 채팅 초안은 전송 성공 뒤에만 초기화한다. 팀 생성 실패는 action이 반환한 사용자용 사유를 표시하고, 사유가 없을 때만 기본 오류 문구를 쓴다. 모집방 참가·파티 합류는 `false`, `ok: false`, 예외를 실패로 처리하고 선택을 보존한 채 현재 참가 영역에서 오류와 재시도를 제공한다. 경기 생성은 제출 직후 인증 확인부터 저장 완료까지 하나의 요청만 실행하고, 인증 확인 실패도 입력을 보존한 채 재시도할 수 있어야 한다.
...
13. 일반 경기의 수동 승인이 없으면 `match_results.final_submitted_at`과 경기 종료 중 늦은 시각부터 전체 `disputeMinutes`가 지난 뒤 유효한 결과, 열린 이의 0건, 심판 경기의 실제 출전자 개인 스탯 완성을 확인해 `system`이 자동 확정할 수 있다. 최종 제출된 미확정 경기는 시간이 지나도 이의·진행 업무에 남고 실제 `confirmed` 전에는 기록 단계로 전환하지 않는다. 개인 스탯 누락으로 확정되지 못한 경기는 자격이 유효한 배정 심판이 기간 후에도 누락을 보완할 수 있으며, 완성된 기록의 편집 기간은 연장하지 않는다. 자동 확정은 0 개인기록을 보충하지 않으며 수동 확정과 같은 lock·멱등 MMR transaction을 사용한다.
...
18. 확정 경기의 기록 참가자는 경기 종료 시점 슬롯이 아니라 `playedPlayerIds` 전체를 기준으로 보존한다. 교체되어 후보로 종료한 실제 출전자도 내 기록·비공개 기록 열람·심판 스탯에서 누락하지 않는다.
19. 개인 스탯 표준 필드는 PTS·REB·AST·STL·BLK·TO·F다. TO는 공격권을 상대에게 넘긴 턴오버이며, 공식 통계와 `personal_record` 통계 summary에서 누적값을 보존한다. 무심판 일반 경기와 `match_record`에는 개인 스탯을 생성하지 않는다.
20. 프로필 내 기록 통계는 `공식기록`, `무심판`, `사후기록(match_record)`, `내 기록(personal_record)` 출처를 분리한다. 공통 점수 통계는 각 출처의 확정 결과만 집계하고, 개인 PTS·REB·AST·STL·BLK·TO·F 누적·경기당 평균은 심판·대회 검증 기록과 본인 `personal_record`에만 표시한다. 경기장별 개인 평균은 해당 경기장의 심판·대회 검증 기록만 사용한다.
...
1. `관리`는 개인 프로필(`/app/profile`)과 팀 공간(`/app/teams`)의 공용 상위 메뉴다. 두 화면과 하위 상세 화면에서 히어로 아래의 `내 프로필 / 내 팀`으로 바로 전환한다. 팀을 플레이에 포함하지 않으며, 기존 팀 소속·초대·관리·탐색 권한과 데이터 조회 범위는 유지한다.
2. 설정(`/app/settings`)과 알림(`/app/notifications`)은 공용 상단에서 직접 연다. 게스트는 기존 `getLoginPath`로 선택한 도구의 복귀 경로를 전달하고 로그인 후 해당 화면으로 돌아온다. 메뉴 이동만으로 인증·관리자 권한을 부여하지 않는다. 사이트 관리자 진입은 `사이트 관리`로 표시하며 프로필 보조 목록의 기존 서버 권한 기준을 유지한다.

## 2026-09-04 운영 업무함

1. `일정 → 내가 운영`(`/app/matches?panel=operations`)은 전용 서버 조회 scope에서 `matches.created_by` 또는 `matches.referee_id`가 현재 인증 사용자와 일치하는 canonical 경기만 받는다. 기존 `/app/operations` 링크는 검색·필터·hash·복귀 상태를 보존해 이 보기로 이동한다. 개인 일정·feed 관계값·참가자 여부·과거 심판·관리자 레벨로 운영 대상을 늘리지 않으며, 동일 경기에서는 배정 심판 역할을 우선 표시한다. 반환 경기의 `recruitingPostId`가 참조하는 읽기 허용 모집글만 함께 반환하며, 개인 일정 scope의 의미는 바꾸지 않는다.
2. 업무함은 새 phase·권한·저장 경로를 만들지 않는다. 경기 선택은 검색어(`q`)와 구간(`filter`)을 포함한 현재 경로를 복귀 경로로 보존해 공용 `MatchRoomModal`을 연다. 단계별 버튼은 할 일을 안내하며 실제 action은 기존 phase helper와 서버 권한 검증을 그대로 사용한다. 진행 중인 경기의 기록 안내는 배정 심판에게만 표시한다.
3. 전용 scope는 `closed`와 개인·사후 기록방을 제외하고 `waiting / locked / checkin / live / postgame / dispute / record / cancelled / void` phase를 함께 반환한다. 서버는 미확정·진행 경기를 종료 경기보다 먼저 제한 조회하고 cursor로 나머지 경기를 이어서 제공한다. 업무함은 이를 `지금 / 다음 / 지난`으로 분류하고, 구간 선택이 없으면 이 순서에서 경기가 있는 첫 구간을 연다. 검색은 읽기 허용된 현재 목록의 경기명·양 팀명·장소에만 적용하며 공백·대소문자를 구분하지 않는다. 로딩·실패·빈 상태를 분리하고 실패를 빈 목록으로 숨기지 않는다. 추가 조회 실패 시 기존 목록과 cursor를 보존하며 새로고침은 첫 페이지부터 다시 조회한다.
...
5. `내가 운영`은 현재 경기 상태에서 할 일을 보여준다. 알림은 초대·승인·경기 변경처럼 발생한 소식과 읽음 상태를 보여주며 공용 상단 종 아이콘에서 연다. 메뉴 통합 때문에 알림을 운영 목록으로 바꾸거나 알림 읽음으로 경기 업무를 완료하지 않는다.
...
3. 필수 심판 승인과 모든 팀장 승인이 끝나기 전에는 대진·경기를 생성하지 않는다. 팀장 승인만으로 대회를 자동 시작하는 구형 흐름은 `governanceVersion=2` 대회에서 사용하지 않는다. 대회 목록도 두 조건을 모두 충족해야 `참가 승인 완료`로 표시하고, 부족한 단계는 `팀 승인 대기` 또는 `심판 승인 대기`로 구분한다.
...
7. 실제 경기는 승인된 대회 심판 중 양 팀과 무관한 심판을 1명 배정한다. 같은 심판의 예상 경기 구간이 겹치는 배정을 금지하고, 심판 미배정·자격 만료·소속 충돌 시 일정 저장과 경기 시작을 DB에서 거부한다. 일정·심판 작업은 같은 공용 오류 안내를 사용하며 미배정·소속 충돌·시간 중복의 원인과 수정 행동을 표시한다.
...
5. 대회 경기의 출전 명단 구성 권한은 경기 `confirmedAt` 유무가 아니라 일정 존재, 경기 생명주기, 해당 팀장 여부로 판정한다. 현재 팀 directory가 부분 로드됐으면 대회 생성 시점의 `teamRosterSnapshot`으로 팀장과 팀원을 보강한다.
...
6. QR 예정 경기는 전원 출석 시 예정시각 20분 전부터 시작할 수 있고, 미출석자가 있으면 예정시각 전 시작을 막는다. 대회 경기는 양 팀 명단 확정이 추가로 필요하며 예정시간 경과나 전원 출석으로 이를 우회할 수 없다. QR 미사용 예정 경기는 기존 10분 전 시작 정책을 유지한다. 늦은 시작은 별도 마감으로 취소하지 않되, 동일 심판의 다른 live 경기가 있으면 시작을 막는다.
...
8. 공개 팀전은 현재 소속 팀원 1명이 대표로 참가해 해당 주장이 된 뒤 출전·후보 명단을 직접 확정한다. 참가 시 정원 전체를 강제하거나 팀원별 pending 초대 row를 만드는 구형 경로는 사용하지 않는다.
...
13. 공개 팀전의 상대 application은 소속 팀원 대표 1명 row를 DB 참가 trigger에서도 허용한다. 비공개 지목 초대는 상대 팀의 현재 팀장에게 연락용 초대 1건을 보낸다. 팀 자체의 참가 가능 인원 검사는 유지하고 실제 출전 정원은 참가 뒤 주장의 출전·후보 명단 확정 단계에서 강제한다.

## 2026-07-24 공개 매칭·대회 QR 출석·현장 교체

1. 사후 경기기록(`match_record`)과 개인 기록(`solo`, `personal_record`) 외 모든 실제 경기방은 QR 출석을 필수 사용하며 선택 switch를 제공하지 않는다. 대회는 배정 심판만 QR을 표시하고 출석판을 운영한다.
...
1. 공개 모집, 비공개 초대, 대회, 개인 기록 생성은 같은 단계형 탐색을 사용한다. 공개·비공개 경기는 `기본 설정 → 규칙·운영 → 구장·비용`의 3단계다. 경기 목적, 인원, 팀, 일정은 기본 설정에 포함하고 운영·취소 정책은 규칙 단계에 포함한다. 다음 버튼은 이동할 단계 이름을 표시하고 실제 단계 이동 때만 새 입력 영역으로 포커스와 스크롤을 이동한다. 단계 이동이나 같은 단계의 조건 변경으로 작성 중인 값을 초기화하지 않는다. 별도 확인 단계는 두지 않고 마지막 입력 단계 아래에서 최종 요약·오류·주의를 확인한 뒤 생성한다. 경기 기록방은 입력량이 적으므로 단일 화면을 사용하며, 모든 유형은 비용·후보·MMR처럼 해당 유형에 없는 항목을 렌더링하거나 저장하지 않는다.
...
5. 모든 경기 방식의 기본 시간 구성은 `8분 × 2구간 러닝타임`이다. `8분 × 4쿼터 러닝타임`도 별도 프리셋으로 제공하며 5v5에서는 마지막 2분을 스톱타임으로 운영한다. `10분 × 4쿼터 스톱타임`은 경기 지위로 오해하지 않도록 별도 시간 프리셋으로 표시한다. 적용된 규칙은 공용 formatter로 먼저 요약하고, 프리셋의 세부 규칙을 펼치거나 접는 동작은 저장값에 영향을 주지 않는다. `직접 설정`과 입력 오류에서는 세부 입력을 표시하고 수정 중인 원본 숫자 값을 유지한다. 1v1·2v2·3v3의 종료 기준과 승리 조건은 해당 방식의 공용 기본 규칙을 유지한다.
...
3. 홈페이지 초기 HTML은 앱 목적, 외부 로그인 데이터 항목과 사용 목적, `/privacy` 링크를 포함한다. 브라우저용 React 랜딩과 의미가 달라지면 안 된다.
...
1. 팀전의 이미 확정된 A/B 팀 엔트리에서는 해당 주장이 출전·후보 명단을 저장하는 순간 팀원이 즉시 로스터에 들어간다. 팀원별 수락은 요구하지 않는다.
...
2-1-1. Supabase가 없는 로컬·데모 환경도 기본 지급 아이콘 목록을 성공 응답으로 제공하고, 같은 allowlist·색상 검증을 거쳐 현재 demo state에 아이콘 설정을 저장한다.
...
2-3. `06~20`은 순서대로 팀 가입 1회, 확정 경기 3·5·10·15회, 3승, 팀장, 수락된 초대 5회, 매칭 성사 10회, 비공개 경기 10회, 랭크 경기 10회, 2점 차 이내 승리 3회, 심판·기록원 수행 5회, 팀 경기 승리 10회, 경기 10회와 신뢰도 90을 조건으로 한다.
...
2-5. `51~65`는 순서대로 첫 승, 3연승, 2점 차 이내 승리, 야간 경기 10회, 다섯 포지션 모두 출전, 경기 50회, 공인 경기 50회, 팀 경기 10회, 수락된 초대 25회, 심판 5회, 기록원 5회, 구장 후기·승인 제보 3회, 팀장, 경기 100회와 신뢰도 95, 경기 300회와 신뢰도 98을 조건으로 한다.
...
9. 팀 사진 내용은 자동 판별하지 않고 형식·용량·변 크기만 검사한다. 현재 표시 중인 `upload` 엠블럼만 다른 인증 사용자가 신고할 수 있으며 팀장은 자기 팀 엠블럼을 신고할 수 없다.
10. 팀 엠블럼 신고는 `reports.type='team_emblem'`으로 저장하고 서버가 현재 팀명, 팀장, object key, 변경 시각을 접수 snapshot으로 강제한다. 같은 신고자의 같은 미종결 팀 신고는 중복 생성하지 않는다. 처리 직전 snapshot key와 현재 key를 같은 DB transaction에서 잠금 비교하며 달라졌으면 오래된 신고로 거부한다.
...
6. 팀 생성은 서버 저장 성공 뒤에만 폼을 초기화하고 반환된 팀 ID의 상세 관리 구역(`#team-control`)으로 이동한다. reducer 또는 서버가 거절하면 입력값과 선택 구장을 유지하고 실패 사유를 표시한다.
...
| 팀 | 생성·수정·soft delete, 팀장 초대와 선수 가입 신청·승인·거절·취소, 1인 3팀·팀당 30명 | `team_membership_invite_decline`, `team_lifecycle` |
...
3. 공개/비공개 팀방은 빈 상태로 만든다. A팀 선택은 방장만 수행하며 방장이 선택 팀의 현재 소속 선수이고 참가 조건을 충족해야 한다. 공개 B팀은 현재 소속된 참가 가능 팀원이 대표로 참가한다. 각 대표는 해당 경기의 주장이 된다. 팀장은 팀 자체 관리 권한일 뿐 경기 운영 권한의 자동 기준이 아니다.
...
- 공개 B사이드는 상대 팀의 현재 소속·참가 가능 팀원 1명이 대표로 참가해 B주장이 된다. 팀원별 pending 초대나 생성 시점 로스터를 만들지 않는다.
...
- A팀을 선택한 owner는 해당 경기의 A주장이 된다. 출전/후보 명단 확정은 팀장이 아니라 방모달의 주장/팀 소집 경로에서 처리한다.
...
## 2026-07-07 기록방 주장 로스터 저장 권한

- `match_record` 로스터 수정은 방장/심판 전용 액션이 아니다.
- 각 주장은 자기 사이드 로스터만 저장할 수 있다.
...
- 기본 `/app/create`는 사전 방만들기만 노출하고, 기록 생성 분기는 `/app/create?intent=record`에서만 노출한다. `/app/create?intent=tournament`은 사전 방만들기의 기존 대회 preset을 선택하며 참가·개최 자격과 생성 검증은 동일하게 적용한다.
...
- 각 주장은 방모달에서 팀을 고르고, 그 팀의 선수 명단에서 `출전/후보/제외`를 선택한 뒤 `선수 확정`을 눌러 저장한다.
...
- 모집방 공유 URL은 `/app/recruiting?post=...`, 경기방 공유 URL은 `/app/matches?match=...`를 canonical deep link로 사용한다. 경기방은 연결된 모집글 유무와 관계없이 실제 경기 ID를 공유한다.
...
- `recordEntryMode="quick"`은 현재 사용자 외 방식별 출전 정원을 무기명 선수로 채우며 상대·팀·선수 연결을 만들지 않는다.
- `recordEntryMode="named"`은 팀명과 우리팀/상대팀 선수명을 자유 입력으로 저장하되 프로필 승인이나 계정 연결을 만들지 않는다. 입력 선수 배치 후 남은 출전 정원은 무기명 선수로 채운다.
- 개인 기록은 `1v1`, `2v2`, `3v3`, `4v4`, `5v5`를 허용한다.
- 개인 기록의 프리텍스트 선수는 방식 정원을 초과할 수 없고, 같은 선수가 우리/상대 또는 같은 사이드에 중복으로 들어갈 수 없다.
- 개인 기록 선수명 입력은 이름만 또는 `이름 포지션`으로 자유 입력할 수 있고, 실제 유저의 이름·해시태그 검색은 선택 사항이다. 저장은 프로필 ID 연결 없이 무기명 선수로만 한다.
...
- 비공개 팀전 확정 조건은 A사이드 owner와 B사이드 초대 수락자가 각 팀의 현재 소속·참가 조건을 충족하는 것이다. A/B 출전·후보 명단은 방 안에서 각 주장이 확정한다.
...
- 출석 단계의 개인 참가자 사이드·후보 이동은 기존 배정을 제거한 뒤 대상 사이드에 원자적으로 반영한다. `teamId`만 있는 1인 참가자는 잠그지 않고, 실제 팀 파티가 2명 이상일 때만 다른 사이드 이동을 막는다.
...
| 방장 | 방 생성자. 공개방에서는 노란 왕관 | 팀장과 다름 |
| 파티장 | 해당 파티 대표. 팀전 B사이드 대표 등 | 방장 권한과 다름 |
| 주장 | 해당 사이드의 운영 대표. 비공개 팀전은 초대 수락자, 공개/비공개 개인전은 현재 사이드 첫 리더 | 팀장과 다름 |
| 팀장 | 실제 팀 관리 권한자 | 방 모달 권한과 직접 연결하지 않기 |
...
1. `사이트 관리`는 권한자만 프로필 보조 링크·설정 진입 카드와 `/app/admin` 화면을 볼 수 있다. 개인·팀의 상위 `관리` 메뉴와 구분한다.
...
| `getRecruitingLobby` | `src/lib/recruiting.js` | 방 슬롯, 사이드, 후보, 자동 출전 전환, 확정 가능 여부 |
...
| 진행 | `/app/recorder` | live/postgame/dispute 중 처리 가능한 경기 | 미제출 종료 경기는 기록 입력창 만료 시, 제출 경기는 이의 종료 시 숨김 |
| 관리 → 내 팀 | `/app/teams` | 내 팀, 팀 관리, 팀 랭킹 | 팀은 실제 소속 단위만 |
| 관리 → 내 프로필 | `/app/profile` | 내 프로필, 티어, 기록 더보기 | 학교/회사는 단일 선택형 소속으로 통합하고 지역/연령은 별도 유지 |
...
9. 주소검색은 기존 브라우저 Naver Maps geocoder를 우선 사용한다. 브라우저 geocoder가 실패하거나 0건이면 `/api/courts/address-search`가 주소 geocoding을 재시도하고, 다시 0건일 때만 Naver 지역검색에서 건물명 후보를 최대 5개 반환한다.
10. 서버 주소검색 fallback은 `VITE_NAVER_MAP_CLIENT_ID` 또는 `NAVER_MAP_CLIENT_ID`와 서버 전용 `NAVER_MAP_CLIENT_SECRET`이 필요하다. 건물명 후보 검색은 별도 서버 전용 `NAVER_SEARCH_CLIENT_ID`, `NAVER_SEARCH_CLIENT_SECRET`을 사용한다.
...
| 후보 | 사이드별 최대 3명 | 자동 출전 전환/기록/교체 대상 |
...
6. B사이드 초대 수락자가 B사이드 파티장/주장이며, 수락 뒤 자기 팀 출전/후보 명단을 지정한다.
...
10. 팀 전용 공개방에서는 다른 팀 초대/참여를 아무나 할 수 없다.
   - 각 사이드를 점유한 팀 entry의 주장만 자기 팀원을 `팀원 소집`으로 출전/후보 명단에 넣을 수 있다.
...
1. 비공개 팀전 대기방에서는 B사이드 초대를 받은 사람이 수락하면 그 사람이 B주장이다.
2. 확정 경기에서는 `match.parties[].partyLeaderId`가 현재 로스터에 있으면 그 사람이 주장이다.
3. 명시적 파티장이 없으면 방장이 속한 사이드는 방장을 우선하고, 그 외 사이드는 첫 출전 선수와 첫 후보 순으로 주장을 정한다. 주장이 미출석 강퇴되면 같은 기준으로 즉시 승계한다.
4. 팀장은 주장 fallback 기준이 아니다.
5. 팀 파티의 주장은 “파티 만든 사람”이라는 표현을 쓰지 않고, 팀 파티를 등록한 계정 또는 초대 수락자로 본다.
...
겹치면 노란 왕관만 표시한다.
공개방/비공개방 모두 주장 기준이 필요하면 파란 왕관을 표시한다. 확정 경기 슬롯은 유효 주장 한 명만 표시하고 모집 엔트리의 파티장 왕관을 중복해 그리지 않는다. 방장이 속한 사이드는 별도 파란 왕관을 함께 그리지 않고 방장 노란 왕관만 표시한다.
팀장은 파란 왕관 기준이 아니다.
...
15. 합류 가능한 기존 파티가 있어도 빠른 파티 합류는 일반 개인·팀 참여 폼을 숨기지 않는다.
...
1. 후보는 사이드별 최대 3명.
2. 경기 확정 시 출전 슬롯이 비어 있고 READY 후보가 있으면 왼쪽 후보부터 자동 출전 전환.
3. 고정 후보는 자동 출전 전환하지 않는다.
4. 자동 출전 전환된 선수는 `promotedReserveIds`에 남긴다.
5. 후보가 자동 출전 전환되면 후보 슬롯은 비워진다.
...
11. 배정 심판이 경기준비방에 오지 않으면 방장이 `심판 미출석`을 요청하고 상대 주장이 인정해야 심판 없는 경기로 전환된다.
...
5-1. 심판 경기의 `player_stats` 이의는 같은 결과 revision에서 접수됐어도 현재 이의 draft에 선수별로 순차 병합한다. 무심판 `team_scores` 이의는 먼저 가결된 점수를 오래된 요청이 덮지 않도록 현재 결과 revision과 정확히 일치할 때만 가결한다.
...
10-1. 방장의 심판 미출석 요청은 `pending`만 저장하고 신뢰도를 바꾸지 않는다. pending 재요청은 막으며 상대 주장의 최초 확인 transaction에서만 신뢰도를 한 번 차감한다.
...
10. More 메뉴에는 별도 신고 항목을 두지 않으며, 일반 신고는 설정에서 시작한다. 방·경기·선수 프로필의 `신고하기`는 설정의 공용 신고 폼으로 연결한다. 설정 직접 진입은 사유부터 선택하고 문맥형 진입은 서버 제출 조건으로 다시 검증할 대상과 원본 경기를 미리 선택한다. 화면별 신고 폼을 따로 만들지 않는다.
11. 신고 사유를 바꿀 때 새 사유와 호환되는 대상·메모는 유지한다. 호환되지 않는 대상은 사용자 확인 뒤 대상만 초기화하며 메모를 말없이 지우지 않는다.
12. 내 신고 내역과 `검토 중` 집계는 현재 로그인 사용자의 신고만 대상으로 하며 `status=open`만 검토 중으로 센다. 접수 결과는 `public.reports`에 저장된 신고 번호, 접수 시각, canonical 상태와 상세 링크를 표시한다. 별도 SLA가 없으므로 예상 처리 시간은 표시하지 않는다.
13. 일반 신고에는 증거 업로드가 없다. 신고 row와 처리 이력의 canonical 저장소는 Supabase `public.reports`이며 브라우저가 신고 row나 R2에 직접 쓰지 않는다.
14. 주요 사유:
   - 나이 속임
   - 티어/MMR 어뷰징
   - 미출석/잠수
...
10. JPG, PNG, WebP, AVIF, HEIC/HEIF 원본은 브라우저가 해석할 수 있으면 최대 `320x320`, `96KB` 이하 정사각 WebP로 자동 변환한다. 고해상도 원본은 품질과 출력 크기를 단계적으로 낮춰 저장 가능한 결과를 만들고, 서버는 인증, 팀장 권한, WebP 시그니처, 픽셀 크기와 바이트 크기를 다시 검사한다. 서버는 RIFF 전체 길이와 청크 경계를 검사하고 이미지 청크 외 메타데이터·애니메이션·미지 청크·뒤에 붙은 데이터를 거절한다. 이 수치는 사용자 성공 메시지에는 노출하지 않는다.
...
23-1. 구장 리뷰 저장·수정·삭제 뒤 통계 캐시는 변경된 구장만 갱신한다. 리뷰 1건 때문에 전체 승인 구장 통계를 동기 재계산하지 않는다.
...
36. 구장신청 주소 입력은 주소 geocoding이 0건일 때만 Naver 지역검색의 건물명 후보를 조회한다. 후보의 건물명·주소·좌표는 지도 이동 기준으로만 사용하고, 최종 주소와 좌표는 사용자가 핀을 확정한 뒤 역지오코딩한 값으로 교체한다. 시설/장소명은 후보 건물명을 초기값으로 쓰되 직접 수정할 수 있으며 외부 카테고리·설명은 저장하지 않는다. RankBall 내부 500m 중복 후보 조회는 유지한다.
...
7. 후보 자동 출전 전환과 파티 유지가 충돌하지 않는가.
...
5. 후보 선수: 후보 수락 -> 자동 출전 전환 -> 출석 -> 기록 확인.
...
8. 같은 신고자가 같은 미종결 대상을 다시 신고하면 새 row를 만들지 않고 기존 `reportId`, `status`, `createdAt`을 반환한다. 새 접수도 같은 세 필드를 실제 저장값으로 반환한다.
...
2. 서버는 Supabase bearer를 검증하고 RPC가 현재 `profileId`가 해당 팀장인지 다시 확인한다.
3. 새 팀 생성은 현재 `profileId`가 팀장으로 포함된 경우만 허용한다.
...
3. 기존 토너먼트 수정은 생성자만 가능하다. 단, 팀 참가 승인은 해당 팀장도 `action=approveTeam`으로 저장할 수 있다.
4. operation 없는 fallback snapshot 경로에서 팀장의 `approveTeam`은 자기 팀 승인 상태와 승인자 정보만 바꿀 수 있고, 대회 핵심 설정, 팀 목록, 상태, bracket, matchIds는 바꿀 수 없다.
...
13. 같은 심판 시험의 병렬 시작은 최초 저장된 문제지만 유지하고, 병렬 채점은 `started` 상태를 최초로 종료한 결과만 확정한다. 늦게 도착한 요청은 이미 확정된 결과를 그대로 반환하며 문제지·합격 결과를 덮어쓰지 않는다.
14. 심판 등록요청은 사용자당 `pending` 1건만 유지한다. 병렬 제출과 같은 ID 재제출은 최초 요청을 반환하며 자격 유형·시험 근거를 덮어쓰지 않는다.
...
14. 테스트 계정 진입은 서버가 발급한 일회용 magic link token hash만 사용한다. 레벨 `100` 운영자가 설정 전환을 시작한 탭은 원래의 서명된 Supabase 세션을 탭 범위에 보관해 동일 Auth user로만 복귀하며, 복귀 정보가 없거나 만료되면 일반 OAuth 로그인을 다시 사용한다.
...
2. `!refereeId`이면 공개/비공개 모두 방 모달에 심판 초대 슬롯을 보여준다. `refereeWanted`는 공개방 심판 직접참여 허용 여부에만 사용한다.
...
6. 개인방에 개인 참가와 팀 파티가 섞여도 확정 경기의 A/B 전체를 파티 팀으로 승격하지 않는다. 사이드 `teamId`는 비우고 실제 팀 파티 참가자의 `playerTeams`와 `parties` 출처만 유지한다.
...
2. 실제 외부 로그인 계정과 테스트 계정은 Supabase Auth token으로 서버에서 현재 `profiles.id`를 확정한다.
...
45. 인증 사용자의 `/app/teams`와 `/app/teams/:teamId` 초기 로드는 `/api/teams/list` 또는 `/api/teams/detail`을 사용해 팀, 팀원, 관련 공개 프로필, 현재 설정, 현재 사용자 팀 초대만 읽는다. 팀 화면 첫 렌더는 넓은 `/api/directory/load`에 의존하면 안 된다.
...
6. 한 팀의 등록 인원은 최대 30명이다. 정규멤버와 용병을 합친 운영 단위로 보며 프론트, 서버 action, DB RPC/trigger가 모두 차단한다.
7. 팀 가입은 `팀장 초대 → 대상자 수락`과 `선수 가입 신청 → 팀장 승인`을 함께 제공한다. 같은 팀·대상자의 pending row는 하나만 허용한다. 기존 초대가 있으면 가입 신청은 초대 상태를 반환하고, 기존 가입 신청이 있으면 팀장 초대가 같은 row를 초대로 전환한다. 팀 정원 30명 또는 사용자 3팀 도달 시 승인 transaction에서 다시 확인하고 해당 pending row를 `expired`로 종료한다.
7-1. 초대 수락과 가입 승인 모두 `팀 → 대상 사용자 → pending row` 순서로 잠근다. 여러 팀장이 같은 사용자의 가입을 동시에 승인해도 사용자별 최대 3팀을 넘길 수 없고, 한 팀의 동시 승인도 30명을 넘길 수 없다.
7-2. 가입 신청 버튼은 현재 사용자 소속 팀 수, 대상 팀 정원, 기존 멤버·pending 신청·초대를 먼저 확인한 뒤 신청서를 연다. 신청 제출 때 서버 RPC가 같은 제한을 transaction lock 안에서 다시 확인한다.
7-3. 가입 신청서의 SNS, 연락처, 키, 포지션, 경기 가능 시간, 연령대, 성별은 모두 선택 입력이며 해당 신청 row에만 저장한다. 미입력은 자동 거절 사유가 아니며 팀장이 신청서를 확인한 뒤 승인 또는 거절한다. 신청자와 해당 팀장만 신청서를 읽는다. 저장 실패는 신청 모달 내부에 표시하고 작성값을 유지해 같은 모달에서 다시 시도할 수 있어야 한다.
...
5. 팀원 초대는 현재 검색 결과에서 사용자가 직접 선택한 선수만 발송 대상으로 삼는다. 검색어 변경, 결과 없음, 발송 성공 뒤에는 이전 선택을 지우며 첫 후보를 자동 선택하지 않는다. 발송 중에는 대상 검색과 역할 변경을 잠가 화면의 선택과 전송 대상을 일치시킨다.
...
- `공개 매칭방 + 팀전`: `visibility:"public"`, `hostJoinMode:"team"`, `teamOnly:true`와 빈 팀/명단으로 만든다. 공용 방 모달에서 A팀을 선택한 뒤 공개 B팀의 현재 소속·참가 가능 팀원 1명이 대표로 참가해 주장이 되며 개인 참여는 막는다.
...
- 팀 구성은 방모달에서 서로 다른 등록 팀 두 개를 먼저 고른다. 각 팀장이 자기 팀의 출전 명단을 정원만큼 제출하고 후보는 사이드당 최대 3명까지 보존한다. 후보는 실제 출전 처리 전에는 `recordApproverIds`, 참가 확인, 개인 MMR 대상에 포함하지 않는다. 기록 입력이 끝나면 실제 참가자가 본인 계정으로 `내 참가 확인`한다.
...
## 2026-07-07 경기기록방 주장 로스터 기준

- 경기기록방 팀전의 주장은 해당 사이드 로스터 저장 권한의 기준점이다.
- 현 DB/서버 저장 경로는 주장을 첫 출전자로 보고 검증한다.
- 주장을 비출전 대표로 분리하려면 별도 `sideLeaderId` 원본을 먼저 설계해야 한다.
- 그 전까지 UI는 주장을 후보/해제 상태로 내리지 않는다.
...
3. 일정 저장은 선택한 허용 구장을 경기 snapshot에 함께 저장하고 양 팀장에게 출전·후보 명단 구성 알림을 갱신한다.
...
6. 불참 side, 사유, 처리자, 처리 시각은 `matches.rules.forfeit`에 남기고 양 팀장에게 결과 알림을 보낸다.
...
1. 대회 생성자는 `tournamentOrganizerId`로 유지한다. 대회 경기 방장은 대회 생성자가 아니라 A사이드 참가 팀장이다.
...
9. 대회 방장은 운영 권한일 뿐 자동 출전자가 아니다. 팀장이 명단에서 직접 출전 또는 후보로 선택된 경우에만 선수 슬롯과 기록자 후보에 포함한다.
10. 경기 결과 최종 승인은 DB에 저장된 각 사이드 실제 출전 선수의 과반으로 계산한다. 출전하지 않은 팀장이나 대회 방장의 승인을 요구하지 않는다.
11. 아직 어느 팀도 출전 명단을 확정하지 않은 대회 경기는 대진의 첫 팀장을 임시 A사이드 방장으로 둔다. 이는 운영 권한과 팀 표시만 위한 값이며 팀장을 선수 슬롯에 자동 배치하지 않는다.
...
34. 팀에는 `captain`이 정확히 1명만 존재한다. 팀 저장, 초대 수락, 가입 승인은 `팀 -> 사용자 ID 정렬` 순서의 transaction lock을 공유하며, 사용자당 최대 3팀과 팀당 최대 30명 제한을 병렬 요청에서도 강제한다.
...
3. 리그 종료 순위는 승수, 득실차, 득점, 팀명, 팀 ID 순이다. 우승팀, 완료 시각, 최종 순위 snapshot을 저장하고 개최자와 승인 팀장에게 앱 알림을 한 번만 만든다.
...
6. 경기 알림 취소 prefix와 종료 상태 판정은 `src/lib/notifications.js`, 모집 신청 상태는 `src/lib/recruiting.js`, 팀장 조회는 `src/data/teamMappers.js`, 구장 표시 주소는 `src/lib/courts.js`를 단일 원본으로 사용한다.
...
5. 사용자가 팀명·홈코트·해시태그를 검색하거나 특정 지역 보기를 선택했을 때만 bounded 검색 결과를 표시한다. 팀 찾기의 검색·즐겨찾기 결과는 기존 검색어와 지역 조건을 변경하지 않고 팀 상세로 바로 이동한다. 로컬 목록에 없는 원격 검색 결과도 `teamPreview`를 전달하고 상세 조회·권한 검증은 기존 팀 상세 흐름을 따른다.
...
8. 시즌 라이벌 매치업은 프로필 지역이 아니라 현재 사용자의 소속 팀 지역을 기준으로, 소속 팀이 포함된 지역 라이벌만 표시한다. 소속 팀이 없으면 라이벌 매치업과 자동 초대를 표시하지 않는다. `매칭 만들기`는 기본값이 비공개인 기존 팀방 생성 흐름을 열어 내 팀과 상대 팀을 미리 선택하고, 서버에는 빈 팀방을 먼저 생성한 뒤 기존 `setRecruitingRoomTeam` action으로 A팀 선택과 B팀 초대를 순서대로 수행한다. 사용자가 공개방으로 바꾸면 공개 팀방 규칙에 따라 A팀만 선택하고 특정 B팀 초대는 보내지 않는다.
...
7. 공개·비공개 전환은 공개 범위와 초대 대상만 바꾼다. 후보 정원, 경기 규칙, 비용, 만남 장소처럼 이미 입력한 공통값을 프리셋 기본값으로 되돌리지 않는다. 이미 선택된 공개·비공개·대회·경기 기록·개인 기록 유형을 다시 눌러도 날짜와 구장을 포함한 작성 중인 값을 초기화하지 않는다.
8. 경기 방식은 1v1·2v2·3v3·3x3·5v5를 선택한다. 실제 경기 인원이 달라질 때만 출전 인원과 명단을 다시 계산한다. 일반 3v3↔3x3 전환은 명단을 유지하고 선택한 방식의 기본 시계·공 규격을 적용한다. 같은 방식을 다시 선택하거나 경기 목적·구성만 전환하면 사용자가 조정한 시계 규칙도 유지한다. 후보 정원, 비용, 만남 장소, 공격권·파울 메모는 항상 유지한다.
...
10. 구장 리뷰와 정보 수정 신고 요청이 실패하거나 예외로 끝나면 클라이언트는 저장 중 상태를 반드시 해제하고 같은 화면에서 다시 시도할 수 있게 한다.
...
12. 샷클락 종료는 짧은 부저, 쿼터 종료와 경기시계 종료는 긴 부저, 휴식 상한 초과는 짧은 2회 경고 부저를 사용한다. 부저는 샷클락 외부 입력용 미디어 세션과 분리된 Web Audio 경로를 사용하며 재생이 차단되면 실패 상태를 표시한다.
...
16. 취소·만료 방의 `같은 설정으로 다시 만들기`는 원본 모집방과 새 모집방의 계보를 서버 transaction에서 기록한다. 첫 다시 만들기는 안내만 표시하고, 같은 계보의 연속 2회부터 경고하며, 3회 이상은 자동 차감하지 않고 관리자 운영 검토 및 신뢰도 조정 가능성을 안내한다.
16-1. 확정 경기는 `같은 경기 또하기`로 새 비공개 방을 만든다. 취소 방 계보·경고 횟수에는 넣지 않는다. 팀전은 기존 A팀으로 방을 만들고 기존 B팀에 새 팀 초대를 보내며, 개인전은 이전 방장·심판·기록자를 제외한 실제 참가자와 후보에게 새 초대를 보낸다. 이전 결과, MMR, 채팅, 출석, 추천, 참가 상태는 복사하지 않는다.
...
7. 경기시계를 시작하면 모바일 전광판의 집중 화면과 브라우저 전체화면을 즉시 요청한다. 시작 실패 시 원래 화면으로 복귀하고, 경기 종료 성공 또는 종료 상태 수신 시 집중 화면과 전체화면을 해제한다. 브라우저가 전체화면을 거부하면 집중 화면은 유지한다. 부저는 브라우저·기기 제한이 있는 베타 기능으로 안내하며 별도 화면 유지 버튼은 제공하지 않는다. 수동 전체화면은 경기시계를 최초 시작하기 전에는 열 수 없고, 한 번 시작한 뒤에는 일시정지 상태에서도 사용할 수 있다. 자동 상대 확정, 자동 점수·공격권 인식, 공인 심판 자동 인증, 실제 라이브 중계처럼 구현되지 않은 기능은 사용 설명에서 현재 지원으로 표시하지 않는다.
...
10. 사용 설명은 사후 경기기록·개인 기록 외 모든 실제 경기방에서 QR 출석을 필수 사용한다고 안내한다. `QR 출석은 경기 20분 전부터 시작합니다. 출전·후보 선수가 모두 출석하면 예정시간 전에도 경기를 시작할 수 있습니다.`를 기본 안내로 사용한다. 5분 토큰, 로그인한 사전 등록 선수 제한, 시작 후 QR은 사전 등록 `no_show`를 원래 사이드 후보로 등록할 뿐 실제 출전이나 팀 배치를 확정하지 않는다는 점을 함께 설명한다.
...
3. 연습 상태는 로그인 여부와 무관하게 열 수 있는 `/app/guide/practice` 페이지 메모리에만 둔다. `useAppData`, 앱 전역 state, `localStorage`, `sessionStorage`, 운영 DB에 합치지 않으며 새로고침 또는 페이지 이탈 시 초기화한다.
...
17. 연습 경기는 QR 출석을 활성화하고 출석 단계에서 공용 출석 패널을 표시한다. 출석 패널과 경기시계는 운영 출석 API를 호출하지 않고 격리된 연습 QR과 명단·출석 state로 동일한 시작 상태를 계산한다. 출전·후보 전원의 연습 출석이 끝나야 경기 시작을 허용하며, `ranked=false` 때문에 친선전으로 오인되지 않도록 방 종류는 `연습경기`로 표시한다. 연습 경기시계 점수 응답도 운영 응답과 같은 점수 revision을 제공한다.
18. 연습 경기시계 시작은 되돌릴 수 있는 로컬 action이므로 브라우저 확인창 없이 첫 클릭으로 실행한다. 운영 경기의 기존 시작 확인과 종료·구간 전환 확인은 유지한다.
...
5. 서버가 현재 Supabase 세션의 관리자 레벨 `100`을 확인한 설정 전환만 현재 관리자 Auth 세션을 탭 범위 복귀 정보로 보관한다. 테스트 계정 전환 목록에는 이 관리자 로그인 ID를 `관리자 원계정`으로 함께 표시하며, 복귀 시 저장한 서명 세션의 동일 Auth user만 허용한다.
6. 관리자 복귀 정보는 권한 판정값이 아니며 브라우저 탭을 닫거나 로그아웃·탈퇴·새 OAuth 로그인을 시작하면 제거한다. 관리자 권한은 복귀한 Supabase JWT를 서버가 다시 검증해 확정한다.
...
4. 모든 이의신청은 접수 시 현재 `result_revision`과 정확히 일치해야 한다. 가결 시 무심판 `team_scores`는 현재 revision과 다시 일치해야 하고, 심판 `player_stats`는 같은 revision에서 병렬 접수된 선수별 요청을 현재 draft에 순차 병합한다. 무심판은 방장, 심판 경기는 배정된 유효 심판이 각 요청을 transaction lock 안에서 가결 또는 부결한다.
...
6-1. 로컬/demo reducer도 `startedAt`이 없는 경기를 종료하거나 종료 시점에 시작 시각을 합성하지 않는다.
...
4. 진행 중 인정 정보는 실제 활성 경과시간과 최소 인정시간을 함께 표시한다. 인정 판정은 서버에 누적된 실제 활성 경과시간을 기준으로 하며, 시계 종료 전에는 `정상 사용`으로 단정하지 않고 종료 뒤 서버 판정만 표시한다.
...
10. 경기시계의 자동 조회는 진행 중인 시계·점수·경기 종료 action을 덮지 않는다. action 시작 전 조회와 이전 경기 조회는 폐기하고 현재 경기의 최신 응답만 반영한다.
11. 담당자가 최소 인정시간 전에 경기시계를 종료했더라도 경기 lifecycle이 종료되지 않았고 현재 구간 시간이 남아 있으면 같은 담당자가 `계속`으로 재개할 수 있다. 이미 정상 사용 조건을 충족했거나 경기 자체가 종료된 시계는 재개하지 않는다.
12. 경기 종료 권한자는 경기시계 인정 조건과 무관하게 같은 `endMatch` lifecycle action으로 경기를 정상 종료할 수 있다. 최소 활성시간을 충족하지 못한 상태의 버튼만 `강제 종료`로 표시한다. 시작 창만 놓쳤지만 최소 활성시간을 채웠다면 `경기 종료`로 표시하고 경기시계 미사용 판정을 별도로 안내한다. 종료된 경기시계가 미사용 판정이어도 이 action을 숨기지 않는다.
...
2. 공개·비공개 경쟁전과 친선전은 모두 QR 출석을 필수 사용하며 생성·수정 화면에서 끌 수 없다.
3. 대회도 QR 출석을 필수 사용한다. 사후 경기기록(`match_record`)과 개인 기록(`solo`, `personal_record`)만 출석 phase와 QR을 만들지 않는다.
4. QR 발급 API, 경기시계 QR, DB 출석·정원 조정 RPC는 사후 경기기록만 제외하고 같은 eligibility로 판정한다.
5. 출석의 최초 조회·수동 새로고침·자동 조회가 겹치면 현재 경기의 최신 요청만 출석 수와 시작 가능 상태를 갱신한다.
...
## 2026-07-31 경기 기록 완료·이의제기·파티 합류 동기화

1. 경기 권한자의 `기록완료`가 성공하면 경기 상세와 현재 사용자 기록 목록을 함께 강제 갱신한다. 확정된 경기는 활성 기록판에서 빠지는 동시에 실제 출전자의 경기기록에 노출돼야 한다.
2. 종료 후 최종 확정 전 방은 경기 상세를 주기적으로 갱신한다. 참가자의 이의제기가 저장되면 심판의 같은 방에 이의제기 큐가 별도 재진입 없이 표시돼야 한다.
3. 이의제기 사유는 스탯 변경이 없어도 저장할 수 있다. 교체·후보 출전 누락처럼 숫자 외 판정이 필요한 사유를 숫자 변경으로 강제하지 않는다.
4. 기존 후보가 팀 파티에 합류해도 후보 배치를 유지한다. 합류 action은 출전 승격이나 파티장 권한 이전으로 사용하지 않으며, 기존 출전 파티장을 명시적으로 보존한다.
5. 파티 합류 뒤 출전 인원은 사이드 정원을 넘지 않아야 한다. 목록 카드와 방 모달은 같은 파티장·출전·후보 snapshot을 사용한다.

## 2026-08-10 경기 취소와 기록 완료 동기화

1. 진행 중 경기의 `경기 취소`와 `경기 종료`는 같은 경기 mutation 잠금을 사용한다. 취소 요청이 시작되면 종료·이의 단계로 중복 전환하지 않는다.
2. 기록 완료 확인창은 열 때의 이의 수·완료 자격을 복사하지 않고, 현재 경기의 열린 이의 수와 최신 완료 자격을 사용한다.
...
## 현재 유효: 대회·시즌 directory 표시 규칙

1. 대회 만들기의 원격 팀 검색 결과는 선택 즉시 팀 snapshot을 보존한다. 로컬 directory에 아직 병합되지 않은 팀도 자격 검증과 참가팀 표시를 유지하고, 생성 payload에는 선택한 팀 ID를 제출한다.
2. 대회 목록과 상세의 참가팀 ID는 `getTournamentTeamIds()`를 공통 사용한다. `teamIds`와 `teamStatuses`에 나뉜 기존 데이터도 같은 팀 목록으로 표시한다.
3. 시즌 개인 순위와 승격권에는 현재 사용자의 `blockedUserIds` 대상자를 표시하지 않는다.
4. 대회 심판 원격 검색에는 이미 초대된 심판을 다시 표시하지 않는다. 초대 요청이 실패하면 검색어를 유지하고 성공한 경우에만 초기화한다.
5. 대회 승자·리그 경기 수/승패/득실·우승·후속 대진은 `status=confirmed`인 경기만 반영한다. 점수나 과거 확정 시각만으로 결과를 확정하지 않으며 승인·이의 단계는 `결과 확인 중`으로 표시한다. 확정된 몰수승과 대진의 부전승은 기존 진출 규칙을 유지한다.

## 2026-07-31 팀 관리·사후 경기기록 구성 안정성

1. 팀 초대 취소, 팀원 역할 변경, 팀원 제외는 하나의 팀 관리 요청만 처리한다. 처리 중에는 팀 관리 입력을 잠그고 실패 이유를 같은 패널에 표시한다. 팀원 제외·팀 삭제 확인창은 서버 실패 시 닫지 않고 오류를 창 안에 표시하며, 새 확인창에서는 이전 요청 오류를 숨긴다.
2. 사후 경기기록 참가자 저장은 성공·실패와 관계없이 저장 대기 상태를 해제해 다시 시도할 수 있어야 한다.
3. 팀 사후 경기기록의 상대 팀은 A사이드와 같은 팀이거나 같은 팀장이 관리하는 팀일 수 없다. 로컬 목록과 원격 검색 결과에 같은 기준을 적용하고 선택 불가 이유를 표시한다.
4. 팀·선수 디렉터리 조회 결과의 객체가 갱신돼도 사용자가 편집 중인 사후 경기기록 구성은 초기화하지 않는다. 서버가 저장한 경기 ID·팀 선택·선수 명단이 바뀔 때만 초안을 다시 맞춘다.

## 2026-07-31 생성 재진입과 원격 즐겨찾기

1. 모든 경기·대회·기록방 생성 제출은 렌더 상태와 별개인 단일 ref mutex로 중복 실행을 막는다.
2. 취소 방 다시 만들기의 팀·선수 재초대는 요청한 재초대가 모두 성공해야 생성 완료로 본다. 하나라도 실패하면 새 방을 종료하고 실패를 표시한다.
3. 원격 검색 결과의 즐겨찾기는 선택한 엔티티 snapshot을 로컬 directory에 먼저 병합한 뒤 낙관 반영한다. 서버 실패 시 즐겨찾기 상태만 롤백한다.

## 2026-07-31 랭킹 directory와 차단 표시 일관성

1. 개인 랭킹 directory는 통합·지역·1v1·2v2·3v3·5v5 탭의 정렬값을 allowlist로 서버에 전달하고, 서버가 해당 MMR로 정렬한 뒤 페이지 범위를 자른다. 다음 페이지도 최초 정렬값을 유지한다.
1-1. Supabase 미연결 로컬 데모에서는 원격 directory 완료 상태를 기다리지 않고 현재 mock 랭킹을 즉시 표시한다. Supabase 설정 환경에서만 원격 directory 로더와 실패 상태를 사용한다.
2. 홈의 원격 선수·심판 검색, 선수·심판 즐겨찾기, 지역 상위 랭커에는 현재 사용자의 `blockedUserIds` 대상자를 표시하지 않는다.
3. 선수 상세 조회 실패와 존재하지 않는 선수는 구분한다. 일시 실패는 같은 경로에서 재시도하고, 정상 조회 뒤 대상이 없을 때만 찾을 수 없음으로 표시한다.
4. 소속 랭킹의 순위는 지역과 일반 소속 유형별로 따로 계산한다. 소속 directory 조회 실패는 기존 데이터를 지우지 않고 같은 페이지에서 재시도한다.

## 2026-07-31 팀 검색·상세·차단 연결

1. 원격 팀 검색에서 선택한 팀 snapshot은 검색 결과 카드와 팀 상세 이동 preview에 유지한다. 팀 상세 즐겨찾기도 이 snapshot을 함께 전달해 directory 병합 전 선택이 사라지지 않게 한다.
2. 팀 생성 제출은 ref mutex로 중복 실행을 막는다.
3. 단일 유형 원격 검색은 로컬 결과에 해당 유형을 기본값으로 적용해 같은 ID의 로컬·원격 항목을 한 건으로 합친다. 플레이어와 심판처럼 의미가 다른 유형은 계속 분리한다.
4. 승격권 전용 랭크보드는 직접 진입하거나 새로고침해도 현재 사용자의 경기 기록을 불러온 뒤 시즌 개인·팀 순위를 계산하며, 조회 실패를 빈 순위로 표시하지 않고 같은 화면에서 재시도한다.
5. 통합 검색은 같은 사용자 ID의 플레이어와 심판을 서로 다른 종류로 유지하되, 같은 종류의 로컬·원격 결과는 하나로 합친다.
6. 원격 플레이어 차단은 로컬 사용자 directory에 아직 없는 ID도 즉시 `blockedUserIds`에 반영한다.

## 2026-08-01 경로 없는 검색·방·지도 복구 규칙

1. 홈·알림에서 모집방을 열 때 대상 row가 로컬에 없으면 공용 상세 로더를 사용한다. 조회 실패는 무반응이나 영구 로딩으로 숨기지 않고 같은 모달에서 재시도한다.
2. 슬롯 팀 명단 저장은 한 번에 하나만 처리한다. 서버 저장이 실패하면 편집 초안과 패널을 유지하고 성공했을 때만 authoritative 명단을 반영한다.
...
## 2026-08-01 대회 운영 안내와 작업 후 재조회 규칙

1. 대회 승인, 심판 초대·배정, 지역 승인, 비승인 개최 작업은 서버 mutation 성공을 최종 성공으로 본다. 후속 상세 재조회 실패는 이미 반영된 작업을 실패로 되돌리지 않으며 같은 mutation의 재실행을 유도하지 않는다.
2. 대회 상세의 다음 행동은 공용 모델에서 현재 역할의 본인 팀·심판 승인 요청, 개최 전 필수 승인·심판 자격과 중립 조건, 경기별 심판 배정·일정 설정·미확정 결과 순서로 선택한다. 기존 권한·심판 검증·일정 잠금 helper를 그대로 사용하고 안내 자체로 수정 권한을 부여하지 않는다. 미정 일정 편집은 주최자에게만 연결하며 명단 제출·경기 시작으로 잠긴 일정은 제외한다. 조회자는 같은 항목의 현황으로 이동하고, 종료·취소 대회는 경기표와 기록 확인을 안내한다.

## 2026-08-03 구장 현장 증거와 AI 자동승인

1. 현재 위치 경로는 모바일 후면 카메라를 우선 여는 native capture input으로 현장 사진 1~2장을 누적하고, 주소 경로는 사진 0~2장을 보관함에서 선택할 수 있다. 사진은 개별 재선택·삭제할 수 있다. 원본 파일 용량을 제한하지 않고 브라우저에서 긴 변 1280px 이하, 장당 목표 220KB로 자동 변환한다. Canvas WebP 인코딩을 지원하지 않는 브라우저는 JPEG 호환본을 보내며 서버가 즉시 메타데이터 없는 WebP로 재인코딩한다. 서버는 장당 300KB 이하 결과만 받는다.
2. 사진은 공개 자산 버킷과 분리된 비공개 R2 버킷에 `court-requests/{requestId}/{sha256}.webp`로 저장한다. DB에는 object key와 해시만 저장하고 관리자 전용 API만 원본을 읽는다.
3. 사용자는 현재 GPS로 핀·주소의 초기 위치를 지정한 뒤 지도에서 실제 코트 중심으로 핀을 조정하거나, 주소 검색 후 지도 핀을 확정한다. 이 선택은 입력 방법일 뿐 승인 종류가 아니다. 서버는 10분 이내 현장 GPS가 정확도 20m 이하이고 핀과 150m 이내인 경우와 사진 EXIF GPS가 핀과 일치하는 경우를 각각 위치 증거로 계산한다. 150m 초과도 신청은 가능하지만 자동승인 위치 증거로 사용하지 않는다.
4. AI는 사진별 코트·골대·라인·체육시설 외관/입구/간판·스크린샷 또는 합성 위험을 구조화해 반환한다. 서버는 두 사진의 역할을 합쳐 최소 한 장의 코트 증거와 두 장 전체의 구장/시설 증거를 판단한다. 최종 결정은 서버 정책 함수가 하며 AI 응답에 DB 승인 권한을 주지 않는다.
5. 자동승인은 실내·야외 공개 구장, 사진 정확히 2장, 신뢰도 90 이상 또는 과거 승인 2건 이상, 30m 내 중복 없음, 코트와 골대 또는 라인이 보이는 사진 1장 이상, 나머지 사진까지 코트 또는 체육시설 증거에 해당하고 두 사진 모두 실사진일 때만 허용한다. 코트 형태는 신청 정보로 저장하되 AI 판정이나 자동승인 조건으로 사용하지 않는다. 위치는 유효한 현장 GPS 또는 모든 사진의 EXIF GPS 일치 중 하나로 충족한다. 사진 GPS가 `uncertain`이면 150m 이내의 유효한 현장 GPS가 함께 있을 때만 허용하고, `mismatch`이면 자동승인하지 않는다. 같은 사진 재사용은 허용하지 않는다.
6. AI 응답 실패나 비정형 응답은 한 번 재시도한다. 미설정·재시도 실패·정책 미달·DB 승인 실패는 자동거절하지 않고 요청과 증거를 `manual_review`로 남기며 제한된 실패 코드를 감사 정보에 기록한다.
7. 모델, 프롬프트 버전, 이미지 해시, 현장 GPS 오차·거리, 사진별 EXIF GPS·촬영시각·현장 및 핀과의 거리, 사진별 판정, 최종 결정과 자동승인 여부를 감사 가능한 증거 row에 보관한다.
...
15. AI 호출 직전에 신청 ID별 최대 비용 48 neuron을 DB advisory lock 안에서 원자 예약한다. 확정 사용량을 기록할 때 예약을 실제 neuron으로 정산하고 미호출 실패는 0으로 해제한다. 기존 사용량과 활성 예약 합계가 8,000 neuron을 넘는 예약은 호출 전에 거부한다.
...
18. 허위 구장 신청 제재는 신고 접수, AI 판정 실패, 자동승인 탈락, 관리자 일반 반려만으로 적용하지 않는다. 관리자가 `court_request` 신고를 인정해 기존 `trustPenaltyApplied`가 요청별 1회 확정된 경우에만 누적 1회 3일, 2회 7일, 3회 이상 30일 동안 추가 신청을 막는다.
...
1. 샷클락을 사용하는 현재 경기시계 담당자는 샷클락 선택 또는 경기시계 시작의 사용자 입력 안에서 1초 제어 미디어를 재생한다. 이후 기기 미디어의 재생·일시정지 입력은 기존 `resetShot` action으로 샷클락을 초기화한다.
2. 경기 종료 mutation 중 플레이 목록을 다시 불러와도 해당 경기 ID를 목록 범위에서 먼저 제거하지 않는다. mutation 응답을 반영한 뒤 공용 lifecycle의 `postgame` 여부로 표시를 결정한다.

## 2026-08-04 방 목록 재조회 경합 방지

1. 경기·모집방 mutation 시작 시 공용 방 mutation 버전을 올린다.
2. 개인 일정·팀 일정·플레이·홈 목록 재조회 중 방 mutation이 시작됐거나 재조회 시작 시 이미 진행 중이었다면, 해당 응답은 현재 목록 ID를 제거하지 않고 새 서버 ID와 합친다.
3. 경합 응답은 경기·모집방 상세를 강제로 덮지 않고 `updatedAt` 기준 병합을 사용한다. mutation이 없는 다음 정상 재조회부터 서버 ID 집합으로 다시 교체한다.

## 2026-08-05 FOUNDING PLAYER

1. 실제 인증 사용자가 `Season Zero` 종료 전 가입하고 프로필 완성을 끝내면 서버가 `founding_player`를 영구 지급한다. 테스트 계정은 대상에서 제외한다.
2. 지급된 `founding_player`는 시즌 종료나 프로필 수정 뒤에도 유지하며 클라이언트가 직접 지급·회수할 수 없다.
3. FOUNDING PLAYER는 기념 자격이다. 무료 MMR이나 경기별 MMR 보정은 제공하지 않으며 알파 기간의 확정 경쟁전과 배치 진행도는 기존 개인 배치 규칙대로 유지한다.
4. `founding_player` 지급 사용자는 `341-founding-player-s0` 업적 프로필 아이콘을 영구 해금한다. 업적 갱신 서버가 프로필의 자격값을 직접 읽어 해금하며 클라이언트 값은 신뢰하지 않는다.
5. 홈의 Season Zero 배너는 지급 완료 사용자를 프로필 아이콘 선택창으로 연결하고, 미지급 사용자를 프로필 완성 화면으로 연결한다.
6. 이미 `founding_player=true`인 기존 프로필에도 `341-founding-player-s0` 보유 행을 비파괴 백필한다. 기존 해금 시각과 선택값은 덮어쓰지 않는다.

## 2026-08-06 게스트 매칭 조회

1. 비로그인 게스트에게 데모 사용자·경기·알림·팀을 주입하지 않는다. 게스트 홈은 일반 홈의 섹션 구조를 유지하되 개인 기록·일정·알림 값은 로그인 안내 상태로 표시하고 수치나 행을 추정하지 않는다.
2. 게스트 매칭 목록은 인증 토큰 없이 허용된 공개 API가 반환한 `status=open`, `visibility=public` 모집만 최대 50건 표시한다. 공개 방 설정과 참가자 명단은 공용 방 모달에서 읽을 수 있다. 참가자 명단은 공개 프로필 카드 정보와 사이드·출전/후보 배치만 최소 투영하며 채팅, 초대, 메모, 제재·확인 상태 등 인증 전용 필드는 반환하거나 추정하지 않는다.
3. 개인 일정은 인증 토큰이 없으면 원격 로딩을 시작하지 않고 로그인 필요 상태로 즉시 종료한다. 인증 전용 앱 경로도 데모 폴백 없이 로그인으로 보낸다.
4. 매칭 목록의 지역 기본 범위는 `전체`다. 시도 선택 목록 첫 항목은 인증 사용자의 canonical 프로필 지역을 사용하는 `내 지역`이고, 전체·내 지역·시도/시군구 선택은 같은 목록 API의 `regionScope`로 처리한다.
5. 공개 매칭이 50건을 넘어 전체 목록 탐색성이 실제로 저하될 때 지역 기본값 변경을 다시 검토한다. 그 전에는 전체 기본값을 유지한다.
6. 게스트는 경기방·대회·사후 경기기록·개인 기록의 생성 화면과 입력 단계를 둘러볼 수 있다. 인증 전용 directory 조회와 실제 mutation은 실행하지 않는다.
7. 최종 생성 action에서 Supabase access token을 확인한다. 토큰이 없으면 현재 생성 draft와 정확한 생성 URL을 같은 탭의 `sessionStorage`에 보관하고 로그인으로 이동한다. 로그인·필수 프로필 설정 뒤 같은 URL로 돌아오면 draft를 복원하며 실제 생성 성공 후에만 보관본을 지운다.
8. 게스트 입력은 운영 데이터가 아니다. 서버 mutation은 기존 인증·권한·방 phase·MMR·기록 검증을 그대로 통과해야 한다.
9. 게스트는 커뮤니티의 게시글 목록·상세·댓글을 실제 공개 데이터로 읽을 수 있다. 좋아요·글·댓글 작성과 수정·삭제는 기존 인증과 권한 검증을 통과해야 한다.
10. 게스트 앱 메뉴는 조회 결과가 0건이어도 유지한다. 운영 Supabase가 설정된 환경에서는 커뮤니티 데모 글을 공개 데이터 대신 표시하지 않는다.
11. 팀 목록·선수/팀 랭크보드·선수/팀 검색은 인증 없이 실제 공개 directory projection으로 조회한다. 개인 설정, 즐겨찾기, 초대, 팀 관리 정보는 공개 응답에 포함하지 않는다.
12. 게스트가 모집 카드를 누르면 인증 화면이 아니라 기존 공용 방 모달을 읽기 전용으로 열고 공개 참가자 명단을 표시한다. 채팅 조회와 참가·수정 등 mutation에서만 원래 방 경로를 보존해 로그인을 요구한다.
13. 게스트가 플레이·나·설정 메뉴를 열면 해당 기능의 controller나 개인 데이터 로드를 시작하지 않고 공용 로그인 안내 상태를 먼저 표시한다. 로그인 action은 현재 경로를 복귀값으로 보존하고 공개 매칭 이동을 함께 제공한다.
14. 팀 메뉴는 게스트에게 실제 공개 팀 목록과 상세를 계속 제공한다. 게스트 팀 상세는 공개 directory의 `teams` 범위로 대상 팀과 공개 팀원 프로필만 읽으며, 개인 설정·초대·관리 정보는 포함하지 않는다. 내 팀 관리와 팀 생성처럼 인증이 필요한 영역만 같은 공용 로그인 안내 상태를 사용한다.
15. 확정된 일반 경기는 원래 모집방의 공개 여부와 관계없이 익명 공개 기록으로 읽을 수 있다. 공개 개인기록도 익명으로 읽을 수 있으며 `visibility=private`인 개인기록만 작성자·허용 사용자 범위에 남긴다. 공개 응답은 경기 결과·규칙·구장·공개 참가자·공개 스탯·MMR만 투영하고 동의, 승인, 분쟁, 증빙, 내부 식별자, 채팅, 초대, 메모, 운영 상태는 포함하지 않는다.
16. 게스트 모집 지역 필터는 서버 목록 쿼리에 전달한다. 최신 전체 목록을 먼저 제한한 뒤 브라우저에서 지역을 거르는 방식은 사용하지 않는다. 공개 참가자 명단은 확정 출전·후보·심판 슬롯에서만 만들고 신청 대기·거절·취소 사용자는 포함하지 않는다.

## 모바일 전광판 구간 점수 자동 집계

1. 경기 진행 중 canonical 총점의 증감은 서버 경기시계의 현재 구간에 같은 트랜잭션으로 누적한다. 정규 1·2·4구간과 단일 `OT` 합산을 지원하고, 휴식 중 보정은 직전 정규 구간에 반영한다. 점수 증감·심판 PTS 제출의 기존 조작 권한은 유지한다.
2. 유효한 시계는 `running / paused / break` 상태, 규칙 범위 안의 정수 구간, 0 이상의 정수 연장 횟수를 가져야 한다. 연장은 마지막 정규 구간 뒤에만 허용한다. 미래 구간 기록을 남긴 채 과거 구간으로 돌아가거나 현재 구간 점수가 음수가 되면 자동 배분을 추측하지 않는다.
3. 시계가 없거나 종료·유효하지 않은 경우, 기존 구간 합계가 canonical 총점과 다른 경우에는 총점 저장은 유지하고 구간만 `[]`로 비워 확인 대상으로 표시한다. 총점 증감이 0이면 기존 구간을 그대로 보존한다. 클라이언트 snapshot은 서버 시계의 authority를 대신하지 않는다.
4. 진행 중 결과 제출의 수동 `periodScores`는 자동 집계를 덮어쓰지 않는다. 종료 후 기존 결과 제출 권한이 있는 무심판 경기 방장 또는 배정 심판이 최종 기록에서 구간 배분을 수정할 수 있다. 구간은 비워둘 수 있으나 입력하면 양 팀 값·순서·canonical 총점 합계를 검증하고, 확정 후에는 잠근다. 심판 경기 총점은 선수 PTS 합계를 유지하며 무심판 최종 제출은 저장된 총점을 보존한다.

## 2026-08-06 확정 경기 참가 취소와 모바일 전광판

1. `경기시계`는 경기시간 표시·시작·정지·재개만 뜻한다. `모바일 전광판`은 경기시계, 점수 입력판, 출석 QR, 샷클락을 합친 현장 운영 화면이다. 사용자 역할명은 `모바일 전광판 담당자`로 표시하고 내부 `clockController` 식별자는 호환을 위해 유지한다.
2. 일반 경기의 출전선수·후보선수는 경기 시작 전 `locked`·`checkin` 단계에서 사유 5~200자를 입력해 본인 참가를 취소할 수 있다. 대회, 사후 경기기록, 개인 기록, 시작·종료·결과가 발생한 경기는 이 action을 허용하지 않는다.
3. 참가 취소는 서버 transaction 하나에서 출전·후보 명단, 파티, 출석, 동의, 승인, 규칙 확인을 정리한다. 출전선수가 취소하면 확정 단계에서는 같은 사이드 첫 후보를, 출석 단계에서는 출석 완료 후보를 자동 출전 전환한다. 픽업 경기는 팀 배정을 다시 미확정으로 돌린다.
4. 후보 출전 전환 뒤에도 한 사이드 출전 인원이 정원보다 적으면 경기는 자동 취소하지 않고 `rosterNeedsFill=true`와 부족 사이드를 저장한다. 방장은 인원을 보충하거나 경기 취소를 선택한다.
5. 참가 취소 신뢰도는 시작 4시간 초과 전 0점, 시작 4시간 이내 2점, 출석 단계 4점을 기본값으로 사용한다. 운영자는 공용 rating policy의 `participantCancelShortNoticePenalty`, `participantCancelCheckinPenalty` 범위 안에서 조정한다.
6. 참가 취소 알림은 취소자, 방장, 배정 심판, 같은 사이드 현재 참가자에게 보낸다. 인원 부족이면 방장 알림을 action-required로 표시한다. 이후 방장이 인원 부족으로 경기를 취소하면 기존 경기 취소 알림을 현재 참가자와 이미 참가 취소한 사람 모두에게 보낸다.
7. `rosterNeedsFill=true`인 경기의 방장 취소는 시작 2시간 전 잠금과 방장 신뢰도 차감을 면제하고 `participant_shortage`를 감사 사유로 저장한다. 일반 방장 취소 정책은 바꾸지 않는다.
8. 공용 방 모달은 모집, 확정, 출석, 진행, 종료, 이의, 기록 단계별 도움말을 같은 팝업에서 제공한다. 도움말은 방장·주장·심판·모바일 전광판 담당자·선수 역할과 참가 취소·교체·기록 확정 흐름을 설명하며 별도 메뉴별 방 모달을 만들지 않는다.
9. 후보선수가 출전하려면 같은 사이드의 들어갈 출전선수를 고른 뒤 `출전선수와 교체` action을 실행한다. 단순한 후보 출석이나 READY만으로 출전 처리하지 않는다.
10. 모바일 전광판은 경기를 뛰는 사람의 휴대폰이나 태블릿에서 사용한다. 실시간 점수 입력과 부저 중복을 막기 위해 현재 담당자와 조작 기기는 한 명·한 대만 활성화한다. 방장이나 배정 심판은 담당자를 지정·변경할 수 있고, 현재 담당자도 권한을 다른 출전선수·후보선수·배정 심판에게 넘길 수 있다. 이전 기기는 읽기 전용으로 전환한다.

## 2026-08-07 위험 작업 확인과 경기 인원 강퇴

1. 방장이나 권한자의 경기 인원 강퇴는 사유 5~200자와 명시 확인을 요구한다. 클라이언트와 서버 모두 누락하거나 범위를 벗어난 사유를 거부한다.
2. 모집방 강퇴 기록에는 정규화한 사유를 남기고, 경기방 서버 action에도 같은 사유를 전달한다. 기존 phase·권한·명단 승계 규칙은 유지한다.
3. 팀원 제외·팀 삭제·회원 탈퇴는 확인 체크박스 이후 실행하고 로그아웃은 단순 확인만 사용한다.

## 2026-08-10 모집방 동시 요청·공유 링크

1. 같은 모집방의 서버 변경 요청이 겹치면 방별 pending 횟수를 누적한다. 한 요청이 끝나도 남은 요청이 있으면 목록 재조회 보호를 유지하고, 횟수가 0일 때만 보호를 해제한다.
2. 게스트 `?post=<id>` 공유 링크의 방이 제한된 공개 피드 목록에 없으면 해당 ID 한 건만 공개 projection으로 조회한다.
3. 단일 조회는 공개·모집 중인 방만 반환한다. 비공개·종료·없는 방은 방 데이터 없이 상태만 반환하고 화면에서 명확히 안내한다.
4. 팀·대회·모집방·경기·선수 프로필 공유는 공용 상세 경로와 공유 버튼을 사용한다. 버튼은 내용을 확인하고 보내기·복사 방법을 고르는 패널을 열며, 선택한 작업만 실행한다. 기기 공유를 지원하지 않으면 복사를 주 동작으로 제공한다. 팀·대회·공용 경기방은 현재 상세 데이터로 만든 안내 문구도 함께 공유·복사한다. 공유창 취소는 안내 없이 종료하고 실패 시 복사 버튼을 안내하되 자동 복사하지 않는다. 복사 실패 시 미리보기의 읽기 전용 주소 또는 안내 문구와 주소를 직접 선택해 복사할 수 있다. 닫거나 내용이 바뀐 패널에는 이전 비동기 결과를 반영하지 않는다. 공유는 조회·참가·수정 권한을 추가하지 않으며 로그인 복귀 경로에 원래 대상과 쿼리를 보존한다. 개인 기록에는 경기 공유 action을 제공하지 않는다.
5. 팀·대회·공용 경기방의 공유 이미지는 사용자가 요청할 때 브라우저에서 PNG로 생성한다. QR에는 공유 버튼과 같은 상세 주소를 넣으며 업로드나 별도 공유 DB row를 만들지 않는다. 준비된 PNG 파일의 공유 지원 여부는 경기 결과 영수증과 같은 공용 helper로 확인한다. 지원 기기는 버튼을 누른 즉시 파일 공유창을 열고, 미지원 기기는 이미지 저장을 제공한다. 이미지 공유 중에는 중복 실행을 막고, 취소 시 안내 없이 종료하며 실패 시 같은 패널에서 저장을 안내한다. 취소·실패 뒤 자동 복사나 다운로드는 하지 않는다. 닫거나 내용이 바뀐 패널에는 이전 공유 결과를 반영하지 않는다. 이미지는 공유 시점의 안내이고 최신 정보와 참가·가입·초대·승인 조건은 원래 상세 화면을 따른다. 기존 경기 결과 영수증은 기존 발급·공유 흐름을 유지한다.
6. 공유 문구와 이미지는 팀 활동 지역·주요 구장, 대회 일정·장소·승인 팀 수·상태, 경기 형식·진행 상태·일정·장소·현재 인원·비용 안내 중 현재 데이터에 있는 값을 사용한다. 경기 인원은 공용 모집방 인원 계산, 경기 형식은 공용 3x3/3v3 판별을 따른다. 비용이 없으면 미안내 또는 주최자 확인으로 표시하고 무료·절약액·초대 실적·보상을 추정하지 않는다.

## 2026-08-10 상세 화면 비동기 상태 격리

1. 상세 조회와 화면별 mutation은 시작 시 대상 ID와 요청 순번을 기록하고, 완료 시 현재 화면과 일치할 때만 상세·메시지·pending 상태를 반영한다.
2. 상세 모달 닫기나 다른 게시글·구장·팀으로 이동하면 이전 요청을 무효화한다. 이전 응답은 현재 화면을 다시 열거나 현재 상세·복원 가능 상태·메시지를 덮어쓰지 않는다.
3. 경기방의 결과 저장·상세 새로고침·구장 후기 저장도 `matchId`와 작업 순번으로 격리한다. 경기 경로가 바뀌면 이전 pending을 새 경기에서 해제하고 이전 완료 응답은 새 경기의 feedback이나 pending을 변경하지 않는다.
4. 경기 최종 승인 확인 체크 미완료는 제출 잠금 조건일 뿐 열린 이의신청으로 표시하지 않는다. 열린 이의 안내는 실제 열린 이의가 1건 이상일 때만 표시한다.

## 2026-08-10 원격 팀·관리자 작업 비동기 일관성

1. 생성 화면의 원격 팀은 B팀 선택이나 대회 초대 전에 팀 상세와 팀원 프로필을 수화하고 최신 프로필로 자격을 다시 계산한다. 프로필 누락은 확인 중으로 표시하며 수화 실패는 재시도할 수 있어야 한다.
2. 원격 팀 즐겨찾기는 팀 snapshot을 함께 전달해 공용 directory에 병합한다. 실패하면 optimistic 상태를 되돌리고 같은 화면에서 재시도할 수 있어야 한다.
3. 관리자 구장 저장·검수·중복·근접 조회는 성공·실패와 관계없이 pending을 해제한다. 실패한 입력과 draft는 유지하고 최신 요청만 화면 상태를 변경한다.
4. 대회 상세 조회는 `tournamentId`와 요청 순번이 모두 현재 화면과 일치할 때만 결과를 반영한다. 대회 경로가 바뀌면 이전 다이얼로그·선택 경기·검색·feedback과 이전 요청을 초기화한다.

## 2026-08-10 목록·위치 비동기 병합

1. directory 조회는 시작 시 프로필·설정·즐겨찾기 mutation 세대를 저장한다. 응답 전에 해당 mutation이 시작되거나 완료되었으면 현재 사용자 프로필과 해당 설정 영역을 병합하지 않으며, 이후 시작한 조회만 최신 값을 반영한다.
2. 구장 위치 GPS·역지오코딩·지도 핀·근처 구장 조회는 하나의 위치 작업 세대를 공유한다. 방식 변경·입력 초기화·화면 제거는 이전 작업을 무효화하고 최신 작업만 주소·좌표·핀 확정·근처 후보를 변경한다.
3. 알림 조회는 시작 이후 성공한 읽음과 삭제를 보존한다. 오래된 목록 응답은 `readAt`을 되돌리거나 로컬 삭제 항목을 복원하지 않는다.

## 2026-08-11 경기 영수증

1. 비회원 경기 영수증의 입력값은 브라우저에 24시간 임시 보관한다. 배경 사진은 IndexedDB에만 보관하고 서버, Supabase Storage, R2에 업로드하지 않는다. 두 출력 스타일은 미리보기 밖의 단일 canonical 사진 편집기를 공유한다. Pointer Events로 한 손가락 이동과 두 손가락 이동·확대·축소·회전을 처리하고, 내부 회전·90도 회전·초기화 action도 같은 사진 변환값을 갱신한다. 스타일별 슬라이더나 별도 편집 상태를 두지 않으며 두 미리보기는 편집 입력을 받지 않는다.
2. 유효한 비회원 영수증은 공유 시 서버에 사진을 제외한 공개 draft를 만든다. 공개 위치 식별자는 `BT-########` 형식의 canonical 경기 공개 코드이며, 소유권 capability는 공개 코드·공개 ID와 분리해 원 작성자 브라우저의 `HttpOnly`, `SameSite=Lax` 쿠키와 서버 해시로 관리한다. draft는 30일 뒤 만료하며 생성은 IP 해시 기준 시간당 10건으로 제한한다.
3. 공개 코드 조회는 로그인 없이 누구나 영수증 표시용 안전 필드를 읽을 수 있고 수정 권한은 주지 않는다. 공개 응답은 내부 원주소와 정확한 개인 MMR을 제외한다. 로그인 뒤 capability를 가진 동일 브라우저만 해당 draft를 한 사용자 프로필로 claim할 수 있으며, 다른 사용자는 QR이나 공개 코드만으로 원본 draft를 claim·수정할 수 없다.
   외부 클라이언트는 `GET /api/match-receipts/public?code=BT-########`로 화면을 열지 않고 같은 공개 투영을 JSON으로 조회한다. 응답은 capability·claim 권한·원주소·개인 MMR·프로필 해시태그·공식 경기 ID를 포함하지 않는다. 조회는 기존 IP 해시 기준 시간당 120회 제한을 공유하고 성공 응답은 30초 캐시한다.
4. 공유 영수증 수신자가 `상세 기록 이어서 작성`을 선택하면 현재 공개 payload를 새 draft와 새 capability로 복제한 뒤 공용 외부 로그인·claim·기존 `personal_record` 작성 흐름으로 이어간다. 원 작성자의 draft는 변경하지 않는다. 생성 성공 뒤 실제 경기 ID의 `/app/receipt?match={matchId}`로 복귀하고 내 기록에 표시한다.
5. 확정 경기 영수증은 기존 경기의 팀·점수·날짜·장소·팀 MMR·현재 사용자 개인 스탯을 사용한다. 확정 일반 경기는 원래 모집방의 공개 여부와 관계없이 canonical 공개 스냅샷을 만들고 `BOXTIER VERIFIED`로 표시한다. 공개 개인기록도 canonical 공개 스냅샷을 만들 수 있지만 `BOXTIER VERIFIED`로 표시하지 않으며, `visibility=private`인 개인기록은 공개 스냅샷을 만들지 않는다. `solo`와 레거시 `personal_record`는 모두 개인기록으로 판정한다. 양 팀 canonical ID가 모두 있는 확정 팀전만 팀 티어 선화 엠블럼을 표시한다. 생성된 모든 QR은 `/app/receipt?code={publicCode}`를 가리킨다. 공개 스냅샷 생성이 실패해도 Story·Feed 이미지 저장은 중단하지 않고 QR만 생략한다. 이 스냅샷은 누구나 조회할 수 있지만 안전 필드만 포함하고 원본 경기방의 조회·수정 권한은 바꾸거나 노출하지 않는다. 기존 canonical draft가 raw `match:{id}` seed를 가진 경우 공개 조회·복제 전에 원본 경기의 현재 공개 조건을 다시 검증하고 응답 seed를 HMAC 값으로 치환한다. 직접 작성·복제 draft는 verified·canonical 팀전 상태를 만들 수 없고 공식 전적이나 MMR에 반영하지 않는다. 외부 지도 URL과 소유권 비밀은 QR에 포함하지 않으며 웹 미리보기의 QR 영역도 같은 조회 대상으로 이동한다. 중앙 개인 스탯은 확정된 배정 심판 경기·대회의 현재 사용자 검증 스탯 또는 현재 사용자가 작성한 `personal_record`의 본인 스탯이 있을 때만 표시한다. 이때 현재 사용자는 인증 공급자의 계정 ID가 아니라 경기 `playerStats`와 같은 canonical 프로필 ID로 식별한다. 일반 무심판 경기와 타인이 작성한 개인 기록은 `GAME INFO`를 유지하며, 공개 스냅샷 생성 가능 여부와 개인 스탯 표시 판정은 분리한다.
...
7. 개인 티어는 실제 인증 세션이 있는 현재 로그인 사용자의 canonical 통합 MMR로만 계산한다. 프로필 해시태그는 같은 사용자의 canonical 해시태그만 사용한다. 두 값이 모두 있을 때만 개인 티어 워터마크·`MY TIER` 문구와 일련번호 아래 프로필 해시태그를 함께 표시하며, 개인 스탯이 있으면 수치도 함께 표시한다. 익명·공개 공유 응답에는 개인 티어와 프로필 해시태그를 모두 포함하지 않는다. 이 표시는 경기 결과·MMR을 변경하지 않는다.
8. 영수증 표시 언어는 한국어를 기본값으로 하는 화면 전용 상태이며 URL의 `lang=en`으로 선택한다. 이 값은 새로고침과 영수증 공유·제작 링크에는 유지하지만 경기 원본·draft·공개 스냅샷에는 저장하지 않는다. 영어 선택 시 영수증 편집 문구·선택지·검증 안내·상태·팝업과 영수증 route의 데스크톱·모바일 앱 메뉴·공용 푸터만 영어로 표시하고, 다른 route로 이동하면 앱 기본 언어인 한국어를 사용한다. 팀명·장소·대회명·코멘트 같은 사용자 입력과 canonical 경기 값은 번역하지 않는다. 한국어 화면의 경기 장소는 지도 선택 결과만 허용하고, 영어 화면은 국내 지도 선택기를 숨긴 채 기존 짧은 장소 입력을 `Venue` 직접 입력으로 사용한다. 지도에서 구장을 선택하면 구장명과 내부 원주소를 보존한다. 자유 표기는 짧은 장소에만 저장하며 지도 검색 지역 판정에는 사용하지 않는다. 경기 장소와 짧은 장소 중 하나만 있으면 위치 필수값을 충족한다. 영수증에는 짧은 장소가 있으면 그것만 표시하고, 비어 있으면 구장명을 표시하며 전체 원주소는 출력하지 않는다. 좌측 상단 일련번호와 경기기록 해시태그는 동일한 `#BT-########` 공개 코드를 표시한다. 공개 코드는 권한이나 소유권 키가 아니며 최초 DB 생성 뒤 변경하지 않는다. 기존 경기의 canonical 공개 draft는 경기 공개 코드를 상속하고, 익명 draft를 경기로 승계하면 그 공개 코드를 경기에도 그대로 사용한다. 사용자가 `새 영수증 만들기`를 실행하면 기존 draft를 수정하지 않고 새 공개 코드를 받는다.
9. 확정 경기의 공용 경기방 모달은 `영수증 발급`을 노출하고 canonical match ID를 `/app/receipt?match={matchId}`로 전달한다. 프로필 기록 등 진입 화면마다 별도 모달이나 경기 계산을 복제하지 않는다.
10. 한 줄 코멘트는 canonical 경기에서도 사용자가 직접 편집하는 선택 입력이며 경기 메모를 자동 복사하지 않는다. 입력 중에는 끝 공백을 보존해 띄어쓰기를 허용하고, 저장·출력 시 태그·제어문자 제거와 연속 공백 정규화를 거친 하나의 최대 22자 값을 공유한다. `BOXTIER SCORE`는 약 11자 기준 최대 2줄, `CLASSIC THERMAL`은 결과 패널 한 줄로 표시하며 비어 있으면 코멘트 행 전체를 출력하지 않는다. 개인 티어가 있으면 티어를 별도 세 번째 내용 행으로 배치한다.
11. canonical 경기 방식 `mode`는 정원·MMR 호환을 위해 `1v1`, `2v2`, `3v3`, `5v5`를 유지한다. 생성 화면에서 `3x3`을 선택하면 `mode=3v3`, `rules.ruleSet=fiba_3x3`과 기본 1구간 10분·21점 또는 시간 종료·2점 차 승리·6호 공을 저장한다. 시간·목표 점수·구간·공 규격을 직접 수정하거나 다른 시계 프리셋을 골라도 선택한 `ruleSet`은 유지한다. 일반 `3v3`의 21점 프리셋도 `3v3`을 유지한다. 방·계약·목록·대회·기록·영수증의 표시와 필터는 같은 공용 helper로 구분하고 사용자 화면에는 `FIBA 3x3`을 쓰지 않는다. 명시적인 다른 `ruleSet`이 있으면 항상 일반 `3v3`이며, 마커가 전혀 없는 기존 경기만 원본 규칙의 1구간·12분·21점·21점 또는 시간 종료·2점 차 승리 조건을 모두 만족할 때 표시 호환상 `3x3`으로 해석한다. 정규화가 채운 기본값은 이 레거시 판정의 증거로 사용하지 않는다.
12. 대회·리그 이름은 선택 표시값이다. 구간별 점수는 `match_results.period_scores`의 순서 있는 `[{label, scoreA, scoreB}]`를 canonical 원본으로 사용하고, 양 팀 값을 쌍으로 입력하며 합계가 canonical 최종 점수와 정확히 일치할 때만 결과와 함께 저장한다. 구간 점수는 비어 있어도 된다. 진행 중 점수 증감은 서버 경기시계의 현재 구간에 자동 누적한다. 구간을 확정할 수 없거나 합계가 맞지 않으면 구간 점수만 `[]`로 비우고 최종 기록에서 확인하도록 한다. 종료 후 이의제기·결과 수정에서 총점이 바뀌면 검증된 새 구간 점수를 함께 제공해야 하며, 없으면 기존 구간 점수를 `[]`로 비운다. `match_record` 생성과 개인 기록 변환은 `getMatchRulesPayload()`의 canonical 규칙을 그대로 보존한다. `5v5` 기본 기록은 4구간, 일반 `3v3`은 1구간이며 3x3 규칙 프리셋에서 변환된 기록은 `rules.ruleSet=fiba_3x3`을 잃지 않는다. 클라이언트 구간 라벨과 DB 검증 라벨은 이 동일한 저장 규칙을 사용한다. 비회원·레거시 영수증의 수동 구간 점수는 canonical 값이 없을 때만 표시 fallback으로 사용하고 경기 진행 상태, 대회 결과, 개인·팀 MMR을 변경하지 않는다.
13. 영수증 편집 화면은 Team A·B별 로컬 사진을 공용 원형 크롭 팝업에서 중심·확대·가로·세로 위치 조정한 뒤 이번 영수증에만 적용할 수 있다. `BOXTIER SCORE`는 결정론적 로컬 선화 변환 확인본을 적용하고, `CLASSIC THERMAL`은 크롭 원본의 투명 배경을 유지한 채 알파 전경만 D 써멀 `18·70·124·176`의 4단계 회색조로 양자화한다. 엠블럼 밖 종이·본문·사진·선·패널은 연속 회색 농도를 보존한 최종 회색조로 출력한다. 원본과 변환 결과는 현재 화면 메모리에만 두고 DB·object key·guest 업로드·draft·공개 스냅샷·R2·Storage·IndexedDB에 저장하지 않으며 초기화·새로고침 시 폐기한다. 로그인 후 팀을 만들고 팀 상세의 `영수증 엠블럼 만들기`에서 저장한 canonical 팀 자산은 Team A·B별 `저장 엠블럼 사용`으로 다음 영수증에서도 재사용한다. 팀 상세는 같은 공용 원형 크롭·자동 변환·확인 절차와 서버 검증을 거쳐 `teams.receipt_emblem_key`·`receipt_emblem_updated_at` 및 `team-emblems/{teamId}/` R2 key를 관리한다. 영수증 화면은 팀이 없거나 저장본이 없으면 로그인·팀 생성 경로를 안내하며 임의 클라이언트 object key와 guest 엠블럼 key를 draft에 보존하지 않는다.
   편집 중 엠블럼 resolver는 선택 스타일의 로컬 선화 또는 로컬 원형 크롭을 우선하고, 없으면 선택한 canonical 팀 저장본을 사용한다. 공개 스냅샷은 사용자가 고른 적용 상태와 `teams.receipt_emblem_key`만 함께 보존해 QR 재조회·새로고침에서도 canonical 팀 영수증 엠블럼을 재현한다. 기록 영수증은 현재 팀의 `receipt_emblem_key`를 우선 사용하고, 팀 목록 로딩 전에는 공개 스냅샷 또는 draft에 보존된 canonical key로 복원한다. 사용자가 선택한 `homeUseLineArt`·`awayUseLineArt` 상태가 참일 때 미리보기와 저장 Canvas에 같은 스타일별 resolver 결과를 반영한다. 실제 canonical 팀의 저장 엠블럼은 티어 엠블럼을 대체하지만 팀 티어 문구는 유지하고, canonical 팀이 없는 무기명 영수증은 팀 티어 문구를 비운다. 선화 결과는 투명 정사각 영역 안에서 비율을 보존해 정규화하고 선화·저장본·티어·중립 엠블럼은 동일한 표시 영역을 사용한다.
   영수증 전용 엠블럼 재업로드 이력은 일반 팀 대표 엠블럼과 분리된 `receipt_emblem_uploaded_at`·`receipt_emblem_upload_count`로 관리한다. 최초 2회는 즉시 변경할 수 있고 3회차부터 마지막 영수증 엠블럼 업로드 기준 30일 뒤에 변경할 수 있으며, 삭제는 이력을 초기화하지 않는다.
14. 홈 통합검색과 신고 경기검색은 같은 경기 공개 코드 정규화·조회 helper를 사용한다. 공개 코드 조회는 공개 가능한 경기방을 우선해 공용 방 모달로 연결하고, 연결 경기방이 없으면 만료되지 않은 익명 공개 영수증을 연다. canonical 경기 스냅샷은 원본 경기방이 공개 조회 대상이 아닐 때 익명 영수증 fallback으로 노출하지 않는다. 신고 검색은 공개 코드를 찾더라도 기존 신고 가능 경기·기간·권한 범위를 통과한 경기만 선택 결과에 포함한다.

15. 브라우저 편집 화면은 AI 선화 변환을 앱에서 직접 호출하지 않는다. `BOXTIER SCORE`의 엠블럼 편집 팝업에서만 사용자가 외부 도구에 붙여 넣을 프롬프트를 복사할 수 있으며, 원형·비율·글자를 유지한 고대비 단색 선화와 완전 투명 alpha PNG를 우선 요구한다. alpha 출력이 불가능한 도구에만 균일한 순수 `#00FF00` 배경을 보조 조건으로 허용한다. 브라우저로 반환한 이미지는 다시 로컬 기계 변환과 사용자 선택을 거쳐야 하며 자동 적용하지 않는다. `CLASSIC THERMAL`에는 이 action을 노출하지 않는다.

16. 영수증 입력 화면은 하나의 canonical draft만 사용하고 `receiptStyle`은 미리보기·내보내기 renderer만 바꾼다. `receiptLocale`, `includePhoto`, 사진 변환값과 하나의 `receiptComment`를 로컬 draft·공개 snapshot·복제·claim 경로에 공통 보존하며 기존 `comment`와 동기화한다. 짧은 팀명 입력은 두지 않고 양 출력 모두 전체 팀명을 한 줄 영역 안에서 축소해 사용한다. 참가 인원 입력도 두지 않는다. 공식 기록은 후보를 포함한 canonical 실제 참가자의 고유 프로필 수를 사용하고, 직접 작성은 경기 방식의 양 팀 정원 합계(`1v1=2`, `2v2=4`, `3v3/3x3=6`, `5v5=10`)를 사용한다. 사진 원본과 화면 메모리의 임시 엠블럼은 공개 payload에 포함하지 않는다. 공식 경기 ID·점수·구간 점수·참가 인원·심판 배정·검증 여부는 trusted canonical 경로에서만 저장하며 사용자가 `VERIFIED`를 만들 수 없다.
17. `CLASSIC THERMAL` Story와 Feed는 같은 정규화 데이터와 결정론적 Canvas 레이아웃을 사용한다. 사진 제외는 사진 포함 레이아웃에서 정확히 288px을 제거한다. 구간 점수가 없으면 표는 제거하되 경기 방식 바로 아래의 본문 마스크 점선 구분선과 결과 패널 전 최소 간격은 유지하고, 한줄평이 없으면 해당 행과 종이 높이를 제거한다. Story는 현재 종이 경계만 내보내 배경을 포함하지 않으며 사진이 없으면 출력 Canvas 높이도 종이와 함께 줄인다. 찢긴 상·하단의 종이 실루엣 바깥 픽셀은 alpha `0`으로 내보내고 검정·숯색을 칠하지 않는다. 종이 내부의 미색과 섬유 질감은 불투명하게 유지한다. Feed는 숯색 배경과 좌우 여백을 유지한 채 종이 전체를 축소해 표시하며 어느 영역도 크롭하지 않는다. QR은 인쇄된 `MATCH NO.`·`GAME ID`와 동일한 공개 코드를 쿼리에 넣은 정확한 `/app/receipt?code=...` 공개 영수증 URL만 담는다. 공개 URL이 없으면 일반 영수증 페이지를 대신 넣지 않고 QR 영역을 출력하지 않는다. quiet zone은 깨끗하게 유지하고 외곽선과 모듈에만 스캔 가능성을 해치지 않는 결정론적 감열 마스크를 적용한다.
18. `CLASSIC THERMAL` 영문 모드는 해당 영수증 페이지의 사이드바·하단 메뉴·hero·입력·상태·내보내기 action까지 영어로 전환한다. 영문 모드의 짧은 `Venue`는 출력용 표시값이며 국내 지도 검색·주소 정규화·지역 판정의 근거로 사용하지 않는다.
19. 외부 AI·자동화의 공유와 이미지 출력은 분리한다. `POST /api/match-receipts/create`는 사진·엠블럼 없는 익명 공개 영수증 JSON draft만 만들고 공개 코드, 만료 시각, 안전 공개 투영, 영수증 경로와 공개 JSON 경로를 반환한다. 엠블럼 입력은 저장하지 않고 거부한다. `POST /api/match-receipts/render`는 전용 Bearer 키가 있는 서버 호출만 받고 `image/png` 바이트를 직접 반환한다. PNG 경로는 `publicCode`·DB row·Supabase Storage·R2 객체를 만들지 않으며 요청 데이터와 변환 이미지를 메모리에서만 사용하고 응답 뒤 폐기한다. 두 경로 모두 `style`, 양 팀명, 양 팀 최종 점수, 실제 ISO 경기 날짜, 짧은 장소, 경기 방식을 필수로 하고 대회명, 구간 점수, 한줄평, 경기 성격, 언어, 경기 시각을 선택으로 받는다. `style`은 `boxtier-score` 또는 `classic-thermal`을 호출자가 명시해야 한다. 사진 URL·자산 ID, MMR, verified 입력은 받지 않고 항상 `includePhoto=false`, `verified=false`다. 구간 점수는 양 팀 쌍·허용 라벨·중복 금지·최대 5개·최종 점수 합계 일치를 모두 검증한다. PNG 경로의 선택 엠블럼은 `homeEmblem`·`awayEmblem` 각각 정확히 `{ imageBase64 }` 형태의 raw Base64 WebP로 받고 `data:` URL·외부 URL·object key를 거부한다. 호출자가 투명 `1:1`, 최대 `320×320px`, `96KB`의 처리본을 준비하며 서버는 container·크기·정사각형을 검증하고 canonical 엠블럼 영역에 contain·중앙 정렬한다. 선택 엠블럼이 없으면 용도별 고정 중립 엠블럼을 사용한다. 서버는 웹 미리보기·Story·Feed와 같은 Canvas ViewModel·레이아웃·폰트·에셋 renderer를 사용하며 별도 조판이나 후처리 renderer를 두지 않는다. `classic-thermal`은 엠블럼의 투명 배경을 유지하고 알파 전경만 D 써멀 `18·70·124·176` 4단계 회색조로 변환하며, 종이·본문·선·패널의 연속 회색 농도는 보존한다. preset 크기와 크롭·여백 규칙도 17번의 canonical 웹 출력 규칙을 그대로 따른다. 요청 본문은 최대 320KB, 한 문자열은 최대 140,000자이며 인스턴스당 동시 렌더는 2개로 제한하고 초과 시 `429`를 반환한다. 생성 서버 부하는 저장 I/O 없이 요청당 이미지 합성 CPU·메모리만 사용하며 전역 과부하 방지는 상위 API rate limit·WAF가 맡는다. 경기 사진을 외부 API로 받으려면 용도별 canonical 업로드 저장소, 파일 검증, 수명주기, 삭제, 렌더링 정책을 먼저 별도 확정해야 한다.
20. 공개 `POST /mcp`는 Streamable HTTP MCP 서버이며 `search`, `fetch`, `create_basketball_receipt`, `get_my_boxtier_account`, `list_my_match_records` 도구를 제공한다. `search`는 `user_room_feed`의 `recruiting` 중 `public`·`region_public`·`open`·`is_active=true`인 canonical 공개 모집 피드와 `room_feed_cards`의 공개 카드만 읽고 ID·제목·canonical 참가 URL을 반환한다. `fetch`는 검색 결과 ID의 같은 공개 모집 조건과 카드 상태를 다시 검증한 뒤 지역·방식·일시·정원 현황·참가 방식·경기 성격·심판 모집 여부만 반환한다. 두 조회 도구는 사용자가 실제 참가방·픽업 경기·팀 대 팀 상대를 찾거나 조건별 매칭방 추천을 요청할 때만 선택하며 일반 농구 지식, NBA 정보, 훈련법, 의료·상거래 등 무관한 질문에는 선택하지 않는다. `create_basketball_receipt`는 19번의 입력 parser와 canonical 메모리 PNG renderer를 재사용하고 MCP `image/png` content를 직접 반환한다. 성공 결과의 canonical 원본 위치는 `content[0]`의 순수 Base64 PNG image 블록이다. 서버는 PNG signature, `IHDR`, 실제 양수 크기, `IDAT`, 파일 끝의 `IEND`를 모두 확인한 뒤에만 성공을 반환한다. `structuredContent`에는 output schema로 고정한 `status`, `mimeType`, `preset`, `style`, `byteLength`, 실제 `width`·`height`, `sha256`, `imageAttached=true`, `publicCode`, `receiptPath`, `downloadUrl`, `downloadExpiresAt`을 반환한다. 수신자는 `content[0].data`를 raw Base64로 복원하고 길이·크기·해시를 이 메타데이터와 대조한다. 같은 최종 PNG 바이트 한 장만 비공개 R2의 `temporary/match-receipts/`에 임시 저장하고 HMAC 서명된 만료 다운로드 URL로 제공한다. URL 기본 유효시간은 10분이며 60초 이상 1시간 이하로만 설정하고, 객체는 R2 lifecycle로 최대 1일 뒤 삭제한다. 다운로드는 opaque 256비트 ID·만료 시각·서명을 모두 검증하고 `private, no-store` 첨부로 같은 바이트를 반환하며 재렌더하지 않는다. 첨부 엠블럼 원본·정규화 결과는 DB·R2·Storage에 저장하지 않고 응답 뒤 폐기한다. QR 재조회를 위해서만 기존 공개 영수증 수명주기를 재사용해 안전 투영된 JSON draft를 `match_receipt_drafts`에 30일 보존하며, 발급된 하나의 `publicCode`를 인쇄된 `MATCH NO.`·`GAME ID`, QR 대상 URL, `structuredContent.publicCode`·`receiptPath`에 공통 사용한다. 이미지 표시 전 완료로 오인될 수 있는 text content와 완료 문구는 반환하지 않는다. 공개 tool input schema는 선택 boolean `debugBase64`를 `default=false`로 노출한다. `debugBase64=true`일 때만 개발 확인용 raw Base64를 `structuredContent.base64`에도 추가하며 일반 응답에는 중복하지 않는다. 공개 입력의 `thermal`·`score`는 각각 canonical `classic-thermal`·`boxtier-score`로 매핑한다. 선택 엠블럼은 기존 `homeEmblem`·`awayEmblem`의 JPG·PNG·WebP 순수 Base64 입력과 ChatGPT 공식 `_meta["openai/fileParams"]`가 주입하는 `homeEmblemFile`·`awayEmblemFile` 첨부 입력을 모두 지원한다. 첨부의 임시 HTTPS URL은 공인 IP만 허용하고 redirect 없이 최대 8초·3MB로 메모리에 읽는다. 서버는 실제 바이트 형식, 최대 3MB·2천만 픽셀·단일 프레임을 검증하고 요청 메모리에서 투명 `320×320` WebP·최대 `96KB`로 비율 유지·중앙 정렬 정규화한 뒤 폐기한다. 엠블럼이 없으면 용도별 고정 중립 엠블럼을 사용한다. `thermal`에서는 공용 renderer가 실제·중립 엠블럼의 알파 전경만 D 써멀 4단계 회색조로 변환하고 투명 배경과 나머지 PNG의 연속 회색 농도는 보존한다. 팀명·최종 점수·경기 날짜·장소·경기 방식은 사용자가 제공한 실제 값이어야 하고 누락값을 추측하지 않는다. 검증 오류는 구조화된 `status=error`와 필드별 issue를 반환하고 PNG 생성 성공은 `status=rendered`로 구분한다. 쿼터 합계 불일치와 농구 외 경기·상거래 영수증 요청은 거부한다. 모든 공개 MCP POST 요청은 IP별 인스턴스 메모리 기준 1분 5회로 제한한다. parser를 통과한 `create_basketball_receipt` PNG 생성 시도는 엠블럼 다운로드·이미지 변환보다 먼저 Supabase 운영 원장에서 비로그인은 salted SHA-256 IP 해시별, 로그인 사용자는 salted SHA-256 canonical 프로필 해시별 최근 24시간 10회로 제한해 외부 URL·CPU 남용을 막는다. 원본 IP와 영수증 입력은 원장에 저장하지 않는다. `initialize`, `tools/list`, 입력 검증 실패는 일일 횟수에서 제외하고 엠블럼 처리·렌더링 실패는 유효한 생성 시도로 포함한다. quota 원장을 확인할 수 없으면 엠블럼 처리·렌더링을 하지 않고 오류를 반환하며 전역 외곽 방어는 WAF가 맡는다. ChatGPT·Claude의 자동 도구 선택은 사용자가 승인된 BoxTier 앱 또는 사용자 지정 MCP connector를 먼저 설치·연결한 범위에서만 동작한다.

20-1. MCP `create_basketball_receipt`는 UI resource를 `ui.resourceUri`나 `openai/outputTemplate`로 연결하지 않는 tool-only 출력이다. 표준 `content[type=image]`에는 양쪽 엠블럼까지 합성된 BoxTier API의 최종 PNG 원본 한 장만 반환하며 일반 호출의 `structuredContent`와 result `_meta`에는 Base64를 중복하지 않는다. 수신자는 BoxTier API가 반환한 PNG 원본만 사용자에게 전달하고 재합성·재렌더·임의 편집하지 않는다. 서버는 v2·v3·v4를 포함한 어떤 영수증 UI resource도 등록하지 않는다. 과거 descriptor를 캐시한 클라이언트는 배포 후 도구 연결을 해제·재연결하거나 재스캔해야 하며, 캐시된 위젯 URI는 더 이상 제공하지 않는다. 도구 descriptor는 `openai/toolInvocation/invoking`도 제공하지 않아 BoxTier 전용 로딩 문구를 만들지 않는다. 단, 클라이언트 기본 도구 실행 UI는 클라이언트 소유이므로 서버가 숨김을 강제하지 않는다. tool 설명은 모델이 별도 위젯을 열거나 처리 중·완료 문구를 반복하지 않고, metadata만 보고 완료 처리하지 않으며 반환된 image content를 사용자에게 직접 제시하도록 지시한다. 서버 성공 조건은 실제 HTTP/SSE tool result에 raw Base64 `image/png` 블록이 직렬화되고 PNG 길이·크기·해시 검증값이 일치하며 동일 바이트의 만료 다운로드 URL이 발급되는 데까지다. 네이티브 대화 이미지 첨부 여부는 MCP 클라이언트가 결정하지만 ChatGPT·Claude 등 MCP 클라이언트는 `downloadUrl`로 원본 파일을 일관되게 받을 수 있다. `debugBase64`는 명시적으로 `true`인 개발 호출에서만 Base64를 중복하는 확인 옵션이며 image content 생성·전달 조건으로 사용하지 않는다.

20-2. 공개 MCP의 `create_basketball_receipt`는 비로그인과 로그인 사용자 모두 사용할 수 있다. 비로그인은 salted SHA-256 IP 해시, 로그인 사용자는 salted SHA-256 canonical 프로필 해시를 일일 원장 key로 사용하고 각각 최근 24시간 10회를 적용한다. 유효한 OAuth Bearer를 canonical 프로필로 확인하고 `rankball_admin_level_for_profile`이 `100`인 owner 계정에만 일일 원장 확인을 면제한다. 다른 관리자와 일반 사용자는 면제하지 않으며 IP별 1분 제한은 owner에게도 적용한다. Bearer가 없으면 익명으로 처리하지만 잘못되거나 만료된 Bearer가 있으면 로그인 challenge를 반환한다. 익명 일일 한도 초과도 로그인 후 사용자 한도로 계속할 수 있도록 OAuth challenge를 반환하고, 로그인 일반 사용자의 한도 초과에는 재로그인을 요구하지 않는다. `get_my_boxtier_account`와 `list_my_match_records`는 `profile` scope 로그인이 필수이며 조회 대상 프로필 ID를 입력으로 받지 않는다.

20-2-1. MCP 첨부 엠블럼 정규화는 투명 캔버스 크기가 아니라 알파 전경의 실제 경계와 가중중심을 canonical 기준으로 사용한다. 불투명 입력이 캔버스를 채우면 알파 경계의 지배적인 균일 흰색·회색 배경판을 최대 2단계 제거한 뒤 실제 도안만 전경으로 판정한다. 배경과 도안을 안전하게 분리할 수 없으면 `emblem_background_not_removable`로 거부하며 배경 전체를 작은 엠블럼으로 축소하지 않는다. 전경 비율을 유지해 `320×320px` 투명 캔버스의 중심에 맞추고 모든 전경 픽셀이 반지름 `138px` 원형 안전영역 안에 들어오게 축소한다. `classic-thermal`은 배경과 원판을 투명하게 유지하고 전경 명도 분포만 D 써멀 `18·70·124·176` 4단계로 변환한다. 이 규칙은 가로·세로로 치우친 복합 엠블럼에도 동일하게 적용하며 변환 결과는 요청 메모리에서만 사용한다. 최종 영수증 렌더러는 이 준비 결과의 픽셀을 재양자화하거나 열화하지 않고, 실제 알파 전경을 동심원 내부에 잘리지 않는 최대 크기로 다시 중앙 정렬한다. 감열 마스크는 엠블럼 자체가 아니라 동심원 테두리에만 적용한다.

20-3. `list_my_match_records`는 `profile` scope의 Supabase OAuth 인증이 필수인 읽기 전용 MCP 도구다. Tool metadata와 OAuth 동의에서 `profile` scope를 요구하고, 서버는 Bearer를 Supabase Auth로 검증한 뒤 Auth 사용자와 canonical 프로필을 연결한다. Supabase OAuth access token JWT에 `scope` claim이 항상 포함된다고 가정하지 않는다. `match_record_participants.profile_id`가 그 프로필과 정확히 일치하는 보존 기간 내 compact 기록만 최신순으로 반환하며 조회 대상 프로필 ID는 tool input으로 받지 않는다. 인증 실패는 보호 리소스 메타데이터를 포함한 `mcp/www_authenticate` challenge로 반환한다.
...
21. Instagram 영수증 봇은 공식 Webhook의 서명 검증을 통과한 텍스트 DM만 처리한다. 입력은 `영수증` 명령과 명시적 필드 형식을 사용하고 19번의 외부 입력 parser·PNG renderer를 재사용한다. Meta 이벤트 ID, 발신자 ID, 정규화 본문은 서버 비밀키 HMAC 해시로만 저장하며 원문과 원 ID는 저장하지 않는다. `연결`은 256비트 일회용 capability를 발급하되 DB에는 그 HMAC만 저장하고, 로그인 뒤 `/api/instagram/account`가 Bearer의 Auth 사용자에서 서버가 확정한 canonical 프로필만 연결한다. 연결하면 같은 발신자 해시의 기존 요약 기록과 제한 원장의 principal도 해당 프로필에 귀속해 연결 전후 사용량을 합산한다. `기록`은 연결 계정의 최근 5건, 로그인 연결 화면과 API는 최근 10건의 compact 요약만 반환하며 조회 프로필 ID를 클라이언트에서 받지 않는다. 영수증 요청은 연결 전에는 발신자, 연결 뒤에는 canonical 프로필 principal 기준으로 하루 `INSTAGRAM_BOT_DAILY_LIMIT=10`을 적용한다. 반복 방지는 영수증 요청에만 principal 기준 30초 쿨다운·시간당 5회, 전체 요청에 시간당 100회, 같은 발신자의 같은 정규화 본문 5분 중복 차단을 적용한다. `연결`·`기록` 명령은 영수증 쿨다운·시간당·일일 한도를 소모하지 않고 이벤트 중복과 전체 시간당 한도만 적용한다. 이벤트 해시 원자 예약과 모든 제한은 Supabase RPC 하나에서 판정한다. 수락 이벤트는 처리 lease와 최대 3회 시도 상태를 저장해 일시적 Graph·Supabase·렌더 실패를 같은 이벤트에서 재시도하며 완료 이벤트만 영구 중복으로 차단한다. 사용자 영수증 중복·쿨다운·시간당·일일 제한에는 원인을 답장하고 내부 이벤트 중복·처리 중·재시도 소진·전체 과부하는 답장과 렌더를 실행하지 않는다. Instagram이 가져갈 공개 이미지 URL에는 이벤트 HMAC에서 결정론적으로 만든 별도의 opaque capability ID만 넣는다. Webhook은 최종 PNG를 한 번만 렌더하고 Meta의 8MB 제한에 맞춘 정확한 최종 바이트를 비공개 R2의 `temporary/match-receipts/`에 저장한 뒤 `instagram_receipt_bot_render_jobs`에는 ID와 만료 시각만 보존한다. 같은 이벤트 재시도에서 만료되지 않은 작업이 있으면 기존 R2 객체를 재사용하고 재렌더하지 않는다. 공개 이미지 endpoint는 DB 만료를 확인한 뒤 같은 R2 바이트를 읽어 반환하며 렌더 입력을 저장하거나 요청 시 재렌더하지 않는다. 이는 19번의 직접 PNG API 메모리 전용 규칙을 변경하지 않는 Instagram 전달용 예외다. R2 객체는 lifecycle로 최대 1일 뒤 삭제하고 공개 endpoint의 논리 만료는 24시간을 넘기지 않는다. 기록에는 검증·정규화된 영수증 요약만 저장하고 PNG 바이트·사진·엠블럼 원본은 DB·Supabase Storage에 저장하지 않으며 엠블럼 원본·정규화 결과는 요청 뒤 폐기한다. 만료 렌더 작업·연결 코드는 삭제하고, 제한 메타데이터는 7일, 미연결 기록은 30일, 연결 기록은 최대 1년 보존한다. Instagram 검증 GET의 `hub.verify_token` 쿼리만 해당 route·method에서 전역 URL credential 차단의 프로토콜 예외로 허용하며 POST와 다른 token 쿼리는 계속 거부한다. 검증 GET은 verify token만 요구하고, POST 런타임 설정은 로그인 방식에 맞춘 allowlist Graph API base URL을 사용한다.
...
2. 랜딩의 외부 로그인은 기존·신규 계정을 구분하지 않고 완료 후 `/app`으로 이동한다. 로그인 복귀 경로는 검증된 내부 경로만 허용한다.
3. 비회원 영수증은 로그인 전 공개 draft와 로컬 입력을 유지한다. 원 작성자는 capability 기반 claim 뒤 작성 화면으로 복귀하고, 공유 영수증 수신자는 새 공개 draft를 복제한 뒤 자신의 기록으로 이어간다. QR만으로 원본 소유권을 이전하지 않는다.
4. 인증 실패 시 랜딩 또는 현재 영수증 화면에 오류만 표시하고 작성 중 데이터는 삭제하지 않는다.
5. 보호 route가 로그인 화면으로 전환될 때는 쿼리·해시를 포함한 원래 보호 경로를 `returnTo`로 보존하고 route gate 상태를 함께 넘긴다. 이 상태에서 로그인 화면의 뒤로가기는 브라우저 history를 사용해 보호 route 진입 직전 위치로 정확히 복귀한다. 보호 action이 로그인 화면을 직접 열 때는 실행 후 복귀할 `returnTo`와 현재 화면인 `backTo`를 분리해 보존하며, route gate 상태가 없을 때만 검증된 `backTo` 또는 `/app`을 사용한다.

## 2026-08-14 공개 경기 영수증 QR 입력 복제

1. 공개 코드가 경기방이 없는 익명 draft로 해석되면 내부 `/app/receipt?draft={publicId}` 경로로 전환한다. 공개 영수증은 사진을 제외한 공개 payload의 팀명, 점수, 날짜, 경기 방식, 경기 성격, 장소, 코멘트, 대회명, 구간 점수를 영수증 입력 초안에 채운다. 수신자는 이 값을 로컬에서 편집할 수 있지만 원본 공개 draft는 변경하지 않는다.
2. 수신자가 값을 수정한 뒤 영수증을 완성하거나 상세 기록으로 전환하면 수정된 값으로 새 익명 draft와 capability를 발급한다. 수정하지 않은 경우에는 기존 공개 payload를 서버에서 복제한다. 사진, 개인 MMR, 프로필 해시태그는 복제 대상이 아니다.
3. 공개 ID는 DB 조회 전에 UUID 형식과 길이를 검증한다. 공개 조회는 IP 원문을 저장하지 않는 요청 해시 기준 시간당 120회로 제한하며, 없는 ID와 형식 오류는 동일한 비노출 응답을 사용한다.
4. 원 작성자의 공개 draft 편집 저장은 draft별 capability를 확인하고 같은 `publicId` 행을 갱신한다. 클라이언트는 저장 요청을 직렬화하고 최신 revision까지 순서대로 반영해 고아 draft, 생성 quota 낭비, capability 응답 순서 충돌을 만들지 않는다. canonical 경기 원본 필드는 편집 저장으로 바꾸지 않는다.
...
1. 영수증 전용 엠블럼의 영구 canonical 소유자는 팀이다. 팀 상세에서 저장한 `teams.receipt_emblem_key`만 새로고침·다른 영수증·공개 공유에서 재사용한다. 영수증 편집기는 현재 브라우저에서 만든 로컬 선화를 미리보기와 Story·Feed Canvas에만 일시 적용하며 영구 자산으로 승격하지 않는다.
2. 브라우저 편집 화면의 로컬 엠블럼 Blob과 data URL은 현재 화면 메모리에서만 유지한다. 외부 PNG 생성 API의 사전 처리 WebP와 MCP 첨부 원본의 서버 정규화 결과도 요청 메모리에서 검증·배치·스타일 변환한 뒤 응답과 함께 폐기하며 DB·R2·Storage에 저장하지 않는다. 영수증 draft 생성·수정·공개 조회·복제·claim은 guest 엠블럼 입력이나 key를 저장·투영·승계하지 않고 공개 draft 생성 API는 엠블럼 입력을 거부한다. 만료 maintenance는 기존 운영 버전이 만든 안전한 draft 전용 R2 key가 있으면 객체를 먼저 정리한 뒤 만료된 draft 행을 삭제한다.
3. 편집 중 resolver는 현재 영수증의 로컬 선화를 canonical 팀 저장본보다 우선한다. 선화 사용 해제는 해당 영수증의 표시 상태만 끄고, 엠블럼 생성·교체·삭제와 업로드 제한 이력은 팀 상세의 canonical 팀 자산 수명주기만 따른다.
