...
1. 개인 엠블럼 표시 원본은 `initial`, `discord`, `upload` 중 하나다. `initial`은 이름 첫 글자와 선택한 기본색을 사용하고, `discord`는 연동된 Discord 프로필 이미지가 있을 때만 선택할 수 있으며, `upload`는 R2에 저장된 정사각 WebP를 사용한다.
2. 개인 엠블럼의 기본색, 테두리 사용 여부, 테두리색은 이미지 원본과 별개다. 팀도 엠블럼 기본색, 테두리 사용 여부, 테두리색을 별도로 저장한다.
3. 직접 업로드는 저장 전 정사각 미리보기에서 확대·축소와 가로·세로 위치를 사용자가 정한다. 서버는 최종 WebP의 형식, 크기, 최대 변 길이를 다시 검사한다.
4. 개인과 팀은 새 이미지 업로드를 각각 처음 2회까지 대기 없이 허용한다. 세 번째 새 이미지부터는 직전 성공 업로드 뒤 30일이 지나야 한다. 제한은 DB transaction lock 안에서 검사하며 클라이언트 시각만 믿지 않는다.
5. 기본색·테두리 변경과 `initial`/`discord` 표시 전환은 이미지 업로드 횟수에 포함하지 않는다. 기존 이미지를 삭제해도 누적 업로드 횟수와 마지막 업로드 시각은 초기화하지 않는다.
6. 팀 엠블럼 이미지·색상 변경은 팀장만 가능하다. 개인 엠블럼은 본인 프로필만 변경할 수 있다.
7. 이미지 교체, 기본 글자 전환, Discord 전환, 삭제, 색상 저장 전에는 결과와 교체 제한을 설명하는 확인 메시지를 표시한다.
8. 개인·팀 목록, 상세, hover, 검색, 홈, 시즌, 경기 화면은 공용 엠블럼 컴포넌트를 사용한다. 이미지가 없거나 로드에 실패하면 선택한 기본색과 이름 첫 글자로 대체한다.
...
10. 원본 이미지는 브라우저에서 최대 `384x384`, `160KB` 이하 정사각 WebP로 최적화한 뒤 서버로 보낸다. 서버는 인증, 팀장 권한, WebP 시그니처, 픽셀 크기와 바이트 크기를 다시 검사한다. 이 수치는 사용자 성공 메시지에는 노출하지 않는다.
11. DB는 공개 URL이 아니라 불변 Cloudflare R2 object key만 저장한다. 표시 URL은 공용 asset base에서 계산한다.
12. 엠블럼 저장은 팀 row transaction lock과 기존 key 비교를 사용한다. DB 반영 성공 뒤 이전 R2 object를 삭제하고, 실패하면 기존 엠블럼을 유지한다.
