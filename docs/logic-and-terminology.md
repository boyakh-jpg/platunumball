...
4. 동일 Google 계정은 원본 식별자를 저장하지 않는 해시 tombstone으로 탈퇴 후 7일 동안 프로필 재생성을 차단한다.
...
4. 공유 영수증 수신자가 `상세 기록 이어서 작성`을 선택하면 현재 공개 payload를 새 draft와 새 capability로 복제한 뒤 Google 로그인·claim·기존 `personal_record` 작성 흐름으로 이어간다. 원 작성자의 draft는 변경하지 않는다. 생성 성공 뒤 실제 경기 ID의 `/app/receipt?match={matchId}`로 복귀하고 내 기록에 표시한다.
...
2. 랜딩의 Google 로그인은 기존·신규 계정을 구분하지 않고 완료 후 `/app`으로 이동한다. 로그인 복귀 경로는 `/app`으로 시작하는 내부 경로만 허용한다.
