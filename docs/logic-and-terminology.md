...
1. 로그인 사용자는 일반 게시글·댓글·1단계 답글을 작성하고 자기 글과 댓글을 수정·소프트 삭제할 수 있다. 운영 레벨 `30` 이상은 공지 작성·고정과 전체 글·댓글 삭제가 가능하다.
2. 게시글 추천은 사용자당 글마다 1회만 저장한다. 인기글은 최근 3일 안에 작성된 자유·질문 글 중 `추천 수 × 3 + 공개 댓글 수`가 높은 순으로 최대 5개를 표시하고 동점이면 최신 글을 우선한다.
...
5. 게시판은 텍스트만 지원한다. 이미지, 익명 글, 댓글 추천, 2단계 이상 답글은 운영 필요가 확인되기 전까지 추가하지 않는다.
...
10. 게시판 분류는 `전체 / 공지 / 자유 / 질문`이다. 전체는 모든 공개 글을 포함하고 공지는 운영 레벨 `30` 이상만 작성한다.
