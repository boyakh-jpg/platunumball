...
1. 비로그인 게스트는 공개 앱 경로에서 체험 데이터를 조회할 수 있다. `/app`, 가이드·연습, 공개 경기·매칭, 커뮤니티, 시즌·랭크보드, 팀·선수·심판·구장·대회·소속 조회가 포함된다.
2. 게스트 매칭방은 읽기 전용이며 참가, 채팅, 초대, 방 수정, 확정, 취소를 실행하지 않는다.
3. 게스트 경로는 운영 사용자나 운영 방의 권한을 부여하지 않는다. 생성·참가·채팅·추천·신고·즐겨찾기·관리·개인 기록·알림·설정 등 모든 mutation과 개인 데이터는 로그인 인증을 계속 요구한다.
