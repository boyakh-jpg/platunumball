...
2. 지원 출전 정원은 사이드당 `1·2·3·5명`, 후보 정원은 `0~3명`이다. 현재 각 사이드의 출전 인원이나 후보 인원보다 정원을 작게 줄이는 수정은 클라이언트와 DB에서 모두 거부하며 기존 슬롯은 바꾸지 않는다.
3. 공개 개인전, 공개 팀전, 비공개 초대전, 현장 픽업 모두 모집방 수정 뒤 현재 참가 row, 참가 상태, A/B·출전·후보 배치를 유지한다. 모집방 규칙 변경을 이유로 참가자를 미확정 상태로 되돌리거나 재수락을 요구하지 않는다.
