...
23-6. `submitMatchResult`, `disputeMatch`, and `resumeMatchApproval` are sent from the frontend as operation-only. The server must reload authoritative match state and run the central reducer before persisting result rows, dispute rows, draft results, final approvals, and rating commits; client match snapshots are not accepted as the source of truth for these actions.
