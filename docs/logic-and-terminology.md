...
3. 현재 경기시계 담당자는 A/B 양 팀 점수를 조작한다. 배정 심판도 A/B 양 팀 점수를 조작한다. 경기시계를 사용하지 않는 무심판 경기만 방장이 A/B 양 팀 점수를 조작한다. 경기시계를 사용하고 담당자가 지정된 경기에서는 방장이라는 이유만으로 점수 권한을 얻지 않는다.
...
7. 신규 교체 사유는 `self`, `late`, `ejection`, `operator`만 허용한다. `injury`와 `recorder_handoff` 신규 저장은 거부하거나 `operator`로 정규화한다. 과거 `injury`는 화면에서 `일반 교체`로 읽을 수 있다.
8. 일반 경기의 최종 승인은 심판 유무와 관계없이 방장만 실행한다. 실행 직전 `더 이상 이의가 없음을 확인하셨나요?` 확인창을 거친다. 과반 승인, 전원 승인, 팀장 승인은 일반 경기에 적용하지 않는다.
9. 열린 이의신청이 하나라도 있으면 최종 승인을 거부한다. 마지막 이의 판정은 경기를 `approval`로 되돌릴 뿐 자동 확정하지 않는다. 최종 승인과 MMR 반영은 별도 서버 transaction·서버시간으로 수행한다. 최종 승인 뒤 신규 이의신청은 거부한다.
10. 일반 live 경기, `match_record`, `personal_record`는 서로 다른 기록 유형이다. `match_record`의 기존 참가자별 승인 규칙과 `personal_record`의 본인 소유 규칙은 일반 경기 helper·문구·최종 승인 규칙에 섞지 않는다.

### 대체된 규칙

- 기록자 점수 권한, 자동 기록자, 양면 점수 위임, 기록권한 승계·교대·takeover 규칙은 대체됐다.
- 기록자 변경을 교체 transaction에 포함하거나 `recorder_handoff` 교체 사유를 신규 저장하는 규칙은 대체됐다.
- 심판 경기에서 심판이 일반 경기 최종 승인을 수행하는 규칙은 대체됐다.
- 일반 경기의 과반 승인·전원 승인·팀장 승인과 마지막 이의 판정 즉시 자동 확정 규칙은 대체됐다.
...
1. 리그·토너먼트 주최자는 생성 시점부터 종료일까지 유효한 `자격심판(candidate)` 이상 자격과 신뢰도 90 이상을 가져야 한다. 별도 `후보심판`, `긴급 소속 심판` 역할은 만들지 않는다.
...
6. 예정 경기는 집결과 현장 변동을 위해 예정시각 10분 전부터 시작할 수 있다. 늦은 시작은 별도 마감으로 취소하지 않되, 동일 심판의 다른 live 경기가 있으면 시작을 막는다.
7. 예정시각은 약속과 심판 일정 충돌 판정에 사용한다. 출전시간, 경기 종료, MMR 검증은 실제 서버 경기시계 시작·일시정지·재개·종료 기록만 사용한다.

## 2026-07-25 충돌 제거와 구조 정리 로드맵

1. P0 실행 경로: 같은 상태 전이를 만드는 구형 action·RPC·권한 예외를 먼저 제거한다. 경기 이의는 `resolveMatchDispute`와 `rankball_match_resolve_dispute_action()`만 사용하며 방장이 각 open 항목을 판정한다. 마지막 판정 뒤에는 별도 최종 승인 확인을 거쳐야 경기 확정과 MMR commit이 실행된다.
...
1. QR 출석은 일반 공개 매칭에서만 방 생성 옵션으로 사용한다. 공개 경쟁전은 경기 전 구성·현장 픽업 모두 기본 활성화하고 공개 친선전은 기본 비활성화한다. 비공개·대회·경기시계 미사용 방에서는 강제로 비활성화한다.
...
11. 예정 경기의 공통 방 단계도 QR API와 같은 시작 10분 전부터 `checkin`으로 전환한다. 무심판 경기 방장은 경기 시작 권한자이므로 본인 출석은 시작 차단 대상에서 제외하고, 나머지 출전·후보 참가자의 미출석만 실제 인원수로 안내한다.
...
4. 경기 기록방은 `구성 방식·경기 인원·종료 일시·구장·메모`를 한 화면에서 입력하고 바로 확인·생성한다. 단계 nav를 만들지 않는다.
5. 경기 기록방은 사용자가 규칙을 입력하지 않으므로 일반 경기 기본 규칙을 만들어 저장하거나 방 모달에 표시하지 않는다. 구장은 검색해 선택할 수 있고 선택하지 않으면 `구장 미정`으로 저장한다.
6. 사후 기록방은 참가 사실 확인과 결과 확인을 참가자 본인의 `최종 승인` 한 번으로 처리한다. 최종 승인은 본인이 실제 참가했고 점수·개인 기록이 맞다는 확인을 함께 뜻한다. 방장, 팀장, 심판은 다른 사람을 대신해 승인할 수 없다.
7. 개인 구성 기록방은 `명단 제출 → 기록 입력 → 각 참가자 최종 승인` 순서다. 팀 구성 기록방은 `팀 선택 → 각 팀장 출전 명단 제출 → 기록 입력 → 양 팀 참가자 최종 승인` 순서다. 팀 선택·명단 제출과 최종 승인을 같은 상태로 표시하지 않는다.
8. 사후 기록방을 중단하는 행동과 알림은 `경기 취소`가 아니라 `기록 취소`로 표시한다. 예정 경기 취소·경기 무효와 구분한다.
9. 사후 기록은 결과 제출 직후와 5분 후에 확인 알림을 보낸다. 참가자는 본인 참가 사실과 결과를 한 번에 최종 승인하거나 이의를 제기한다.
10. 결과 제출 뒤 방에 설정된 10분·15분·20분 이의시간이 끝나고 열린 이의가 없으면 무응답자도 포함해 기록을 자동확정한다. 사후 기록은 항상 MMR·팀 MMR·경쟁 승패·연승에서 제외한다.
11. 자동확정 전까지는 승인 인원과 미응답 인원을 표시한다. 자동확정 뒤 불복은 기록 재승인이 아니라 경기 신고로 처리한다.
12. 참가 사실 또는 결과가 다른 참가자가 `결과가 다릅니다`로 이의를 제기하면 무응답으로 처리하지 않고 즉시 이의조정 상태로 전환한다. 이의가 열려 있는 동안 해당 기록의 개인 통계와 전적도 반영하지 않는다.
13. 이의조정은 방장이 각 요청을 가결 또는 부결한다. 마지막 열린 이의 판정과 함께 현재 수정안을 즉시 확정하며 참가자 재승인은 받지 않는다. 확정 뒤 불복은 경기 신고로 처리한다.
14. 최종 승인 요청은 멱등 키를 사용하고, 요청자·응답자·시각·IP/기기 위험 신호·변경 전후 값을 감사 로그에 남긴다. 같은 계정의 다중 슬롯, 본인 대리 승인, 명단 수정 뒤 이전 승인 재사용을 서버에서 거부한다.
...
8. 배정 확정은 `sideAssignmentStatus=draft`이고 `sideAssignmentRevision`이 1 이상이며, A/B 출전 인원이 경기 방식의 정원과 일치하고, 모든 참석자가 출전 또는 대기 중 하나에만 배치되며, 한 선수가 양쪽 사이드에 중복되지 않을 때만 성공한다.
9. 배정 확정 뒤 경기 중에는 사이드를 바꾸지 않는다. 교대는 같은 사이드의 출전·대기 사이에서만 수행해 결과와 선수 기록의 소속을 유지한다.
10. 균등 교대 방식은 경기 구간이 4개면 `매 쿼터`, 2개면 `하프 종료`, 단일 구간이면 `3분`, `5분`, `7분`, `10분` 간격 또는 `직접 교대`를 사용한다. 시스템은 자동으로 팀이나 슬롯을 변경하지 않는다.
11. 교대 순서와 대상은 방장 또는 배정 심판이 직접 정하고 확정해야 반영한다. 실제 교대 시각, 들어간 선수, 나온 선수, 사이드, 확정자를 교대 이벤트로 저장하며 지각·부상·퇴장·운영자 직접 변경을 허용하고 사유를 남긴다.
...
10. 마지막 open 건 판정은 방장의 최종 완료 처리다. 서버는 open 큐가 0건인지 확인한 뒤 결과와 MMR을 같은 트랜잭션에서 즉시 확정하며 양 팀 새 과반 승인을 요구하지 않는다.
...
- 팀 구성은 방모달에서 서로 다른 등록 팀 두 개를 먼저 고른다. 각 팀 주장이 자기 팀의 출전 명단만 정원만큼 제출하며 후보 슬롯은 사용하지 않는다. 기록 입력이 끝나면 양 팀 참가자 전원이 본인 계정으로 최종 승인한다.
- 참가 명단과 팀 로스터가 끝나기 전에는 최종 승인 요청을 막고, 승인 요청 뒤 명단이나 결과를 바꾸면 기존 최종 승인을 무효화한다. 서버는 `recordApproverIds`와 실제 출전 명단을 함께 검증하며 방장·팀장 대리 승인을 거부한다.
- 최종 승인 요청 뒤 설정된 10분·15분·20분 이의시간이 끝나고 열린 이의가 없으면 자동확정한다. 사후 기록은 승인 방식과 무관하게 개인·팀 MMR, 경쟁 승패, 연승에 반영하지 않는다. 결과 반대 응답은 이의조정으로 전환한다.
...
5. 최초 담당자는 출전 선수 중 방장을 우선한다. 배정 심판이 있으면 심판이 최초 담당 출전 선수를 지정할 수 있지만 심판 자신은 시계를 조작하지 않는다.
6. 현재 담당자 또는 경기 관리자는 다른 출전 선수에게 조작권을 넘길 수 있다. 이전을 확인한 즉시 기존 담당 기기는 읽기 전용이 된다.
...
9. 경기시계의 점수판은 `match_results`에 커밋된 심판·기록원 점수만 읽고 3초마다 갱신한다. 경기시계 화면에서 점수를 수정하지 않는다.
10. 경기시계의 A/B 점수는 배정 심판이 있거나 `getEffectiveStatRecorders()` 기준 A/B 양쪽 기록원이 모두 있을 때만 표시한다. 그 외에는 점수를 숨기고 경기시간만 표시한다.
...
11. 방 취소는 경기 시작 2시간 전부터 막는다. 시작 6시간 전부터 12시간 전 사이는 방장 신뢰도 3점, 시작 2시간 전부터 6시간 전 사이는 5점을 차감한다. 반려되거나 기한 만료된 일정 변경 때문에 방을 취소하는 경우에는 신뢰도를 차감하지 않는다.
12. 취소된 방은 일정 메뉴의 `취소된 방` 보기에 남겨 취소 시각·사유·신뢰도 반영 여부를 확인할 수 있게 한다. 알림의 방 링크도 원래 방 상세를 열어 같은 내용을 보여준다.
13. 취소된 방의 방장은 `같은 설정으로 다시 만들기`를 사용할 수 있다. 새 방은 새 ID로 만들고 이전 참가자, 후보, 초대, 채팅, 출석, 동의, 수정 횟수, 변경안 이력을 복사하지 않는다. 즉시 경기 외에는 날짜와 시간을 다시 선택해야 한다.
...
10. 사용 설명의 QR 출석은 경기시계를 사용하는 일반 공개 매칭방의 선택 기능으로 안내한다. 5분 토큰, 경기 10분 전 시작, 로그인한 사전 등록 선수 제한을 함께 설명하며, 시작 후 QR은 경기 전에 등록됐지만 미출석 처리된 선수를 원래 사이드 후보로 등록할 뿐 실제 출전이나 팀 배치를 확정하지 않는다고 명시한다.
...
> 이 절의 1~6항은 바로 아래 `심판 기반 개인 기록과 무심판 점수 확정` 5~9항의 후속 확정안으로 대체한다.
...
## 2026-07-28 경기시계 담당자·교체 단순화

> 이 절은 위 `경기 중 선수 교체·기록권한 통합`과 `심판 기반 개인 기록과 무심판 점수 확정`의 기록자·양면 위임·takeover 규칙을 대체한다.
...
7. 일반 경기의 최종 승인은 심판 유무와 관계없이 방장이 수행한다. 종료 뒤 점수 정정은 기존 이의·revision 이력을 사용하며 경기 중 기록자 역할을 다시 만들지 않는다. 마지막 이의 판정만으로 자동 확정하지 않는다.
