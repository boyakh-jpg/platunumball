...
4. 이 문서와 서버 정책·테스트에는 운영 검증을 위한 정확한 값을 유지할 수 있다. 소유자 전용 관리자 화면의 정책 설정과 정정 배율은 사용자 공개 화면으로 보지 않는다.
...
8. 일반 경기의 최종 승인은 결과 제출 3분 뒤 배정 심판이 있으면 배정 심판만, 무심판이면 방장만 실행한다. 실행 직전 `더 이상 이의가 없음을 확인하셨나요?` 확인창에서 명시적으로 확인하고 `disputesAcknowledged=true`를 서버에 보낸다. 과반 승인, 전원 승인, 팀장 승인은 일반 경기에 적용하지 않는다.
...
15. 타인 선수 프로필의 기록 API는 대상 선수가 참가한 공개 경기와 대상 선수가 작성한 공개 `personal_record`를 함께 제공한다. 비공개 기록은 제외하고, 프로필의 `teamHistory`·`statSummary` 공개 설정은 기존대로 각 섹션 노출에 적용한다. 공개 개인 기록은 공식 통계·업적·MMR과 합산하지 않는다.
...
7. 개인 구성 기록방은 `명단 제출 → 팀 점수 입력 → 각 참가자 내 참가 확인` 순서다. 팀 구성 기록방은 `팀 선택 → 각 팀장 출전 명단 제출 → 팀 점수 입력 → 양 팀 참가자 내 참가 확인` 순서다. 팀 선택·명단 제출과 참가 확인을 같은 상태로 표시하지 않는다.
...
7. 심판 검색창은 입력 전 focus 시에도 현재 활성 자격을 가진 심판을 제한 조회해 추천한다. 운영 심판은 신뢰도 90 이상을 요구하고, 허용된 알파 테스트 심판은 테스트 진행을 위해 신뢰도 하락과 무관하게 자격을 유지한다. 검색 서버는 활성 `referee_appointments`와 허용된 알파 테스트 심판을 먼저 확정한 뒤 공개 프로필을 조회하며, 일반 프로필 상위 결과를 먼저 자르지 않는다. 이름·해시태그·지역 입력 검색에도 같은 자격 필터를 적용하고 현재 경기 선수는 결과에서 제외한다.
...
- 팀 구성은 방모달에서 서로 다른 등록 팀 두 개를 먼저 고른다. 각 팀 주장이 자기 팀의 출전 명단만 정원만큼 제출하며 후보 슬롯은 사용하지 않는다. 기록 입력이 끝나면 각 참가자가 본인 계정으로 `내 참가 확인`한다.
...
9. `경기 기록`, `대회`는 이전 화면의 `pickup` 상태를 상속하지 않는다. 경기 기록은 후보 없는 사후 명단과 확인자 한정 1v1 10%·2v2 20%·3v3 35%·5v5 50% 개인 MMR·팀 MMR 미반영 정책을 사용하고, 대회는 일반 명단과 선택한 MMR 정책을 적용한다.
