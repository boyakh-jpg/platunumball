...
3. 정상 사용은 `5분 안 시작`, `경기시계 종료 기록`, `정규 구간 총 예상시간의 70% 이상 실제 진행`을 모두 충족해야 한다. 휴식과 일시정지는 실제 진행시간에서 제외한다.
...
7. 쿼터 진행 중에는 쿼터 시작 action을 표시하지 않는다. 남은 시간이 0이면 일시정지·계속 action을 숨기고 `쿼터 종료`만 허용한다. 정규 마지막 쿼터 뒤 동점일 때만 `연장 N`을 시작할 수 있다.
