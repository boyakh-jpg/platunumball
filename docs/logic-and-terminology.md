...
13. 영수증 편집 화면에서는 팀 엠블럼 파일 업로드·가공·저장을 제공하지 않는다. 로그인 후 팀을 만들고 팀 상세의 `영수증 엠블럼 만들기`에서 저장한 canonical 팀 자산만 Team A·B별 `저장 엠블럼 사용`으로 적용한다. 팀 상세는 공용 원형 크롭·자동 변환·확인 절차와 서버 검증을 거쳐 `teams.receipt_emblem_key`·`receipt_emblem_updated_at` 및 `team-emblems/{teamId}/` R2 key를 관리한다. 영수증 화면은 팀이 없거나 저장본이 없으면 로그인·팀 생성 경로를 안내하며 임의 클라이언트 object key와 guest 엠블럼 key를 draft에 보존하지 않는다.
...
1. 영수증 전용 엠블럼의 canonical 소유자는 팀이다. 팀 상세에서 저장한 `teams.receipt_emblem_key`만 영수증 미리보기와 Story·Feed Canvas에 적용한다.
2. 익명·공개 draft는 엠블럼 Blob이나 guest object key를 저장·복제·개인 기록으로 승계하지 않는다. 기존 guest 엠블럼 필드는 공개 투영·수정·clone 경계에서 제거하고 업로드 endpoint는 새 저장을 받지 않는다.
3. 선화 사용 해제는 해당 영수증의 표시 상태만 끈다. 엠블럼 생성·교체·삭제와 업로드 제한 이력은 팀 상세의 canonical 팀 자산 수명주기만 따른다.
