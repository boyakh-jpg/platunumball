...
15. 브라우저 편집 화면은 AI 선화 변환을 앱에서 직접 호출하지 않는다. `BOXTIER SCORE`의 엠블럼 편집 팝업에서만 사용자가 외부 도구에 붙여 넣을 프롬프트를 복사할 수 있으며, 원형·비율·글자를 유지한 고대비 단색 선화와 완전 투명 alpha PNG를 우선 요구한다. alpha 출력이 불가능한 도구에만 균일한 순수 `#00FF00` 배경을 보조 조건으로 허용한다. 브라우저로 반환한 이미지는 다시 로컬 기계 변환과 사용자 선택을 거쳐야 하며 자동 적용하지 않는다. `CLASSIC THERMAL`에는 이 action을 노출하지 않는다. 외부 생성 API 호출자는 19번의 완성 WebP 계약을 직접 충족한다.
...
19. 외부 AI·자동화는 `POST /api/match-receipts/create`로 사진 없는 익명 공개 영수증 draft를 생성한다. 입력은 `style`, 양 팀명, 양 팀 최종 점수, 실제 ISO 경기 날짜, 짧은 장소, 경기 방식을 필수로 하고 대회명, 구간 점수, 한줄평, 경기 성격, 언어, 경기 시각을 선택으로 받는다. `style`은 `boxtier-score` 또는 `classic-thermal`을 호출자가 명시해야 하며 서버가 임의로 감열지 여부를 정하지 않는다. 구간 점수는 양 팀 쌍·허용 라벨·중복 금지·최대 5개·최종 점수 합계 일치를 모두 검증한다. 선택 엠블럼은 `homeEmblem`·`awayEmblem` 각각 정확히 `{ imageBase64 }` 형태로 받는다. 값은 호출자가 미리 만든 raw Base64 WebP이며 `data:` URL이 아니다. 이미지는 `1:1`, 투명 배경, `#D6A522` 단색 선화, 충분한 외곽 여백과 시각적 중앙 정렬, 최대 `320×320px`, `96KB`를 충족해야 한다. 서버는 다시 변환·리사이즈·재인코딩하지 않고 Base64, 안전한 WebP container, 크기, 최대 치수, 정사각형만 검증한 뒤 `match-receipt-emblems/drafts/{publicId}/{side}-{digest}.webp` R2 key에 원본 바이트를 저장한다. 임의 엠블럼 URL·object key·data URL은 거부한다. 공개 투영은 해당 `publicId`에 속한 안전 key만 표시용 `homeEmblemKey`·`awayEmblemKey`로 내보내며 canonical 팀 엠블럼이나 verified 근거로 취급하지 않는다. 렌더러는 비율을 유지해 영역 중앙에 넣고 `CLASSIC THERMAL` 출력에서 검정 1비트로 바꾼다. 엠블럼이 없으면 팀명 기반 중립 표시를 쓴다. 이 경로는 임의 사진 URL·자산 ID, MMR, verified 입력을 받지 않고 항상 `includePhoto=false`, `verified=false`로 생성한다. 응답은 capability를 노출하지 않고 공개 코드, 만료 시각, 안전 공개 투영, 영수증 경로와 공개 JSON 경로만 반환한다. 생성 제한은 기존 IP 해시 기준 시간당 10건을 공유한다. 경기 사진을 외부 API로 받으려면 용도별 canonical 업로드 저장소, 파일 검증, 수명주기, 삭제, 렌더링 정책을 먼저 별도 확정해야 한다.
...
2. 브라우저 편집 화면의 로컬 엠블럼 Blob과 data URL은 현재 화면 메모리에서만 유지한다. 외부 생성 API의 사전 처리 WebP만 예외로 draft 전용 R2 key에 저장한다. key는 해당 `publicId` 범위에서만 공개 투영하며 draft 만료 maintenance가 DB payload의 안전 key를 확인해 R2 객체와 내부 guest 필드를 함께 제거한다. 외부 호출자가 object key를 직접 지정할 수 없고, 업로드 실패나 DB 저장 실패 시 이번 요청에서 먼저 저장한 객체를 정리한다. 외부 API draft 엠블럼은 다른 영수증으로 복제하거나 개인 기록으로 승계하지 않는다.
