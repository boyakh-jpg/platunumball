...
8. 일반 경기의 최종 승인은 결과 제출과 경기 종료 중 늦은 시각부터 3분 뒤 배정 심판이 있으면 배정 심판만, 무심판이면 방장만 실행한다. 실행 직전 `더 이상 이의가 없음을 확인하셨나요?` 확인창에서 명시적으로 확인하고 `disputesAcknowledged=true`를 서버에 보낸다. 과반 승인, 전원 승인, 팀장 승인은 일반 경기에 적용하지 않는다.
