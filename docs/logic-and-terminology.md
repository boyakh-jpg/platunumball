...
11. canonical 경기 방식 `mode`는 정원·MMR 호환을 위해 `1v1`, `2v2`, `3v3`, `5v5`를 유지한다. `3v3`에서 3x3 규칙 프리셋을 선택하면 1구간 10분·21점 또는 시간 종료·2점 차 승리·6호 공과 `rules.ruleSet=fiba_3x3`을 저장하고 방·계약·목록·영수증의 표시 방식만 `3x3`으로 통일한다. 사용자 화면에는 `FIBA 3x3`을 쓰지 않는다. 명시적인 다른 `ruleSet`이 있으면 항상 일반 `3v3`이며, 마커가 전혀 없는 기존 경기만 원본 규칙의 1구간·12분·21점·21점 또는 시간 종료·2점 차 승리 조건을 모두 만족할 때 표시 호환상 `3x3`으로 해석한다. 정규화가 채운 기본값은 이 레거시 판정의 증거로 사용하지 않는다.
