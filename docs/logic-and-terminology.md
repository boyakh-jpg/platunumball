...
23-6. `disputeMatch` is sent from the frontend as operation-only. The server must reload authoritative match state and run the central reducer before persisting the dispute row and draft result; client match snapshots are not accepted as the dispute source of truth.
