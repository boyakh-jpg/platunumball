...
19. 외부 AI·자동화는 `POST /api/match-receipts/create`로 사진 없는 익명 공개 영수증 draft를 생성한다. 입력은 `style`, 양 팀명, 양 팀 최종 점수, 실제 ISO 경기 날짜, 짧은 장소, 경기 방식을 필수로 하고 대회명, 구간 점수, 한줄평, 경기 성격, 언어, 경기 시각을 선택으로 받는다. `style`은 `boxtier-score` 또는 `classic-thermal`을 호출자가 명시해야 하며 서버가 임의로 감열지 여부를 정하지 않는다. 구간 점수는 양 팀 쌍·허용 라벨·중복 금지·최대 5개·최종 점수 합계 일치를 모두 검증한다. 엠블럼 bytes·Base64·URL·object key 입력은 모두 거부하고 guest draft 엠블럼을 R2·DB·공개 snapshot에 저장하지 않는다. 익명 출력은 팀명과 고정 중립 마크를 사용한다. 이 경로는 임의 사진 URL·자산 ID, MMR, verified 입력을 받지 않고 항상 `includePhoto=false`, `verified=false`로 생성한다. 응답은 capability를 노출하지 않고 공개 코드, 만료 시각, 안전 공개 투영, 영수증 경로와 공개 JSON 경로만 반환한다. 생성 제한은 기존 IP 해시 기준 시간당 10건을 공유한다. 경기 사진이나 엠블럼을 외부 API로 받으려면 용도별 canonical 업로드 저장소, 인증·소유권, 파일 검증, 수명주기, 삭제, 렌더링 정책을 먼저 별도 확정해야 한다.
...
2. 브라우저 편집 화면의 로컬 엠블럼 Blob과 data URL은 현재 화면 메모리에서만 유지한다. 영수증 draft 생성·수정·공개 조회·복제·claim은 guest 엠블럼 입력이나 key를 저장·투영·승계하지 않는다. 만료 maintenance는 과거 legacy draft payload에 남은 안전한 guest key만 R2에서 정리한 뒤 내부 필드를 제거한다.
