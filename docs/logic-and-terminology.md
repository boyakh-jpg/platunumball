...
2. 게스트 매칭 목록은 인증 토큰 없이 허용된 공개 API가 반환한 `status=open`, `visibility=public` 모집만 최대 50건 표시한다. 공개 방 설정은 공용 방 모달에서 읽을 수 있지만 참가자, 채팅, 초대, 메모, 방 상태 등 인증 전용 필드는 반환하거나 추정하지 않는다.
...
12. 게스트가 모집 카드를 누르면 인증 화면이 아니라 기존 공용 방 모달을 읽기 전용으로 연다. 참가·채팅·수정 등 mutation에서만 원래 방 경로를 보존해 로그인을 요구한다.
