...
3. 현장 GPS는 지도 핀과 별도로 저장한다. 신청자는 주소를 선택하고 지도 핀을 실제 코트에 맞춘 뒤 현장 GPS를 확인해야 사진을 촬영할 수 있다. 위치 확인이 없거나 10분이 지난 경우 촬영을 진행하지 않는다. 위치 정확도 20m 이하이고 핀과 현장 위치가 30m 이하일 때만 자동승인하며 정확도·거리 기준을 벗어나면 관리자 검토로 보낸다.
4. AI는 사진별 농구 코트·골대·전체 모습·코트 형태·스크린샷 또는 합성 위험을 구조화해 반환한다. 최종 결정은 서버 정책 함수가 하며 AI 응답에 DB 승인 권한을 주지 않는다.
5. 자동승인은 야외·공개 구장, 사진 2장 이상, 신뢰도 90 이상 또는 과거 승인 2건 이상, 30m 내 중복 없음, 사진별 구조화 판정과 서버의 필수 시각 증거 5개가 모두 충족되어야 한다. 같은 사진 재사용은 허용하지 않는다.
...
20. 사진 EXIF의 위도·경도는 휴대폰 설정이나 브라우저 처리로 누락될 수 있고 조작도 가능하므로 자동승인 위치의 기준으로 사용하지 않는다. 현장 GPS를 기준으로 유지하며 사진 메타데이터는 WebP 변환 때 제거한다. 향후 EXIF 위치를 읽더라도 값이 존재할 때 불일치 경고를 주는 보조 신호로만 사용한다.
