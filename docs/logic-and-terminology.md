...
1. 방 만들기에서 확정한 `ballProvider`, `vestsProvided`, `scoreboardAvailable`, `shotClockAvailable`, `statRecorderAvailable`은 모집방과 확정 경기의 공통 계약 정보다.
2. 공통 방 모달은 다섯 값을 규칙 영역에 항상 표시한다. 거짓 값도 숨기지 않고 `없음`으로 표시해 참가자가 장비를 준비해야 하는지 판단할 수 있게 한다.
3. 확정 경기의 방 모달은 과거 모집방 snapshot보다 현재 경기의 `rules`를 우선해 운영 장비를 표시한다.

## 2026-07-24 경기시계 검증과 MMR 반영

1. 모달의 `경기 시작`은 기존 경기 생명주기를 시작한다. 경기시계는 실제 운용 증거이며 시계를 시작하지 않아도 경기는 진행할 수 있다.
...
4. 정상 사용이면 기존 MMR 반영률을 유지한다. 미사용이면 기존 경기 품질 계수에 1v1 50%, 2v2 65%, 3v3 80%, 5v5 90%를 곱한다. 경기시계 기능 도입 전에 시작된 경기에는 소급 적용하지 않는다.
