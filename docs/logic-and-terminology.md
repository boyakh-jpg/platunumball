...
- 경기 메뉴는 내 일정 안에서 공개 모집, 비공개 초대, 개인전, 팀전, 기록방을 분리해 볼 수 있다.
...
- 진행 메뉴의 점수판, 기록자, 후보/교체 출전 판단은 얇은 목록 row를 원본으로 쓰지 않는다.
- 선택된 경기 1건만 상세 로드해 `statRecorders`, `playedPlayerIds`, `result.playerStats`를 맞춘다.
...
- `비공개 경기방 + 팀전`: `visibility:"private"`, `hostJoinMode:"team"`으로 만들고 A사이드 출전은 생성자가 채우며 B사이드는 대표 1명만 초대한다. B사이드 라인업은 대표 수락 후 방에서 고른다.
