...
12. 비로그인 사용자는 공개 앱 경로에서 번들 체험 데이터를 읽을 수 있다. 공개 범위는 홈, 가이드·연습, 공개 경기·매칭, 커뮤니티, 시즌·랭크보드, 팀·선수·심판·구장·대회·소속 조회다. 체험 상태는 인증 세션이나 서버 권한을 만들지 않는다.
...
1. 비로그인 게스트에게 데모 사용자·경기·알림·팀을 주입하지 않는다. 게스트 홈은 개인 기록과 알림을 렌더링하지 않는다.
2. 게스트 매칭 목록은 인증 토큰 없이 허용된 공개 API가 반환한 `status=open`, `visibility=public` 모집만 최대 50건 표시한다. 참가자, 채팅, 초대, 메모, 방 상태 등 인증 전용 필드는 반환하거나 추정하지 않는다.
