...
20-1. MCP 영수증 도구 descriptor는 표시 resource·`openai/outputTemplate`·위젯 이미지 메타데이터를 연결하지 않는다. 표준 `content[type=image]`에 양쪽 엠블럼까지 합성된 최종 PNG 한 장만 반환하며 일반 호출의 `structuredContent`에는 Base64를 중복하지 않는다. 이미 과거 descriptor를 캐시한 ChatGPT 연결의 template fetch 실패만 막기 위해 기존 `ui://boxtier/basketball-receipt-v2.html`·`ui://boxtier/basketball-receipt-v3.html` URI는 호환 resource로 계속 제공한다. 호환 resource는 tool result의 기존 `image/png`만 투명 배경에 표시하고 로딩 문구·상태 박스·다운로드 action·외부 이미지 URL·별도 저장을 만들지 않으며 신규 descriptor에는 연결하지 않는다. tool 설명은 모델이 metadata만 보고 완료 처리하지 않고 반환된 image content를 사용자에게 직접 제시하도록 지시한다. 서버 성공 조건은 실제 HTTP/SSE tool result에 raw Base64 `image/png` 블록이 직렬화되고 PNG 길이·크기·해시 검증값이 일치하는 데까지다. 네이티브 대화창의 실제 이미지 표시·다운로드 UI는 MCP 클라이언트가 결정하므로 서버가 보장한다고 표현하지 않는다. `debugBase64`는 명시적으로 `true`인 개발 호출에서만 Base64를 중복하는 확인 옵션이며 image content 생성·전달 조건으로 사용하지 않는다.
