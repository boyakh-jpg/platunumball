...
8. 영수증 표시 언어는 한국어를 기본값으로 하는 화면 전용 상태이며 경기 원본·draft·공개 스냅샷에 저장하지 않는다. 한국어 화면의 경기 장소는 지도 선택 결과만 허용하고, 영어 화면은 국내 지도 선택기를 숨긴 채 기존 짧은 장소 입력을 `Venue` 직접 입력으로 사용한다. 지도에서 구장을 선택하면 구장명과 내부 원주소를 보존한다. 자유 표기는 짧은 장소에만 저장하며 지도 검색 지역 판정에는 사용하지 않는다. 경기 장소와 짧은 장소 중 하나만 있으면 위치 필수값을 충족한다. 영수증에는 짧은 장소가 있으면 그것만 표시하고, 비어 있으면 구장명을 표시하며 전체 원주소는 출력하지 않는다. 좌측 상단 일련번호와 경기기록 해시태그는 동일한 `#BT-########` 공개 코드를 표시한다. 공개 코드는 권한이나 소유권 키가 아니며 최초 DB 생성 뒤 변경하지 않는다. 기존 경기의 canonical 공개 draft는 경기 공개 코드를 상속하고, 익명 draft를 경기로 승계하면 그 공개 코드를 경기에도 그대로 사용한다. 사용자가 `새 영수증 만들기`를 실행하면 기존 draft를 수정하지 않고 새 공개 코드를 받는다.
