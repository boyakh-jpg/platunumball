...
7. 쿼터 진행 중에는 쿼터 시작 action을 표시하지 않는다. 정규 마지막 쿼터 뒤 동점일 때만 `연장 N`을 시작할 수 있다.
8. 샷클락은 사용 안 함, 30초, 35초, 60초 중 선택한다. 샷클락 초기화는 경기시계 담당자만 수행하며 MMR 정상 사용 판단은 샷클락이 아니라 경기시계를 기준으로 한다.
...
12. 샷클락 종료는 짧은 부저, 쿼터 종료와 경기시계 종료는 긴 부저, 휴식 상한 초과는 짧은 2회 경고 부저를 사용한다. 브라우저에서는 실제 미디어 재생을 우선하고 실패할 때만 Web Audio로 대체한다.
