...
1. 현재 위치 경로는 모바일 후면 카메라를 우선 여는 native capture input으로 현장 사진 1~4장을 누적하고, 주소 경로는 사진 0~4장을 보관함에서 선택할 수 있다. 사진은 개별 재선택·삭제할 수 있다. 원본 파일 용량을 제한하지 않고 브라우저에서 긴 변 1280px 이하, WebP, 장당 목표 220KB로 자동 변환하며 서버는 장당 300KB 이하 결과만 받는다.
...
16. AI 호출 전 브라우저는 원본 사진의 짧은 변 640px 이상과 96px 축소 샘플의 노출·대비·경계 선명도를 검사한다. 서버는 변환된 실제 WebP가 12KB 이상, 짧은 변 360px 이상, 300,000px 이상인지 다시 검사한다. RIFF 전체 길이·청크 경계·허용 이미지 청크도 확인해 메타데이터·애니메이션·미지 청크·뒤에 붙은 데이터를 거절한다. 미달 사진은 R2 저장·AI 호출·신청 생성을 시작하지 않고 재촬영을 요구한다.
