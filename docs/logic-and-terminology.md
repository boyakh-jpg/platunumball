...
5. 모든 경기 방식의 기본 시간 구성은 `8분 × 2구간 러닝타임`이다. `8분 × 4쿼터 러닝타임`도 별도 프리셋으로 제공하며 5v5에서는 마지막 2분을 스톱타임으로 운영한다. `10분 × 4쿼터 스톱타임`은 경기 지위로 오해하지 않도록 별도 시간 프리셋으로 표시한다. 1v1·2v2·3v3의 종료 기준과 승리 조건은 해당 방식의 공용 기본 규칙을 유지한다.
