...
1. 공개 모집, 비공개 초대, 대회, 기존 경기 기록, 개인 기록 생성은 같은 단계형 탐색을 사용한다. 공개·비공개 경기는 `경기 성격 → 인원·출전 정책 → 경기 규칙 → 구장·비용 → 운영·취소 정책 → 최종 확인`의 6단계다. 대회·경기 기록·개인 기록은 같은 공용 단계·정책을 쓰되 비용, 후보, MMR처럼 해당 유형에 없는 항목을 렌더링하거나 저장하지 않는다.
