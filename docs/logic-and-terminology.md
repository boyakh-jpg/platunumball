...
15. AI 선화 변환은 앱이 직접 호출하지 않는다. `BOXTIER SCORE`의 엠블럼 편집 팝업에서만 사용자가 외부 도구에 붙여 넣을 프롬프트를 복사할 수 있으며, 원형·비율·글자를 유지한 고대비 단색 선화와 완전 투명 alpha PNG를 우선 요구한다. alpha 출력이 불가능한 도구에만 균일한 순수 `#00FF00` 배경을 보조 조건으로 허용한다. 반환 이미지는 다시 로컬 기계 변환과 사용자 선택을 거쳐야 하며 자동 적용하지 않는다. `CLASSIC THERMAL`에는 이 action을 노출하지 않는다.
...
2. 익명·공개 draft는 엠블럼 Blob이나 guest object key를 저장·복제·개인 기록으로 승계하지 않는다. 로컬 선화는 초기화·새로고침·공유에서 사라지고, 기존 guest 엠블럼 필드는 공개 투영·수정·clone 경계에서 제거하며 업로드 endpoint는 새 저장을 받지 않는다.
