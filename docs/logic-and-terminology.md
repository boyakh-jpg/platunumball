...
1. 구장 신청은 모바일 후면 카메라를 우선 여는 native capture input으로 현장 사진 1~4장을 한 장씩 누적한다. 원본 파일 용량을 제한하지 않고 브라우저에서 긴 변 1280px 이하, WebP, 장당 목표 220KB로 자동 변환하며 서버는 장당 300KB 이하 결과만 받는다.
2. 사진은 공개 자산 버킷과 분리된 비공개 R2 버킷에 `court-requests/{requestId}/{sha256}.webp`로 저장한다. DB에는 object key와 해시만 저장하고 관리자 전용 API만 원본을 읽는다.
3. 현장 GPS는 지도 핀과 별도로 저장한다. 지도 핀 확정 뒤 촬영 버튼은 바로 카메라를 열고, 사진이 돌아오면 최근 현장 GPS가 없는 경우 즉시 위치를 확인한다. 위치 확인에 실패한 사진은 보관하지 않는다. 위치 정확도 20m 이하이고 핀과 현장 위치가 30m 이하일 때만 자동승인하며 정확도·거리 기준을 벗어나면 관리자 검토로 보낸다.
