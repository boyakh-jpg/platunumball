...
11. canonical 경기 방식 `mode`는 정원·MMR 호환을 위해 `1v1`, `2v2`, `3v3`, `5v5`를 유지한다. `3v3`에서 FIBA 3x3 21점 프리셋을 선택하면 1구간 10분·21점 또는 시간 종료·2점 차 승리·6호 공과 `rules.ruleSet=fiba_3x3`을 저장하고 방·계약·목록·영수증의 표시 방식만 `3x3`으로 통일한다. 일반 `3v3`은 계속 `3v3`으로 표시하며 `other`를 만들지 않는다. 마커가 없는 기존 경기 중 1구간·12분·21점·21점 또는 시간 종료·2점 차 승리 조건을 모두 만족하는 값은 표시 호환상 `3x3`으로 해석한다.
12. 대회·리그 이름은 선택 표시값이다. 구간별 점수는 `match_results.period_scores`의 순서 있는 `[{label, scoreA, scoreB}]`를 canonical 원본으로 사용하고, 양 팀 값을 쌍으로 입력하며 합계가 canonical 최종 점수와 정확히 일치할 때만 결과와 함께 저장한다. 구간 점수는 비어 있어도 되며 최종 점수를 바꾸는 수락 이의제기에서는 임의 보정하지 않고 비운다. 비회원·레거시 영수증의 수동 구간 점수는 canonical 값이 없을 때만 표시 fallback으로 사용하고 경기 진행 상태, 대회 결과, 개인·팀 MMR을 변경하지 않는다.
