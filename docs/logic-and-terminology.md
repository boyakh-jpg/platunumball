...
11. 조회수는 로그인 사용자와 익명 사용자 모두 게시글 상세 조회 시 한국 날짜 기준 방문자당 하루 1회 반영한다. 로그인 사용자는 프로필, 익명 사용자는 서명된 `HttpOnly` 쿠키를 HMAC한 식별자를 사용하며 원본 IP는 저장하지 않는다. 같은 날의 반복 열기와 목록 노출은 조회수를 올리지 않는다.
