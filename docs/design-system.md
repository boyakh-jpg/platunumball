...
3. 후보 정원 control은 `출전 수+0`, `출전 수+1`, `출전 수+2`, `출전 수+3`으로 표시한다. 0을 선택하면 후보 정책, 후보 비용 경고, 후보 슬롯, 후보 설명 영역 자체를 숨겨 빈 박스를 남기지 않는다.
...
7. `친선전`, `경쟁전`, `픽업`은 같은 프리셋 grid 안에 둔다. 세 프리셋은 데스크톱에서 3열로 배치하고 `640px` 이하에서는 1열로 배치한다. 픽업을 선택하면 경기 형식을 개인전으로 고정하고 팀전 option을 비활성화한다. 인원 단계는 `사이드 참가 인원`, `현장 팀배정 · 방장 수동 순환`으로 표시하며 고정 선발·후보나 자동 로테이션이 있는 것처럼 안내하지 않는다. 최종 요약에는 MMR 미반영과 수동 운영을 함께 표시한다.
