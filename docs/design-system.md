...
12. `주소 시설명·중복 코트 정리`는 미저장 셀이 없을 때만 중복 후보 검수 화면을 연다. 그룹 진행 위치, 시설명·주소, 감지 DB 행 수, 행별 표준명·코트 구분·상태·지도 버튼을 먼저 보여주고 `이 장소에 실제 코트가 몇 개 있나요?` 숫자 입력과 확정 버튼을 둔다. 입력 수보다 많은 행은 확정 전 비활성화 예정 수를 경고하고, 부족하면 DB 부족 수를 안내한다. 확정 후 다음 후보로 자동 이동하며 이전·다음·검수 종료를 제공한다. mobile에서는 구장 목록과 입력·이동 버튼을 한 열로 쌓고 버튼 높이를 44px 이상 유지한다. 주소 시설명 또는 수동명이 기존 OSM fallback보다 우선된 검수 카드는 `명칭판정`을 `주소·수동명 우선`, `근거시설`을 현재 시설명으로 표시하고 과거 OSM 거리를 현재 근거처럼 노출하지 않는다.
