...
## 2026-07-14 경기 메뉴 상태판

1. 경기 메뉴 상태 선택은 `내 일정`, `처리 필요`, `예정·진행`, `닫힘` 네 카드로 고정한다.
2. 데스크톱은 한 줄 4카드, 좁은 화면은 가로 스크롤 없이 2열로 접는다.
3. hero 상황판은 `MY`, `ACTION`, `SOON`, `CLOSED`를 2열 compact metric으로 표시한다.
4. 네 상태의 숫자와 목록은 같은 feed 후보 목록과 같은 분류 함수를 사용한다.
...
2. `기록하기`는 이미 끝난 경기의 경기기록방을 만들고, 경기 메뉴 일정이 아니라 진행 메뉴 기록 확인 흐름에 붙인다.
...
3-3. 경기기록방 생성 화면은 2v2 이상 팀전에서 팀 선택과 상대 대표 선택만 받는다. 출전/후보 명단 선택 UI는 기록방 안에서 처리한다.
3-3-1. 공개/비공개 팀전 생성 화면도 팀원 출전/후보 picker를 노출하지 않는다. 생성 화면은 팀과 대표만 고르고, 방 모달에서 각 사이드장이 명단을 확정한다.
...
16. 진행 메뉴 첫 화면은 경기 메뉴와 같은 카드 목록 문법을 쓴다. 카드 CTA는 공통 방 모달을 열고, 진행 메뉴 안에 별도 점수판/상세 작업판을 만들지 않는다.
17. 진행 메뉴에서 열린 공통 방 모달은 기록판과 선수 교체 패널을 슬롯보다 위에 둔다. 경기/매칭 메뉴의 방 모달 순서는 유지한다.
18. 진행 메뉴 기록판과 기록 입력 패널도 방 모달 공통 panel border/radius/background 토큰을 써서 매칭 방 모달과 같은 테두리 체계를 유지한다.
19. 방모달 큰 제목은 다크/라이트 모두 공통 주황 토큰 `--rb-orange`를 쓴다.
20. 진행 메뉴 경기 목록 카드도 매칭 메뉴 공개방 카드와 같은 `om-match-card arena-lobby-card` skin을 쓰고, 카드 전체 클릭으로 공통 방 모달을 연다.
21. 방모달 shell은 border/radius/background를 유지하고, 세로 스크롤은 내부 `.arena-lobby-modal-scroll`에서만 처리해 좌우 테두리가 잘리지 않게 한다.
22. 진행 메뉴 카드 목록은 매칭 메뉴 카드처럼 compact row 높이를 유지한다. 긴 제목, 메타, 요약은 줄바꿈으로 카드 높이를 키우지 말고 한 줄 안에서 잘라낸다.
...
4. 진행 메뉴에서 선택 경기 상세를 불러오는 동안은 명시적 detail load로 보고 같은 전역 오버레이를 쓴다.
...
2-4. 비공개 개인전 생성에는 공용 `SearchPicker`로 개인 초대 대상을 선택하는 진입점을 둔다. 선택 UI는 기존 검색 결과/안내 박스 패턴을 재사용하고 별도 모달을 만들지 않는다. 선택 후 검색 결과는 닫고, 선택된 대상은 같은 영역의 chip 버튼으로 해제할 수 있어야 한다.
...
5. 경기기록방은 경기 메뉴 일정 필터가 아니라 진행 메뉴 기록 확인 흐름에서 표시한다.
...
5. 진행 메뉴에서 열린 live/postgame/dispute 경기방도 기록판을 슬롯보다 위에 둔다.
...
6. 경기 메뉴의 캘린더 구조 wrapper, 일정 summary, section head, empty state는 항상 투명하다. 달력 본체, 필터 control group, 실제 경기·대회 항목만 테두리와 card surface를 가진다.
7. 일정 row는 데스크톱 한 줄 편집을 우선하고 좁은 화면에서는 날짜·시간·구장·액션을 한 열로 쌓아 control이 잘리거나 겹치지 않게 한다.
8. 경기 메뉴의 대회 카드는 데스크톱에서 정보·배지·상태를 왼쪽에 두고 `대진표` 또는 `리그표`를 오른쪽 고정 폭의 주황 CTA로 표시한다. CTA를 별도 하단 row나 중첩 card로 감싸지 않으며, `720px` 이하에서만 하단 전체 폭으로 배치한다.
9. 리그 경기표는 데스크톱에서 두 열의 compact fixture 카드를 허용하되 각 카드의 상태, 대진, 날짜·시간, 구장, 액션 열 위치를 동일하게 맞춘다. `720px` 이하에서는 한 열로 바꾸고 액션을 카드 하단에 배치한다.
9-1. 리그 순위표와 경기표는 `390px` 모바일 폭에서 문서 전체 가로스크롤을 만들지 않는다. 필요한 경우 표 내부만 스크롤하고 fixture 카드는 한 열로 접는다.
10. 경기 메뉴의 `내 진행 일정` 필터는 달력과 중복되는 `기간` 행을 두지 않고 `관계 / 유형` 두 compact 행으로 정렬한다. 중첩 control group 외곽을 만들지 않고 데스크톱 버튼은 `34px`, 간격은 `4px`를 쓰며 모바일 버튼은 `42px` 터치 높이를 유지한다. 선택한 날짜를 다시 누르면 날짜 필터를 해제한다.
11. 경기 메뉴의 상태 카드, 내 팀 경기, 비공개 대회, 관계/유형/날짜/월 필터는 URL query와 동기화한다. 브라우저 뒤로가기는 직전 query를 읽어 해당 카드와 필터 active 상태를 복원한다.

## 2026-07-15 알림함

1. 알림함은 `읽지 않음`과 `지난 알림`을 segmented tab으로 분리하고 각 탭에 개수를 표시한다.
2. 지난 알림에는 생성 시각과 개별 휴지통 아이콘을 표시한다. 휴지통은 tooltip과 접근성 이름을 가진다.
3. 모바일 알림 row는 본문과 액션을 세로로 쌓고 탭은 같은 폭을 사용한다.
...
6. 모바일 경기 기록판은 양 팀 점수 스테퍼를 세로 두 행으로 쌓는다. 선수 기록은 한 선수 안에서 2열로 배치하고 `340px` 이하에서는 1열로 전환한다. 각 스테퍼 열은 부모 폭보다 커지지 않게 유동 폭을 써서 화살표·값·옆 기록이 겹치지 않게 한다.
...
1. 경기 화면 상단 보기 카드는 `내 일정 / 처리 필요 / 예정·진행 / 닫힘 / 내 팀 경기 / 비공개 대회` 6개로 구성한다. 데스크톱은 한 행, 중간 너비는 3열, 모바일은 2열이다.
