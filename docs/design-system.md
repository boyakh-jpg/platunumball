...
## 2026-07-20 개인·팀 엠블럼 편집

1. `나`의 개인 엠블럼은 현재 기본값만 편집한다. 개인 사진·Discord 사진 선택은 제공하지 않는다. 추후 검수된 아이콘 자산을 받으면 같은 카드에 아이콘 카탈로그를 연결한다.
2. 개인 기본값과 팀 기본값을 선택했을 때만 기본색, 테두리 사용 여부, 테두리색 control을 표시한다. 테두리를 끄면 테두리색 control은 비활성화한다.
...
7. 데스크톱은 원본 선택과 색상 control을 가로로 배치한다. `720px` 이하에서는 한 열로 내려 가로 overflow가 생기지 않게 한다. 크롭 dialog는 모바일에서 viewport 하단 sheet처럼 배치하고 내부만 스크롤한다.
8. 공용 개인 엠블럼과 `TeamEmblem`은 이름 첫 글자 기본값을 사용하고, 팀 이미지만 로드 실패 시 기본값으로 되돌아간다. 선택된 배경색과 테두리색은 dark/light 양쪽에서 같은 사용자 값으로 유지한다.
...
1-1. `1121~1400px` 데스크톱은 6개 보기 카드를 한 행으로 유지하되 아이콘과 설명을 숨긴 압축형을 사용한다. 제목과 숫자는 한 줄 안에서 서로 겹치거나 세로로 잘리지 않아야 한다.
...
1. 경기 메뉴의 일정 경기·모집방, 매칭 메뉴의 공개방, 진행 메뉴의 기록 경기는 같은 `om-match-card` 외곽 surface와 타이포 위계를 사용한다.
