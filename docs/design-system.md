...
2. `scope: "mine"` 모집방 보강 로드는 경기 목록 렌더를 막지 않는다.
3. 모집방 일정은 보강 로드가 끝난 뒤 같은 일정 목록에 합쳐진다.
4. 경기/모집방 카드가 아직 하나도 없고 내 모집방 보강 로드가 진행 중이면 `해당 큐 없음`을 먼저 보여주지 않고 loading empty-state를 유지한다.
