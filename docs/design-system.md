...
19-6. `720px` 이하 방 모달은 출전·후보 slot과 wrapper의 폭을 같은 `--room-slot-width`로 직접 고정하고, 출전 row·후보 row·파티 묶음 높이를 같은 `--room-slot-height`로 고정한다. grid 계산 결과에 따라 어느 한쪽만 늘어나면 안 된다.
