...
매칭 메뉴의 외형과 상단 흐름은 유지할 수 있지만, 중간 요약 박스는 경기 메뉴처럼 `A / 전체 / B`, 룰, 상태 설명 순서로 통일한다.
...
- 모집방 라이트 모드 슬롯과 파티 묶음 배경은 gradient 없이 flat surface를 쓴다. 파티 연결선은 주요 민트 색으로 맞추고 슬롯 테두리에서 끊기지 않게 아바타 중심까지 이어진다.
...
- 매칭 목록 카드에는 슬롯 숫자만 남기고 룰/상세 문구는 방 모달에서 확인하게 한다.
...
- 하단 메뉴는 모바일에서 한 줄, 큰 아이콘, 충분한 터치 높이를 유지한다.
