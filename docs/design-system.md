...
6-3. 팀 엠블럼과 프로필 아이콘에서 테두리를 사용하면 후광도 저장된 테두리색을 사용한다. 테두리를 끄면 외곽 후광을 표시하지 않는다.
7. 데스크톱은 원본 선택과 색상 control을 가로로 배치한다. 프로필 아이콘 dialog의 세 원본 버튼은 모바일에서도 3열 compact 높이를 유지하고, 색상·팀 편집 control은 `720px` 이하에서 한 열로 내려 가로 overflow가 생기지 않게 한다. 팀 사진 action은 모바일에서 안내를 한 행으로 두고 복원·선택 버튼을 2열로 맞춘다. 아이콘 dialog와 크롭 dialog는 모바일에서 viewport 하단 sheet처럼 배치하고 내부만 스크롤한다.
