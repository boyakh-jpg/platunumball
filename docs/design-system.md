...
1. 공개·비공개 경기, 대회, 경기 기록, 개인 기록 생성은 공용 단계 nav를 사용한다. 공개·비공개 경기의 기본 흐름은 `기본 설정 → 규칙·운영 → 구장·비용 → 확인` 4단계다. 현재 단계의 카드만 보여주고 `이전`·`다음`으로 이동하며 최종 단계에서만 전체 요약과 생성 오류·주의를 함께 표시한다. 유형과 무관한 단계·control은 빈 공간을 남기지 않고 건너뛴다.
...
7. `픽업·자유경기`는 같은 프리셋 grid 안에 둔다. 네 프리셋은 데스크톱 2열, `640px` 이하 1열로 배치한다. 픽업을 선택하면 경기 형식을 개인전으로 고정하고 팀전 option을 비활성화한다. 인원 단계는 `사이드 참가 인원`, `현장 팀배정 · 방장 수동 순환`으로 표시하며 고정 선발·후보나 자동 로테이션이 있는 것처럼 안내하지 않는다. 최종 요약에는 MMR 미반영과 수동 운영을 함께 표시한다.
