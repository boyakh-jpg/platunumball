...
8. 공용 `ProfileEmblem`은 이름 첫 글자, `TeamEmblem`은 저장된 팀명/약칭 디자인을 기본값으로 사용한다. 선택한 Discord/팀 이미지 로드 실패 시 기본값으로 되돌아가며, 선택된 배경색과 테두리색은 dark/light 양쪽에서 같은 사용자 값으로 유지한다. Discord 사진은 원형 영역을 채우고 검수 아이콘은 제작 안전영역을 보존하도록 `object-fit: contain`을 사용한다.
