...
12. `중복주소 코트번호 정리`는 미저장 셀이 없을 때만 실행한다. 처리 중 버튼을 잠그고 완료 후 전체 반영 구장 수와 중복 주소·코트 수를 같은 toolbar 상태 문구로 보여준다. 주소 시설명 또는 수동명이 기존 OSM fallback보다 우선된 검수 카드는 `명칭판정`을 `주소·수동명 우선`, `근거시설`을 현재 시설명으로 표시하고 과거 OSM 거리를 현재 근거처럼 노출하지 않는다.
