...
3. 픽업 체크인 화면은 `출석 확인 → A/B 배정 → 대기 배정 → 균등 교대 설정 → 배정 확정` 순서의 작업판을 표시한다. 배정 확정 뒤에만 A/B 경기장과 같은 사이드 내 교대 action을 표시한다.
4. 교대 설정은 4구간에서 `매 쿼터`, 2구간에서 `하프 종료`, 단일 구간에서 `3분/5분/7분/10분/직접 교대`를 제공한다. 추천 선수와 교대 이유를 보여주고 방장 또는 배정 심판의 명시적 확정 버튼을 둔다.
5. 모바일 체크인 작업판은 출석 목록과 배정 목록을 세로로 배치하고 선수를 누른 뒤 A/B 출전·대기를 고르게 한다. 선수 카드를 드래그해야만 완료할 수 있는 UI는 사용하지 않는다.
...
1. 사후 기록 전용 본문은 `참가자 구성`, `참가 확인`, `기록 입력`, `결과 승인`을 각각 독립된 상태와 panel로 표시한다. 참가 확인 전 선수를 승인 완료처럼 표시하지 않는다.
2. 참가 후보에게는 `참가했습니다`, `참가하지 않았습니다`를 제공하고 결과 승인 단계에서는 `결과 승인`, `결과가 다릅니다`를 제공한다. 방장·팀장 화면에 다른 참가자의 대리 승인 action을 만들지 않는다.
...
5. 한 명이라도 `참가하지 않았습니다` 또는 `결과가 다릅니다`를 고르면 primary action을 숨기고 `이의조정 중` 상태 strip과 관계자 수정·재확인 흐름을 먼저 보여준다.
6. 사후 기록 상태 badge는 `참가 확인 대기`, `기록 입력 가능`, `결과 승인 대기`, `부분 확인`, `이의조정 중`, `확정`만 사용한다. `무응답 승인`, `자동 승인`, `전체 확정`으로 오해할 문구를 사용하지 않는다.
...
7. `친선전`, `경쟁전`은 `matchPurpose` grid에 두고 `경기 전 구성`, `현장 픽업`은 별도의 `formationMode` grid에 둔다. 두 축은 데스크톱에서 나란히, 태블릿에서는 세로로 배치하며 각 축의 두 선택은 충분한 너비에서 한 줄을 유지한다. 현장 픽업을 선택하면 참가 방식을 개인전으로 고정하고 팀전 option을 비활성화한다. 인원 영역은 `출전 필요 인원`, `전체 참가 정원`, `체크인 후 팀 배정`으로 표시하며 모집 중 A/B·후보나 자동 배정이 있는 것처럼 안내하지 않는다. 최종 요약에는 친선·MMR 미반영, 통합 참가자 풀, 방장/심판 확정 교대를 함께 표시한다.
