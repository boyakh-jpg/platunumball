...
8. 단계 nav는 현재 유형의 3~4개 단계 수에 맞춰 동일 폭으로 배치한다. 모바일에서도 내부 가로 스크롤을 만들지 않고 `420px` 이하에서는 번호와 라벨을 세로로 접는다.
9. 마지막 입력 단계 하단에는 `이전`을 유지한다. 생성 차단 사유는 최종 확인 카드에 한 번만 표시하고, 생성 버튼 옆에는 실제 저장 실패 메시지만 표시한다.
10. 경기 기록의 상대 선수·확인 팀 선택은 `기록 기본` 단계에 둔다. 마지막 `메모` 단계까지 미루지 않는다.
