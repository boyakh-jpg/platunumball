...
11. 초기 원장 정리 기간에는 같은 서버 필터 결과를 한 행씩 보여주는 `1개씩 검수` 모드를 함께 제공한다. 현재 위치와 전체 건수, 이전, 변경 없이 다음, 저장하고 다음, 검수 종료를 한 control bar에 두며 미저장 값이 있으면 이동을 막는다. 마지막 행 다음에는 다음 서버 페이지를 불러오고 기존 100행 표와 일괄 저장은 대량 수정용으로 유지한다. 카드 기본 순서는 `명시적 검토 필요 → 검증 대기 → 출처 확인 또는 핵심 속성 보완 → 검증 완료`다. 원터치 판정은 `공개·비공개·읍면동 순번명·추가 확인·폐쇄·중복`만 제공한다. 시설명은 직접 수정하되 표준 구장명 미리보기에 즉시 반영하고, 이용 방식·시설 및 코트 속성은 큰 선택 chip으로 따로 정하며 선택 변경은 붉은 배경으로 표시한다. 검수 workspace는 desktop에서 근거 요약과 편집 영역을 나누고 mobile에서는 한 열로 쌓아 모든 판정·속성 버튼의 높이를 44px 이상 유지한다.
