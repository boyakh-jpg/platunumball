...
2026-07-18: 승인 구장 검색은 정확 검색 결과가 없을 때만 active 구장명·주소 토큰에서 한 글자 오타를 보조 매칭한다. 정확 결과가 있으면 기존 순서와 최대 노출 수를 유지한다.
...
1. 홈 검색의 구장 결과 row 전체는 `/app/courts/:courtId`로 이동한다. 검색 결과 안에 별도 상세 버튼을 중첩하지 않는다.
...
6. 등록 구장 hover 카드의 하단 액션은 `구장 보기`와 `지도 보기`를 한 행에 둔다. `구장 보기`는 `/app/courts/:courtId`로 이동하며 등록 ID가 없는 임시 구장에는 노출하지 않는다.
