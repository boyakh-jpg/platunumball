...
3. 이미지 선택 뒤 공용 크롭 dialog에서 원형 결과 미리보기, 확대·축소, 가로 위치, 세로 위치를 제공한다. 미리보기와 저장 canvas는 같은 계산을 사용한다.
...
6-2. 공용 `TeamEmblem`의 기본 글자는 각 줄의 가장 긴 글자 수를 기준으로 자동 조절한다. 한 글자는 원형 안전영역을 넓게 채우고, 여러 글자는 잘리지 않는 범위에서 단계적으로 축소한다.
6-3. 팀 엠블럼과 프로필 아이콘에서 테두리를 사용하면 후광도 저장된 테두리색을 사용한다. 테두리를 끄면 외곽 후광을 표시하지 않는다.
7. 데스크톱은 원본 선택과 색상 control을 가로로 배치한다. 프로필 아이콘 dialog의 세 원본 버튼은 모바일에서도 3열 compact 높이를 유지하고, 색상·팀 편집 control은 `720px` 이하에서 한 열로 내려 가로 overflow가 생기지 않게 한다. 아이콘 dialog와 크롭 dialog는 모바일에서 viewport 하단 sheet처럼 배치하고 내부만 스크롤한다.
