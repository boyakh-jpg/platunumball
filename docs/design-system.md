...
4. 구장 행의 `지도`와 `이름 변경`은 같은 작업 셀에 둔다. 이름 변경 중 시설명과 자동 계산된 구장명 셀은 저장 전까지 기존 danger 토큰을 섞은 연한 붉은 배경으로 표시하고, 저장 성공 뒤 기본 surface로 되돌린다.
5. 수정 중인 한 행에만 변경 사유와 저장 액션을 펼친다. 나머지 99행 높이는 바꾸지 않는다. dark/light 모두 공용 card, input, line, danger 토큰을 사용한다.
