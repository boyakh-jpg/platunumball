...
7. `친선전`, `경쟁전`은 경기 목적 grid에 두고 `경기 전 구성`, `현장 픽업`은 별도의 팀 구성 grid에 둔다. 두 축은 데스크톱에서 나란히, 태블릿에서는 세로로 배치하며 각 축의 두 선택은 충분한 너비에서 한 줄을 유지한다. 현장 픽업을 선택하면 참가 방식을 개인전으로 고정하고 팀전 option을 비활성화한다. 인원 영역은 `순환 대기 인원`, `현장 팀배정 · 방장 수동 순환`으로 표시하며 고정 선발·후보나 자동 로테이션이 있는 것처럼 안내하지 않는다. 최종 요약에는 친선·MMR 미반영과 수동 운영을 함께 표시한다.
