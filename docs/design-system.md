...
1. 일정 메뉴의 시작 전 경기·모집방, 매칭 메뉴의 공개방, 플레이 메뉴의 진행 경기는 같은 `om-match-card` 외곽 surface와 타이포 위계를 사용한다.
2. 비공개 대회 카드는 같은 외곽 surface·상태선·주황 액션 구조를 사용한다. 리그 fixture와 토너먼트 대진은 정보 밀도에 맞춘 compact 변형이며, 홈 확정·최근 경기는 홈 레일용 compact 변형이다.
3. 목록 카드의 팀 대진·점수·상태 정보는 외곽 카드 안에서 다시 회색 패널로 감싸지 않는다. 배지와 버튼만 자체 surface를 가진다.
