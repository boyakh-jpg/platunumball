...
2. 데이터 칼럼은 정렬 버튼과 바로 아래 필터 control을 함께 둔다. text/select/date filter는 `필터 적용` 또는 Enter 때만 서버를 호출하며 입력 중 매 글자마다 요청하지 않는다.
...
5. 표는 세로·가로 스크롤을 자체 처리하고 하단 가로 scrollbar 공간을 항상 확보한다. `네이버지도`, `수정` 또는 `저장·취소` 작업 열은 가장 왼쪽에 고정한다.
...
7. 수정 중인 한 행의 허용된 셀만 input/select로 바꾼다. enum·boolean은 dropdown으로 제한하고 연락처·공식 URL·예약 URL·운영 정보도 같은 행에서 편집한다. 변경된 셀과 자동 재계산된 표준명은 저장 전 danger 토큰의 붉은 배경으로 표시한다.
8. 셀에서 ESC를 누르면 전체 행을 닫지 않고 해당 셀만 편집 시작 전 값으로 복구한다. 저장은 4자 이상의 변경 사유를 요구하고 취소·탭 전환·필터·정렬·페이지 이동·modal 닫기 전에 미저장 변경 폐기를 확인한다.
