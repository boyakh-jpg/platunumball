...
1. 픽업방 빈 슬롯 검색은 선수만 반환한다. 검색창 placeholder는 `선수 검색`으로 표시하고 팀 검색 결과, 즐겨찾기 팀, 팀원 일괄 선택 패널은 렌더링하지 않는다.
2. 픽업 참가자의 `내 슬롯 관리`에는 A/B 출전과 A/B 순환 대기 이동만 표시한다. 팀원 명단 편집, 파티 합류·나가기, 파티장 배지는 표시하지 않는다.
3. 이동 버튼은 공용 작은 버튼 grid를 사용하며 비어 있는 위치만 활성화한다. 모바일에서도 네 이동 상태가 겹치거나 잘리지 않게 기존 반응형 슬롯 액션 규칙을 사용한다.
...
2. 선택 가능한 경기시간 프리셋이 하나뿐이면 프리셋 행을 숨기고 바로 세부 규칙을 표시한다. 단일 `공 제공` control은 2열의 절반만 차지하지 않고 가용 너비를 사용한다.
...
1. 공개·비공개 경기, 대회, 경기 기록, 개인 기록 생성은 공용 단계 nav를 사용한다. 공개·비공개 경기의 기본 흐름은 `기본 설정 → 규칙·운영 → 구장·비용` 3단계다. 현재 단계의 카드만 보여주고 `이전`·`다음`으로 이동하며 마지막 입력 단계 아래에 전체 요약과 생성 오류·주의·생성 버튼을 함께 표시한다. 확인만 하는 별도 단계는 만들지 않으며 유형과 무관한 단계·control은 빈 공간을 남기지 않고 건너뛴다.
2. 경기 성격 프리셋, 공개 범위, 1v1·2v2·3v3·5v5 선택은 기본 설정 안에서 같은 공용 control을 쓴다. 모드를 바꾸면 출전 정원, 시계 프리셋, 요약을 중앙 정책에서 함께 갱신한다.
...
7. `친선전`, `경쟁전`, `픽업`은 같은 프리셋 grid 안에 둔다. 세 프리셋은 데스크톱에서 3열로 배치하고 `640px` 이하에서는 1열로 배치한다. 픽업을 선택하면 참가 방식을 개인전으로 고정하고 팀전 option을 비활성화한다. 인원 영역은 `순환 대기 인원`, `현장 팀배정 · 방장 수동 순환`으로 표시하며 고정 선발·후보나 자동 로테이션이 있는 것처럼 안내하지 않는다. 최종 요약에는 MMR 미반영과 수동 운영을 함께 표시한다.
...
11. 구장 미선택 상태의 지도 데이터는 구장 단계에 진입할 때 선택 지역을 우선하고 `전체`이면 사용자 현재 지역으로 서버 조회한다. `active`이면서 위도·경도가 있는 구장만 지역별 최대 200개를 받고 일반 검색의 최대 25개 제한은 유지한다. 지역 판정은 `sigungu` 하나에만 의존하지 않고 지역·도로명·지번 주소를 함께 사용한다. 지역 구장 조회 중에는 빈 상태 대신 loading을 표시하고, 조회 결과는 기존 구장·검색 선택 결과와 ID 기준으로 병합한다.
...
12. `중복주소 코트번호 정리`는 미저장 셀이 없을 때만 실행한다. 처리 중 버튼을 잠그고 완료 후 전체 반영 구장 수와 중복 주소·코트 수를 같은 toolbar 상태 문구로 보여준다.
...
1. 방 만들기에서는 `정규전 반영`, `공식경기`, `사전등록`, `MMR 반영`, `검증 강화`, `명단 사전 확정`을 별도 체크박스로 노출하지 않는다. 친선·픽업·경쟁 프리셋이 기록 반영 여부를 결정한다.
