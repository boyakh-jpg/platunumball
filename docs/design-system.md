...
7. 데스크톱은 원본 선택과 색상 control을 가로로 배치한다. 프로필 아이콘 dialog의 세 원본 버튼은 모바일에서도 3열 compact 높이를 유지하고, 색상·팀 편집 control은 `720px` 이하에서 2열 compact 배치를 우선하되 저장 버튼은 필요한 경우 전체 너비를 사용해 가로 overflow가 생기지 않게 한다. 팀 사진 action은 모바일에서 안내를 한 행으로 두고 복원·선택 버튼을 2열로 맞춘다. 아이콘 dialog와 크롭 dialog는 모바일에서 viewport 하단 sheet처럼 배치하고 내부만 스크롤한다.
