...
## 2026-07-22 6단계 경기 생성과 추천 UI

1. 공개·비공개 경기, 대회, 경기 기록, 개인 기록 생성은 공용 단계 nav를 사용한다. 현재 단계의 카드만 보여주고 `이전`·`다음`으로 이동하며 최종 단계에서만 전체 요약과 생성 오류·주의를 함께 표시한다. 유형과 무관한 단계·control은 빈 공간을 남기지 않고 건너뛴다.
2. 경기 성격 프리셋과 1v1·2v2·3v3·5v5 선택은 모든 너비에서 같은 공용 control을 쓴다. 모드를 바꾸면 출전 정원, 시계 프리셋, 요약을 중앙 정책에서 함께 갱신한다.
