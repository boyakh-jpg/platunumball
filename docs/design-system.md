...
1. 경기 일정 관계 필터는 `전체 / 내가 만든 방 / 내 참여방 / 내 팀 참여방 / 초대받은 방`을 같은 버튼 계층으로 표시한다.
2. `내 팀 참여방`은 초기 목록에 섞지 않고, 버튼을 누른 동안만 팀 일정 조회 상태를 버튼 문구로 표시한다.
3. 좁은 경기 요약 카드의 팀명은 말줄임으로 잘라내지 않고 최대 두 줄로 줄바꿈한다. 점수·상태·액션은 팀명과 겹치지 않아야 한다.
4. 랜딩의 `매칭 만들기`와 `경기 기록하기`는 모바일에서도 한 행의 같은 폭 CTA로 표시한다.
5. 랭크보드는 데스크톱 사이드바와 홈 지역 랭킹 카드에서 진입할 수 있다. 모바일 하단 내비게이션 항목 수는 늘리지 않는다.
6. 프로필·팀·심판·구장 hover 카드를 터치로 열었을 때 닫기 버튼은 카드 오른쪽 안쪽 `14px`에 둔다. 카드 자체는 가로 스크롤을 만들지 않는다.
