...
2026-07-02: 홈 tier rail은 hero 내부가 아니라 오른쪽 최상단 rail에 두고 알림 카드보다 먼저 보여준다. tier rail은 통합 티어 배너와 최근 5경기 승수만 보여주고, 큰 티어명은 hero title font가 적용되게 영문 티어명으로 쓴다. 모드별/팀전 티어 카드는 홈에서 제외한다. 모집 중인 방과 결과 승인 대기는 오른쪽 보조 rail이 아니라 왼쪽 메인 라인에 각각 한 줄 카드로 둔다.
2026-07-04: 홈 `내 확정 경기` 카드는 실제 경기만 표시한다. 모집방은 아래 `모집 중인 방` 카드에 붙이고, 실제 경기가 3개를 넘으면 카드 하단에 전체 보기 버튼을 둔다. 모집 중인 방과 결과 승인 대기 목록의 제목만 스포츠 표시체와 주황색을 쓴다.
