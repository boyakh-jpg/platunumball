...
1. 모바일 터치 hover 카드는 고정 하단 내비게이션 높이와 safe area를 제외한 위치에 둔다. 카드와 전체 폭 액션이 하단 내비게이션에 가려지면 안 된다.
2. 터치 hover 카드 표면은 불투명한 `surface`를 사용한다. 라이트·다크 모두 뒤 카드의 글자가 비치지 않아야 한다.
3. 모바일 floating `SearchPicker` 결과는 화면 하단 sheet로 표시하고 하단 내비게이션 위에 고정한다. 짧은 viewport와 키보드가 열린 상태에서도 결과 목록 전체를 내부 스크롤로 확인할 수 있어야 한다.
