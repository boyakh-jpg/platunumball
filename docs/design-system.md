...
6. 경기 메뉴의 캘린더 구조 wrapper, section head, empty state는 투명하다. `내 진행 일정` 필터 summary는 외곽 card surface 하나를 가지며 내부 필터 wrapper는 투명하게 유지한다. 달력 본체, control group, 실제 경기·대회 항목은 각자 정해진 surface를 가진다.
...
9-2. 토너먼트 경기 일정 편집 행은 사이드바가 유지되는 `1220px` 이하에서 3열로 접어 저장·몰수 액션이 화면 밖으로 잘리지 않게 한다.
