...
1. 방 만들기에서는 `정규전 반영`, `공식경기`, `사전등록`을 각각 단독 체크박스로 나열하지 않는다. `기록 반영` 박스 안에서 `MMR 반영`, `검증 강화`, `명단 사전 확정`으로 표시한다.
2. 각 option은 한 줄 제목과 짧은 보조 설명을 함께 가진다. 좁은 화면에서는 체크박스와 텍스트가 줄바꿈되더라도 가로 overflow를 만들지 않는다.
