...
9. 행별 `네이버지도`는 BOXTIER 내부 지도 화면을 거치지 않고 네이버 지도 웹을 고정된 이름의 별도 브라우저 창으로 연다. 다음 행을 누르면 같은 창의 구장만 바뀐다. 좌표와 구장명을 전달해 임의 위치에도 핀과 정보창을 표시하고, 좌표가 없을 때만 주소를 검색한다. 초기 확대는 네이버 기본값을 따르며 핀 정보창의 파노라마 버튼은 네이버가 선택한 인접 거리뷰를 핀 방향으로 연다.
