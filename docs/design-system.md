...
6-1. 팀 `기본값` 편집은 기존 첫 글자 기본값/팀 이름/약칭 기준, 1~8자 약칭, 스포츠/고딕/명조/모노 글꼴 프리셋, 배경색, 테두리를 한 영역에서 제공한다. 공용 `TeamEmblem`이 팀 이름을 최대 세 줄로 줄이고 글자 크기를 자동 조절하므로 별도 이미지 파일을 만들지 않는다.
