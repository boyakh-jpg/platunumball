...
11. 구장 미선택 상태의 지도 데이터는 경기 생성 4단계에 진입할 때 현재 시군구로 서버 조회한다. `active`이면서 위도·경도가 있는 구장만 최대 200개를 받고 일반 검색의 최대 25개 제한은 유지한다. 지역 구장 조회 중에는 빈 상태 대신 loading을 표시하고, 조회 결과는 기존 구장·검색 선택 결과와 ID 기준으로 병합한다.
