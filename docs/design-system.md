...
2. 프로필 본문 카드는 현재 아이콘과 `아이콘 변경`, `업적 보기`만 표시한다. `아이콘 변경`은 별도 dialog를 열고 그 안에서 `기본값`, `Discord`, `아이콘`을 고른다. Discord는 연결된 프로필 사진이 있을 때만 선택할 수 있고 `아이콘`은 검수된 340개 카탈로그를 분류 탭과 compact grid로 보여준다. 직접 사진 업로드는 제공하지 않는다.
