...
1. 설정의 개인 엠블럼은 첫 글자, Discord, 직접 업로드 세 원본을 한 카드 안에서 선택한다. Discord 이미지가 없으면 해당 선택지만 비활성화하고 기본 첫 글자를 유지한다.
2. 개인·팀 엠블럼 편집기는 기본색, 테두리 사용 여부, 테두리색을 같은 control group에 둔다. 테두리를 끄면 테두리색 control은 비활성화한다.
...
5. 변경 확인 dialog에는 첫 2회 즉시 업로드와 이후 30일 제한을 저장 전에 표시한다. 색상·테두리만 바꾸는 경우 업로드 횟수에 포함되지 않음을 표시한다.
6. 데스크톱은 세 원본 선택과 색상 control을 가로로 배치한다. `720px` 이하에서는 한 열로 내려 가로 overflow가 생기지 않게 한다. 크롭 dialog는 모바일에서 viewport 하단 sheet처럼 배치하고 내부만 스크롤한다.
7. 공용 개인 엠블럼과 `TeamEmblem`은 이미지 실패 시 이름 첫 글자로 되돌아간다. 선택된 배경색과 테두리색은 dark/light 양쪽에서 같은 사용자 값으로 유지한다.
