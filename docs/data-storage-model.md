...
| Vercel/R2 public asset | `/assets/profile-icons/{icon-key}.png` | 활성 340개 검수 프로필 아이콘. `221~225`는 심판 시험 채점 완료 횟수 아이콘이며 DB에는 확장자 없는 key만 저장 |
...
- `221~225`는 `referee_exam_attempts`에서 `passed` 또는 `failed`로 채점이 끝나고 `finished_at`이 저장된 본인 시험만 세어 1·3·5·10·20회에 해금한다. 시작만 한 시험은 세지 않는다.
...
- Supabase 환경의 모집·경기·토너먼트 mutation은 operation-only server action과 authoritative DB RPC로 커밋한다. 브라우저 snapshot은 저장 원본으로 받지 않는다.
...
- Push worker도 같은 Vault secret을 사용하는 별도 Supabase Cron이 1분마다 `/api/notifications/push-worker`를 호출한다. Vercel Cron과 외부 `cron-job.org`는 사용하지 않는다.
...
- Match server action은 앱 웹 알림을 원본으로 시작 1시간 전 참가자·심판 안내, 시작 20분·10분 전 미출석 선수 QR 안내, 시작 10분 전 현재 방관리자 출석 현황, 경기 종료, 설정된 이의신청 마감 5분 전 안내를 만든다. Discord 연결·수신 설정을 통과한 대상에게만 같은 알림 ID의 delivery row를 추가하며 경기 전 delivery는 예약시각에서 90초 이상 늦으면 취소한다. worker는 발송 직전 현재 Discord 연결 ID도 다시 확인해 연결 해제·변경 뒤 예전 ID로 보내지 않는다. 이의신청 안내 시각은 10분·15분·20분 창에 맞춰 종료 5분·10분·15분 뒤로 계산한다.
- 일정·실제 참여 명단·방관리자가 바뀌면 미발송 경기 전 웹 알림과 Discord delivery를 현재 대상자 기준으로 재생성한다. 전원 출석 시 방관리자 웹 알림은 고유 ID로 한 번만 만들고, 경기 시작·취소·무효 시 남은 경기 전 예약 row를 정리한다.
...
- Tournament create/approve uses `rankball_tournament_operation_action()` for bracket validation, first-round generation, and atomic persistence.
...
- Tournament follow-up rounds are generated by the confirmed-match DB trigger when both source winners are available.
...
- The same full simulation includes a 3-team `tournament_bye_round` scenario that verifies one first-round bye and automatic final creation after the remaining first-round match is confirmed.
...
- Backend flow simulation includes `match_agree_sql_reducer`, which creates a 2v2 room, records one non-final `agreeMatch` operation-only call, and verifies the `rankball_match_agree_action()` SQL reducer path.
...
- Configure production Discord Bot token, bridge secret, room channel/thread links, and a long-running bridge worker.
...
## 2026-07-13 authoritative operation boundary

- Recruiting, match, and tournament mutation endpoints require `{ operation }`; raw client snapshots are rejected.
- The frontend mutation repository sends operation-only payloads. Local reducers remain only for non-Supabase demo mode.
- Recruiting management mutations use `rankball_recruiting_management_action()` or their dedicated locked RPC. Missing reducers return `503` instead of replaying a client snapshot.
- Match result/stat merge, referee absence, room/record roster edits, player removal, dispute resume, final approval, and MMR use locked DB RPCs. Final rating changes commit in the same transaction as match confirmation.
- Tournament validation, first-round creation, and follow-up generation use `rankball_tournament_operation_action()` and the confirmed-match advance trigger.
