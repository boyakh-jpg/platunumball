...
- The frontend sends `submitMatchResult`, `disputeMatch`, and `resumeMatchApproval` as operation-only; the server reloads authoritative state, builds result/dispute/finalization snapshots, and persists the result instead of trusting a client match snapshot.
