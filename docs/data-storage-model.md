...
- The frontend sends `agreeMatch`, `approveMatch`, `checkInMatchPlayer`, `startMatch`, `endMatch`, `submitMatchResult`, `disputeMatch`, `resumeMatchApproval`, `submitMatchThumbs`, and `toggleMatchStar` as operation-only; the server uses SQL reducers where supported and otherwise reloads authoritative state before persisting attendance, lifecycle, result, dispute, finalization, rating, or trust updates.
