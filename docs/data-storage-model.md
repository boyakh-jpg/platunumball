...
- The frontend sends `submitMatchResult`, `approveMatch`, `disputeMatch`, `resumeMatchApproval`, `submitMatchThumbs`, and `toggleMatchStar` as operation-only; the server uses SQL reducers where supported and otherwise reloads authoritative state before persisting result, dispute, finalization, rating, or trust updates.
