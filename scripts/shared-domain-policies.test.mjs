...
test("referee rulebook matches current FIBA and BOXTIER operating rules", async () => {
  ...
  assert.match(rulebookText, /개인기록과 턴오버\(TO\)/);
  const turnoverGuideline = REFEREE_STAT_GUIDELINES.find((item) => item.stat === "턴오버(TO)");
  assert.equal(turnoverGuideline?.scene, "stats");
  assert.match(JSON.stringify(turnoverGuideline), /슛 실패.*리바운드.*턴오버가 아닙니다/);
  assert.match(JSON.stringify(REFEREE_RULEBOOK_EASY_SECTIONS), /공격권을 상대에게 넘긴 원인이 된 선수에게 턴오버\(TO\)/);
  assert.match(rulebookText, /공격권을 상대에게 넘긴 원인이 된 선수에게 턴오버\(TO\)/);
  ...
  assert.match(tutorial, /공격권을 상대에게 넘긴 원인이 된 선수에게 턴오버\(TO\)/);
...
test("경기 영수증은 필수값과 승패를 실행형 정책으로 판정한다", () => {
  ...
  assert.deepEqual(getMatchReceiptOutcome({ ...base, homeScore: 21, awayScore: 18 }), { key: "home", label: "WIN A · LOSS B" });
  assert.deepEqual(getMatchReceiptOutcome({ ...base, homeScore: 18, awayScore: 21 }), { key: "away", label: "WIN B · LOSS A" });
