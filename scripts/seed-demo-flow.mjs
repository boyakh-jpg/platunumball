...
function withoutRetiredRecorderFields(value) {
  if (Array.isArray(value)) return value.map(withoutRetiredRecorderFields);
  if (!value || typeof value !== "object") return value;
  return Object.fromEntries(
    Object.entries(value)
      .filter(([key]) => !RETIRED_RECORDER_FIELDS.has(key))
      .map(([key, child]) => [key, withoutRetiredRecorderFields(child)]),
  );
}
...
state = withUser(state, "u6", (scoped) => resolveMatchDispute(
  scoped,
  lifecycleMatchId,
  lifecycleDisputeId,
  "rejected",
...
state = withUser(state, "u1", (scoped) => resolveMatchDispute(
  scoped,
  lifecycleMatchId,
  lifecycleDisputeId,
  "rejected",
  "방장은 심판 경기 이의를 판정할 수 없음",
));
assertFlow(getMatch(state, lifecycleMatchId).status === "disputed", "심판 경기 방장은 이의 판정 불가", {
  status: getMatch(state, lifecycleMatchId).status,
});

state = withUser(state, "u11", (scoped) => resolveMatchDispute(
  scoped,
  lifecycleMatchId,
  lifecycleDisputeId,
  "rejected",
...
assertFlow(getMatch(state, lifecycleMatchId).status === "approval", "심판 이의 판정 후 최종 승인 대기", {
  status: getMatch(state, lifecycleMatchId).status,
});
...
state = withoutRetiredRecorderFields(state);
...
console.log(JSON.stringify({
  ...
  checkOnly,
  storageKey: STORAGE_KEY,
  statePath: checkOnly || generatedOnly ? null : stateOut.pathname,
  reportPath: checkOnly || generatedOnly ? null : reportOut.pathname,
