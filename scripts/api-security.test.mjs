...
test("public landing exposes aggregate counts and bounded public feed fields", async () => {
  ...
  assert.match(source, /select\("id,type,title,mode,court_id,court_name,region,scheduled_date,scheduled_time,scheduled_at,timing_type:room_state->>timingType,ranked,official,pre_registered,rating_scale,age_restriction,allowed_age_groups,rules,stakes,court_reserved,court_fee,spots,referee_wanted,referee_trust_min,stat_entry_minutes,dispute_minutes,host_join_mode,side_capacity,bench_capacity"\)[\s\S]*?\.limit\(recruitingLimit\)/);
  ...
  assert.doesNotMatch(source, /player_ids|room_state(?!->>timingType)|created_by|referee_id|memo|evidence/);
});
