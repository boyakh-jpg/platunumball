...
test("심판 stats 전용 프로필 표시와 score-only 정책 계약을 고정한다", async () => {
  ...
  assert.match(matchRoomSource, /const open = Boolean\(statEditorPlayer && \(hasReferee \|\| isSoloRecord\)\)/);
...
test("RPC grant health distinguishes current entry points from retired signatures", async () => {
  const [
    ...
    readFile(
      new URL(
        "../supabase/migrations/20260804121640_court_request_submission_limits.sql",
        import.meta.url,
      ),
      "utf8",
    ),
    readFile(
      new URL(
        "../supabase/migrations/20260805160000_register_reject_court_request_rpc.sql",
        import.meta.url,
      ),
      "utf8",
    ),
    readFile(
      new URL(
        "../supabase/migrations/20260806123000_expand_team_membership_and_profile.sql",
        import.meta.url,
      ),
      "utf8",
    ),
    readFile(
      new URL(
        "../supabase/migrations/20260806202031_register_account_withdrawal_rpc.sql",
        import.meta.url,
      ),
      "utf8",
    ),
    readFile(
      new URL(
        "../supabase/migrations/20260806213000_confirmed_match_participation_cancel.sql",
        import.meta.url,
      ),
      "utf8",
    ),
...
public.rankball_match_participation_cancel_action(text,text,text)
...
public.rankball_reject_court_request(text,integer,text,text)
