...
export async function queueMatchDiscordDeliveries(supabase, match = {}, action = "sync") {
  ...
  if (action === "endMatch") {
    ...
    addRows(participantIds, profiles, {
      ...
      sendAt: new Date(endedAt.getTime() + disputeReminder.offsetMinutes * MINUTE_MS).toISOString(),
    });
  }
...
function toMatchRow(match = {}, actorProfileId = "") {
  ...
  const recordStartedAt = recordType === RECORD_TYPES.matchRecord
    ? parseMatchScheduleDate(`${match.scheduledDate ?? schedule.scheduledDate ?? ""} ${match.scheduledTime ?? schedule.scheduledTime ?? ""}`)
